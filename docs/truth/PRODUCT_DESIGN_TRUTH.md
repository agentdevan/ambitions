...
As of Linear `AMB-1544` / `AMB-1545`, new backend/runtime authority belongs under `Core/LocalRuntimeOS/`. As of `AMB-1546`, the command-spine source and the current `RuntimeMutation` transaction-kernel bridge have moved under `Native/Ambitions/Core/LocalRuntimeOS/`. As of `AMB-1547`, source-present EventJournal foundation files exist under `Native/Ambitions/Core/LocalRuntimeOS/EventJournal/` with focused append-only, checksum, replay, compaction, tombstone, and command-event append tests. As of `AMB-1548`, source-present ProjectionEngine foundation files exist under `Native/Ambitions/Core/LocalRuntimeOS/ProjectionEngine/` with focused definition, cursor, invalidation, checksum, materialization, surface projection, read-model inventory, and diff tests. Existing source that remains under `Core/Runtime/`, `Core/Persistence/`, and legacy projection owners is implementation scaffolding and migration debt unless a later source train moves or proves the responsibility under `Core/LocalRuntimeOS/`. This tree names the target owner; it does not prove that the full LocalRuntimeOS storage split, complete replay system, side-effect outbox, privacy firewall, sync continuity, migration repair, diagnostics implementation, or all-mutation command/event/projection/receipt/replay enforcement exists today.
