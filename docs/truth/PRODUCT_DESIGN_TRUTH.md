...
Claim-status normalization for the backend/runtime architecture cluster:

- Cluster status: Partial / Implemented Yellow unless current validation is rerun
  and linked for the exact subclaim.
- Implemented Green subclaims are limited to exact source/runtime gates named
  ...
  they do not expand to release, visual, device, privacy/legal, account, R2,
  CloudKit, or total product completion.
- Partial subclaims include source-present LocalRuntimeOS foundation ownership
  and focused-test history for the named subtrees below.
...
- Non-claim: this product/design truth cluster is not build, test, device,
  visual, accessibility, privacy/legal, account, R2, CloudKit, TestFlight,
  App Store, release, or product-completion proof.

As of Linear `AMB-1544` / `AMB-1545`, new backend/runtime authority belongs under `Core/LocalRuntimeOS/`. As of `AMB-1546`, Commands source moved under `Native/Ambitions/Core/LocalRuntimeOS/Commands/`, and the initial `RuntimeMutation` transaction bridge moved under `Native/Ambitions/Core/LocalRuntimeOS/Transactions/`. As of `AMB-1547`, source-present EventJournal foundation files exist under `Native/Ambitions/Core/LocalRuntimeOS/EventJournal/` with focused append-only, checksum, replay, compaction, tombstone, and command-event append tests. As of `AMB-1548`, source-present Projections foundation files exist under `Native/Ambitions/Core/LocalRuntimeOS/Projections/` with focused definition, cursor, invalidation, checksum, materialization, surface projection, read-model inventory, and diff tests. As of `AMB-1549`, source-present Storage foundation files exist under `Native/Ambitions/Core/LocalRuntimeOS/Storage/`, the SwiftData object-store source is owned by `ObjectStoreSwiftData.swift`, and focused simulator tests cover SQLite event/projection/search stores, redacted App Group snapshots, file/blob storage, encrypted backup packages, and migration dry-run metadata. As of `AMB-1560`, source-present Search files exist under `Native/Ambitions/Core/LocalRuntimeOS/Search/`, the former `Core/Runtime` local search index owner has been removed, and focused simulator tests cover canonical owner files, local search ranking preservation, FTS-backed Find / Act / Inspect results, provenance, privacy filtering, action validation, deterministic local semantic ranking, and projection-fed rebuild. As of `AMB-1553`, source-present Boundary foundation files exist under `Native/Ambitions/Core/LocalRuntimeOS/Boundary/`, the former `Core/Runtime` privacy/source-atlas boundary owners and former `Core/Persistence` Source Atlas public-artifact privacy-boundary owner have been removed from those owners, and focused simulator tests cover local-only capability decisions, offline-core account independence, private-graph sync denial, public-reference/account-identity egress allowance, private runtime/R2 egress denial, hosted private graph denial, external cloud LLM denial, Source Atlas access decisions, private-graph egress markers, and public-artifact cache metadata. As of `AMB-1554`, source-present State files exist under `Native/Ambitions/Core/LocalRuntimeOS/State/`, define the ten required object-state store families, keep SwiftData out of canonical mutation authority, and prove only the `AppStateStore` SwiftData adapter through command/event/projection/receipt/replay context enforcement. As of `AMB-1561`, source-present ExternalWrites foundation source exists under `Native/Ambitions/Core/LocalRuntimeOS/ExternalWrites/`, the former side-effect ledger domain and persistence owners have been moved into that owner, and focused simulator tests cover policy gating, local-commit-before-external-attempt behavior, lease/result recording, notification outbox recording, EventKit outbox recording, and private notification payload minimization. As of `AMB-1555`, source-present PrivateLifeRuntimeKernel foundation source exists under `Native/Ambitions/Core/LocalRuntimeOS/PrivateLifeRuntimeKernel/`, the former runtime-kernel, replayable-decision-trace, and personalization-factor-ledger owners have been moved into that owner, and focused simulator tests cover local-only boundary behavior, deterministic decision records, recommendation gating, life-context runtime effects, personalization factor replay, source ownership, and typed local-runtime signal preservation. As of `AMB-1556`, source-present Planning source exists under `Native/Ambitions/Core/LocalRuntimeOS/Planning/`, the former `Core/Domain/Planning` files, `Core/Domain/GoalEngine` StepCandidateField model files, and `Core/Runtime` StepCandidateField/SourceAtlas bridge files have been removed from those owners, and focused simulator tests cover canonical owner files, old owner removal, deterministic planning graph construction, dependency resolution, progress preservation, smaller-step proposal selection, plan repair tracing, unscoped free-floating step planning, moved planning evaluation behavior, and the StepCandidateField simulation gauntlet. As of `AMB-1557`, source-present Scheduling files exist under `Native/Ambitions/Core/LocalRuntimeOS/Scheduling/`, the former `Core/Runtime` protected-placement and priority-placement policy files have been removed from that owner, the rendered Time placement coordinator evaluates placement through `PlacementEngine` before mutation, recurring occurrence generation uses `RecurrenceEngine`, and focused simulator tests cover canonical owner files, old owner removal, protected placement review, graph conflict surfacing, DST-safe recurrence, and local calendar store persistence. As of `AMB-1558`, source-present CaptureRouting files exist under `Native/Ambitions/Core/LocalRuntimeOS/CaptureRouting/`, the former `Core/Domain` capture routing and command-mapping owner files have been removed, and focused simulator tests cover durable intake, route decisions, draft/direct lookup indexes, attachment checksum/quarantine behavior, correction ledger writes, promotion transaction receipts, and DefaultCaptureService routing. As of `AMB-1559` and the AMB-1669 rename slice, source-present Inspection source exists under `Native/Ambitions/Core/LocalRuntimeOS/Inspection/`, the former `Core/Domain` receipt/event/proof/tombstone/replay model owners, former `Core/Runtime/ProofLedger.swift` owner, and former `Core/Persistence` trust-history and goal-intent receipt recorder owners have been removed, and focused test source covers command/event/receipt/proof/source/audit/undo/history/replay planning behavior. As of `AMB-1564`, source-present PrivacySecurity foundation source exists under `Native/Ambitions/Core/LocalRuntimeOS/PrivacySecurity/`, the former `Core/Persistence` storage privacy/security boundary owner has been moved into that owner, and focused simulator tests cover privacy classification, redaction, egress firewall receipts, export review gating, local-auth gating, file protection, encrypted blob-vault round trip, and privacy manifest runtime mapping. As of `AMB-1565`, source-present Repair files exist under `Native/Ambitions/Core/LocalRuntimeOS/Repair/`, the former `Core/Persistence` migration/repair owners have been removed from that owner, and focused simulator tests cover schema ledger validation, migration planning, non-executable repair-plan gating, pre-migration backup receipts, invariant checking, non-destructive corruption quarantine/runtime doctor behavior, restore rollback, dry-run review, canonical owner removal, and the later `AMB-1597` RuntimeDoctor redacted local drift readers with receipt-backed preview repair plans. As of `AMB-1566`, source-present Diagnostics foundation source exists under `Native/Ambitions/Core/LocalRuntimeOS/Diagnostics/`, and focused simulator tests cover canonical owner files, redacted deterministic diagnostic records, runtime trace inspection, duplicate-safe projection inspection, store health sample diagnostics, and performance budget overage visibility. As of `AMB-1562`, source-present Continuity files exist under `Native/Ambitions/Core/LocalRuntimeOS/Continuity/`, the former `Core/Persistence` SyncCapability/CloudKit continuity owner files and former `Core/Domain/Planning` LivingPlan continuity owner file have been removed, and focused simulator tests cover local-device authority, proof-backed continuity eligibility, private-graph payload denial, account-state diagnostics, deterministic merge/conflict quarantine, tombstone propagation limits, and sign-out cleanup retaining local data. As of `AMB-1563`, source-present SourceAtlas files exist under `Native/Ambitions/Core/LocalRuntimeOS/SourceAtlas/`, the former `Core/Domain`, `Core/Persistence`, and `Core/Runtime` SourceAtlas owner files have been removed from those owners, and focused simulator tests cover canonical owner files, public-only request compilation, private-graph request rejection, manifest verification, freshness evaluation, last-known-good cache selection, safe public R2 GET request metadata compilation, and SourceAtlas projection materialization. SourceAtlas proof does not prove R2 production deployment, public transparency logs, production signing operations, app-wide SourceAtlas consumption, or full LocalRuntimeOS completion. Existing source that remains under `Core/Runtime/`, `Core/Persistence/`, and legacy projection owners is implementation scaffolding and migration debt unless a later source train moves or proves the responsibility under `Core/LocalRuntimeOS/`. This tree names the target owner; it does not prove that the full LocalRuntimeOS storage split is live across every app path, the complete replay system is consumed everywhere, every app-wide planning/pathing consumer uses Planning, every scheduling/time/recovery/recurrence path consumes Scheduling, every capture entrypoint consumes CaptureRouting, every trust/proof/source/history/receipt/undo path consumes Inspection, every capacity/recovery/correction/projection invalidation path is kernel-owned, every object family has moved to ObjectState stores, side-effect outbox enforcement covers every external-write path, app-wide privacy firewall/network-egress enforcement beyond the current source/runtime gates, production CloudKit continuity, destructive or executed migration/repair automation beyond receipt-backed previews, app-wide diagnostics consumption or repair routing, app-wide Search consumption by every search or Memory Lens path, or all-mutation command/event/projection/receipt/replay enforcement exists today.

As of `AMB-1567`, source-present Transactions foundation files exist under `Native/Ambitions/Core/LocalRuntimeOS/Transactions/`, including `RuntimeTransaction`, `RuntimeTransactionCoordinator`, `RuntimeMutationPlan`, `RuntimeWriteSet`, `RuntimeReadSet`, `RuntimeCommitReceipt`, `RuntimeRollbackPlan`, `RuntimeConflictDetector`, `RuntimeIdempotencyStore`, and `RuntimeMutation`. Focused simulator tests cover canonical owner files, old owner removal, validated transaction preparation, event/projection-backed commit receipts, rollback plans, duplicate idempotency replay, stale read-set conflict detection, and invalid-command rejection. This proof is bounded runtime evidence, not a claim that every app path is command-only or that LocalRuntimeOS is complete.

As of `AMB-1568`, source-present Commands files exist under `Native/Ambitions/Core/LocalRuntimeOS/Commands/`, including `CommandEnvelope`, `CommandCompiler`, `CommandValidator`, `CommandAuthorizer`, `CommandIdempotencyKey`, `CommandJournal`, `CommandReducer`, `CommandResult`, `CommandReceiptFactory`, and `CommandReplayAdapter`. Focused simulator tests cover canonical owner files, durable command-journal append before scoped capture mutation, idempotency replay lookup, authorization denial, reducer policy output, typed receipt metadata, Today command routing, app-container journal wiring, and no mutation after command-journal append failure. This proof is bounded runtime evidence, not a claim that every app path is command-only or that LocalRuntimeOS is complete.
  ...
  Core/
    ...
    LocalRuntimeOS/
      ...
      Repair/
        ...
        RuntimeDoctorRepairTypes/
        RuntimeDoctorHealthReaders/
        RuntimeDoctorRepairPlans/
        RepairPlanEngine/
        RestoreRollback/
...
- ==Projection/Commands/== as the final command owner
- direct repository/service mutation paths outside `Core/LocalRuntimeOS/Commands/`, `Transactions/`, `EventJournal/`, `Projections/`, `Receipt`, and `Replay` semantics
