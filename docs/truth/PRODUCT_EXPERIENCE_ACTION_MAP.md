...
Last audited commit: P1C final commit recorded in train closeout
...
|---|---|---|---|
| Foundation Requirement | Reminders, Steps, recurring Steps, quick Capture, calendar-grade Time, search, notifications, completion, missed Step recovery, local persistence, offline core | Make Ambitions useful as a standalone personal planning product | Partial source/runtime support exists; P1A adds focused local Step lifecycle proof, P1A.1 adds rendered Step detail/recovery controls proof, P1A.2 adds normal Day Rail Step row tap-to-complete/move/recovery proof, P1B adds recurring Step runtime/persistence proof, and P1C adds scoped Capture-to-Step local-save/runtime persistence proof; full creation, accessibility-sweep, no-network device workflow, visual, and release proof are not established |
...
| Scoped P1A/P1A.1/P1A.2/P1B/P1C Step lifecycle and Capture-to-Step source/runtime/rendered-control proof | `Native/Ambitions/Core/Runtime/SimpleStepLifecycleService.swift`, `Native/Ambitions/Core/Runtime/CaptureService+03-DefaultCaptureService.swift`, `Native/Ambitions/Projection/SurfaceLenses/TodayFeatureService+02-RepositoryBackedTodayService+Repository05-performFeedbackAction.swift`, `Native/Ambitions/DesignSystem/ProductObjects/TodayDayRailPanels+02-AmbitionsDayRailView+04-upNextRow.swift`, `Native/AmbitionsTests/Today/TodayCommandHandlerTests.swift`, `Native/AmbitionsTests/Runtime/RecurringStepLifecycleServiceTests.swift`, `Native/AmbitionsTests/Persistence/CaptureServiceTests.swift`, `Native/AmbitionsUITests/AmbitionsUITests.swift` |
...
| Foundation Step and Capture local-save lifecycle | Focused P1A source/runtime test covers create, Today projection, reschedule, missed recovery, completion, local evidence, and SwiftData-backed reload; P1A.1 UI test covers rendered Step detail controls and closure/recovery outcome controls; P1A.2 UI tests cover normal Day Rail Step row tap-to-detail, complete feedback, move feedback, and recovery options; P1B runtime tests cover recurring Step creation, generated occurrences, completion without ending recurrence, pause/resume, SwiftData-backed reload, and local-only runtime boundaries; P1C runtime tests cover Capture Step-route save, existing Capture route-to-Step, Today projection target, SwiftData-backed reload, and local-only runtime boundaries | Full reminder creation UI, full recurring Step UI/accessibility proof, rendered Capture-to-Today UI proof, no-network device workflow, notification delivery, and release proof are not established |
