...
Last audited commit: P1B final commit recorded in train closeout
...
|---|---|---|---|
| Foundation Requirement | Reminders, Steps, recurring Steps, quick Capture, calendar-grade Time, search, notifications, completion, missed Step recovery, local persistence, offline core | Make Ambitions useful as a standalone personal planning product | Partial source/runtime support exists; P1A adds focused local Step lifecycle proof, P1A.1 adds rendered Step detail/recovery controls proof, P1A.2 adds normal Day Rail Step row tap-to-complete/move/recovery proof, and P1B adds recurring Step runtime/persistence proof; full creation, accessibility-sweep, device, and release proof are not established |
...
| Scoped P1A/P1A.1/P1A.2/P1B Step lifecycle source/runtime/rendered-control proof | `Native/Ambitions/Core/Runtime/SimpleStepLifecycleService.swift`, `Native/Ambitions/Projection/SurfaceLenses/TodayFeatureService+02-RepositoryBackedTodayService+Repository05-performFeedbackAction.swift`, `Native/Ambitions/DesignSystem/ProductObjects/TodayDayRailPanels+02-AmbitionsDayRailView+04-upNextRow.swift`, `Native/AmbitionsTests/Today/TodayCommandHandlerTests.swift`, `Native/AmbitionsTests/Runtime/RecurringStepLifecycleServiceTests.swift`, `Native/AmbitionsUITests/AmbitionsUITests.swift` |
