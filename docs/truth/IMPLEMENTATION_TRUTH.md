...
- `Native/Ambitions/Core/LocalRuntimeOS/` is only partially source-present. It does not prove the full local runtime OS until later implementation trains cover full cross-path replay, active app-wide storage consumption, full app-wide side-effect outbox enforcement, app-wide PrivateLifeRuntimeKernel ownership of planning/capacity/recovery/correction/projection invalidation paths, privacy firewall, migration repair, diagnostics, and continuity.
