...
The retained gate parses the artifacts and derives the decision facts. The deterministic candidate identity binds candidate ID, proposed target, exact source set and hash, live full repository HEAD, `project.yml`, and the changed-file router authority. Every authorization artifact repeats that identity and live authority binding, and authorization requires a clean tracked worktree with repository-contained, non-symlink source and authority files so live bytes cannot diverge from the bound HEAD. The history artifact must equal the live first-parent commit window and recomputed per-file touch maps. Compiler evidence links a raw result containing the `swiftc` or `xcrun swiftc` invocation, transcript, typed diagnostics, and exit result; the gate derives target, exact sources, zero errors, and agreement with declared graph dependencies. Graph nodes, edges, memberships, and test targets are parsed to recompute cycles, single production membership, and host reachability. A linked compiler symbol graph provides a nonempty set of public USRs and derived signature hashes; each must have exactly one approved, tracked, repository-contained consumer contract. Raw route rows must cover every source file with both module and hosted integration test identifiers that the corresponding raw test logs prove executed. Test summaries must identify the expected lane and target, exit successfully, execute tests, use distinct result identities, bind an `.xcresult` `Info.plist`, and match their executed identifiers, count, and zero-failure result to a hash-bound raw log.

Benchmark authorization reads exactly the policy-owned sample count from typed wrappers that link hash-bound, retained `benchmark-summary.json`-shaped raw summaries and separate focused-test or build-for-testing summaries. Candidate cohorts bind the live HEAD; baseline cohorts bind one real first-parent ancestor. All samples use warm canonical `.codex/DerivedData/Ambitions` (relative or the exact repository absolute path), one comparable package/Xcode/macOS/CPU identity, one simulator identity for test cohorts, distinct execution identities, stable commands/lanes/scenarios within each cohort, and durations derived from the raw summaries. Focused-test results additionally link their `.xcresult` `Info.plist` and raw log; build results link a raw log with a build-success marker. The independent review must name different author and reviewer identities, approve with empty Critical and Important arrays, and fingerprint every history, compiler, graph, symbol graph, API, route, test, result-bundle metadata, raw log, benchmark wrapper, raw benchmark summary, and focused/build result artifact. A missing referenced artifact remains `observed`; a present malformed, swapped, escaped, or digest-mismatched artifact is `rejected`. Only complete current derived evidence may evaluate `authorized`.
