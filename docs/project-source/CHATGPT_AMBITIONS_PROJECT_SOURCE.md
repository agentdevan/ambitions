...
- Codex must not be given vague “fix this” work.
- Every execution bundle requires a repo-backed dossier.
- Runtime app paths must be real.
- No fake success, fake placement, dead controls, placeholder routes, or source-only closure.
- Copy must be minimal and icon-first.
- Root surfaces must not expose internal architecture names.
- Proof is required before Done.
- ChatGPT may help design/spec/critique, but repo canon is authoritative.
...
- Do not introduce root headers with internal object names.
- Do not reintroduce bordered dock or visible tab labels.
- Do not reintroduce quick-capture half sheet.
- Do not use `Task` as the user-facing model; use Step.
- Do not create fake placement or fake success.
