...
The frontmatter and recorded lifecycle events are the sole authoritative sources for the lifecycle state of the current revision. Revision 1 was sealed at `2026-08-03T13:06:04Z` with contract hash `sha256:40d8bfa07e1e374c3345b617d1370e79ea696c28befe76e7d266883ff04416e0`, then received the revision-bound Content verdict `NEEDS REVISION`. Revision 2 was created by the recorded reopen event, sealed at `2026-08-03T15:24:29Z` with contract hash `sha256:cbbdc69545cc0aa9a662a847aedc5f3446722891fce6390ba8ad9d88e94ad9e0`, received the revision-bound Content verdict `NEEDS REVISION`, and was followed by the recorded reopen event that created revision 3. Those prior revision records remain historical and do not bind a later revision.

The document repository baseline remains `fc8eca8d7b913a3d6cef82e33bfd3c7f419c0021`, the commit that introduced the hashed comparison evidence. That evidence records the earlier pre-revision-1 comparison against remote HEAD `2199f8de3b19dddf16cb42d995f77a581ddee03d`. For this correction, the required repository files were inspected at the exact pre-write branch HEAD `34fa6190265d94afecae1522657a91132321c9b8`. This later inspection does not replace the preserved document baseline or create lifecycle authority. The active package and selected template identities remain bound in frontmatter.

The principal repository finding is unchanged: the installed package defines immutable phase templates, exact package and template identity, committed evidence hashes, adjacent-phase input binding, canonical paths, revision-bound sealing and review, and a drift split in which changes under recorded freshness paths are relevant while other changed paths are reported as unrelated. The installed acceptance test exercises a synthetic Research → Scope → Design chain and both drift classes.

No lifecycle CLI execution or validation is claimed by this authoring task. Scope is permitted only when the recorded lifecycle fields and events show the required seal and passed reviews for the exact Research revision being consumed. Historical seals and reviews cannot be reused across revisions.
...
6. How should prior sealed and reviewed revisions be represented without making body claims about the current revision's lifecycle state?
7. What remains unknown until downstream lifecycle fields and events record the required authority for Scope authoring?
...
The active skill requires one role at a time. Producer work inspects current canon, source, tests, evidence, and initiative files; creates a self-contained canonical document; commits the completed authoring result; and preserves the phase authority boundary. Research has `authority_class = "evidence"` and cannot commit product behavior or implementation architecture.

The lifecycle contract makes revisions and state binding explicit. A `needs-revision` or stale document may reopen as a new revision before authority-bearing edits. The implementation increments the revision, clears the current revision's seal and review bindings, and appends a reopen record while preserving earlier seal and review history. The lifecycle state of any revision is therefore determined by its frontmatter and recorded events, not by narrative body text.
...
- lifecycle validation binds seal and review fields to exact revisions and contract hashes;
- sealing derives freshness paths, computes a contract hash, and binds both to the revision being sealed;
- a rejected or stale document can reopen only as a new revision, with current revision seal and review fields reset while prior events remain in history;
...
Author-declared provenance, not repository evidence: no originating chat, previous lifecycle conversation, or local checkout was used for this revision 3 correction. This declaration is not used to support any `FIND-*` claim. No lifecycle CLI execution or CLI validation is claimed.
...
| Treat a prior revision's seal, contract hash, freshness paths, or review as authority for a later revision | Avoids later lifecycle work | Contradicts revision binding and can make historical records appear current | Rejected |
| State the current lifecycle status, contract hash, freshness set, review state, or next transition in narrative body text | Gives readers a prose shortcut | The statement can become false when the same revision transitions while the body remains unchanged | Rejected |
| Keep lifecycle-state authority in frontmatter and recorded events while expressing downstream conditions generically | Preserves audit history and keeps body claims stable across lifecycle transitions | Readers must consult the authoritative lifecycle records for current state | Recommended |

## Findings

| Finding ID | Classification | Finding | Source IDs | Scope implication |
|---|---|---|---|---|
| FIND-001 | Fact | The committed comparison evidence records that the supplied branch and remote HEAD were identical before original fixture authoring, and that the evidence baseline was the direct child commit containing `comparison.md`. | SRC-012 | A later fixture Scope can retain the auditable evidence baseline while binding the exact passed Research revision and commit recorded at that time. |
...
| FIND-008 | Fact | Revision 1 and revision 2 each have explicitly revision-bound seal and Content review records, and each subsequent reopen event created a later revision while preserving those records as history. | SRC-006, SRC-007, SRC-015 | A later Scope must bind only the exact Research revision, contract hash, review records, and commit that the authoritative lifecycle fields show as eligible for consumption. |
| FIND-009 | Bounded conclusion | Repository evidence is sufficient to recommend a fixture-only Scope direction, but this Research body does not determine current lifecycle status, seal values, review outcomes, consumer eligibility, or future drift. Those facts belong exclusively to frontmatter and recorded lifecycle events. | SRC-003, SRC-005, SRC-006, SRC-007, SRC-008, SRC-015 | Downstream work must read and bind actual lifecycle records and actual drift instead of inferring current state from narrative prose or pre-filling expected outcomes. |

## Recommended direction

Maintain this Research as the canonical evidence document for the synthetic fixture, with lifecycle-state authority confined to frontmatter and recorded events. Historical revision records remain revision-bound and cannot satisfy later revision bindings.

When the recorded lifecycle fields show the required seal and passed Content and Consumer reviews for an exact Research revision, a fixture-only Scope may turn the findings into observable documentation obligations rather than Ambitions product behavior. That Scope should require:

- exact binding to the eligible Research revision, contract hash, and commit recorded at consumption time;
...
- Treating any prior revision's seal, contract hash, freshness paths, or review as current authority for a later revision.
- Recording or inventing a Content or Consumer review verdict during producer authoring.
- Claiming CLI validation, Codex review, consumer readiness, implementation approval, merge approval, or release approval.
- Authoring Scope or Design unless the authoritative lifecycle fields and events show the required upstream eligibility.
- Encoding the current revision's lifecycle status, seal values, review state, or next transition in narrative body text.

## Remaining unknowns

1. Whether a compatible lifecycle CLI run will report structural and repository validation success for the committed revision being checked.
2. The contract hash and freshness set associated with any future seal event not yet recorded.
3. The findings, improvements, or required revisions associated with any future review event not yet recorded.
4. Whether the authoritative lifecycle fields and events will permit Scope at the time a Scope producer evaluates this Research.
5. The exact future Scope and Design revisions, contract hashes, input commits, and review records.
6. Which repository paths, if any, will drift after an eligible Research revision is sealed and how each relevant path will be assessed.
7. Whether later active package or template changes will require reconciliation before consumption.

Revision 1 and revision 2 contract hashes and freshness paths are known historical records. They are not reusable authority for another revision.
...
| RISK-002 | A downstream document invents missing review or authority state. | Medium | High | Bind only actual committed passed upstream metadata from authoritative lifecycle fields and events; leave unknowns explicit. |
| RISK-003 | Relevant owner paths are omitted, causing meaningful drift to appear unrelated. | Medium | High | Carry complete source, test, dependency, input, evidence, package, and template paths into the authoritative freshness union when recorded. |
...
| RISK-007 | Documentation quality is conflated with CLI, review, merge, or release approval. | Medium | High | Make no such claim; recognize each transition or review only when its explicit lifecycle record exists. |
| RISK-008 | A prior revision's seal or review is reused as authority for another revision. | Medium | High | Require exact revision, contract-hash, review, and commit equality from authoritative lifecycle records. |
| RISK-009 | Narrative body text becomes stale when lifecycle state changes. | Medium | High | Keep current lifecycle state solely in frontmatter and recorded events; phrase body conditions generically. |
...
| SRC-007 | `.agents/skills/ambitions-product-development-lifecycle/scripts/product_docs/transitions.py` | Ambitions repository |  | 2026-08-03 | High | Recheck when creation, sealing, review, reopen, or input-binding transitions change | FIND-004, FIND-008, FIND-009 | Defines canonical identity, baseline capture, package/template binding, seal and review bindings, adjacent-phase inputs, and reopening as a new revision with preserved history. |
...
| SRC-015 | `.agents/skills/ambitions-product-development-lifecycle/references/lifecycle-contract.md` | Ambitions repository |  | 2026-08-03 | High | Recheck when lifecycle states, revision rules, or authority boundaries change | FIND-003, FIND-007, FIND-008, FIND-009 | Defines immutable revisioned shapes, draft-only authority edits, revision-bound seals and reviews, reopening as a new revision, and the workflow's non-authorization boundary. |

## Handoff to Scope

This Research does not itself authorize or create Scope.

A Scope producer may consume this Research only when the authoritative lifecycle fields and recorded events show the required seal and passed Content and Consumer reviews for the exact revision being consumed. At that time, the producer may consume:
...
- Revision: `2`
- Correction boundary: narrative lifecycle-state claims were revised while frontmatter and recorded lifecycle events remained authoritative.
- Historical boundary: the revision 1 seal and `NEEDS REVISION` review remained preserved records and did not bind revision 2.
- Provenance boundary: the no-originating-chat, no-previous-lifecycle-conversation, and no-local-checkout statements were author declarations, not repository evidence.
### Seal event
...
- Reopened at: `2026-08-03T15:28:24Z`
- Revision: `3`
- Repository baseline: `fc8eca8d7b913a3d6cef82e33bfd3c7f419c0021`
- Corrective work: Bindings preserved.

### Revision 3 correction note
...
- docs/canon/generated/CODEX_START_HERE.md
- docs/product-development/lifecycle-fixture/evidence/comparison.md
### Review event: REV-CONTENT-RESEARCH-003

- Review lane: `CONTENT`
- Verdict: `PASS`
- Reviewer surface: `chatgpt; no earlier producer conversation used`
- Reviewed at: `2026-08-03T18:41:00Z`
- Reviewed revision: `3`
...
- None
### Review event: REV-CONSUMER-RESEARCH-003-REPLAY

- Review lane: `CONSUMER`
- Verdict: `PASS`
- Reviewer surface: `codex`
- Reviewed at: `2026-08-03T18:55:02Z`
- Reviewed revision: `3`
- Reviewed contract hash: `sha256:850dea049c234d05bc54493ee5c6adde18c15f438a27d1c1cfba7bbc953042c1`
