...
Planned Next:
- Phase 11.5 — UI Reality & Design System Enforcement
...
### UI Reality Direction (Locked)
**Slightly richer minimalism**
- Still minimal and restrained
- More visual clarity than the current rendered state
- Stronger interaction affordances
- Better chip/selector/button states
- Better grouping and hierarchy
- Still calm, premium, and Apple-adjacent rather than loud or decorative
...
### Phase 10 — Accounts + Sync Foundation (Locked Planning Rules)

#### Account Positioning
Ambitions accounts should feel like simple utility accounts first, with a more premium identity layer later.

Initial account purpose:
- cross-device continuity
- backup
- sync
- future-safe identity for personalization

Accounts should not initially feel:
- social
- membership-like
- over-personalized
- brand-heavy
...
2. other fallback options later if needed

#### Existing Local User Migration
**Attach local data after confirmation**

When an existing local-only user creates an account, the app should confirm attaching the current device’s Ambitions data to that account.

Rules:
- no silent overwrite
- no duplication
- no manual migration complexity
...
#### Sync Philosophy
**Restrained hybrid sync**

- local-first experience remains
- cloud is treated as a real system (not a mirror)
- offline reliability must remain strong
- sync must not slow down the app

#### Conflict Handling
**Conservative merge with fallback review**

- auto-merge when safe
- preserve both versions when ambiguous
...
- after onboarding
- when user wants sync
- when backup becomes relevant

#### Migration Tone
**Quiet hybrid**

Example direction:
- “Keep your Ambitions data across devices”
- “Access your plans anywhere”

Avoid:
- aggressive upsell tone
- social framing
...
### Phase 11 — Richer Goal Management + Plan Review (Locked Planning Rules)

#### Goal Edit Behavior
When a user edits a goal after downstream milestones/tasks already exist:
- do not silently regenerate
- do not force manual cleanup
- ask the user whether to:
  - keep downstream work
  - regenerate affected parts
  - regenerate all downstream work when appropriate

#### Regeneration Scope
Offer both:
- targeted regeneration of affected milestones/tasks
...
#### Protection Rules
When targeted regeneration is used, Ambitions should protect:
- user-edited downstream items
- completed items
- in-progress items

Whenever possible, preserve both trust and continuity.

#### Rollback
When regeneration is executed, preserve a lightweight temporary rollback snapshot:
- enough to safely undo a bad regeneration
- enough to preserve user trust
- not a full version-history system in this phase

#### Recommended Plan Framing
Generated plans should be presented as:
- recommended
- intentional
- user-adjustable
- not weakly framed as a draft

#### Review Editing Power
Before accepting a recommended plan, the user should have:
- light editing power
- not just accept/regenerate
- not full manual control

Examples of allowed light editing:
- reorder tasks
- remove a few tasks
- lightly adjust durations or timing
- make small shaping changes without turning the product into manual project management software

#### Plan Rationale
Plan review should include:
- light rationale
- short trust-building explanation where helpful
- enough transparency to explain why items are included
- not deep engine-style technical breakdowns

#### Review Structure
Use a hybrid review model:
- light review inline from Today when appropriate
- deeper review in a dedicated Plan Review surface when needed

#### Goal Edit Impact Preview
Before confirming a goal edit, Ambitions should show a light impact preview:
- short summary of what may change
- affected milestones/tasks at a high level
- whether regeneration is recommended

#### Pause / Archive Behavior
When a user pauses or archives a goal, present calm downstream options such as:
- remove from active plans
- keep already scheduled items
- archive or defer downstream work

Do not silently wipe active work.

### Phase 11.5 — UI Reality & Design System Enforcement
Goal:
Bring the actual rendered Ambitions UI up to the intended product standard by enforcing the design system, fixing interaction affordances, and unifying visual hierarchy across the live iPhone build.

Rules:
- no architecture churn
- no new product feature expansion
- no fake redesign language
- use the existing design system properly instead of defining more unused primitives
- keep the app calm and premium, but with more visual clarity and better affordances than the current rendered state

### Phase 11.5 — UI Reality & Design System Enforcement (Locked Planning Rules)

#### Visual Direction
**Slightly richer minimalism**
- still restrained
- still premium
- more visual clarity and statefulness than the current build
- stronger affordances for selections, actions, cards, and grouped content

#### Design System Enforcement
Interactive UI must use real interactive components:
- OptionChip for selectable chips/options
- Button for actions
- Pill for metadata/state markers where appropriate
- Surface for grouped content
- TextField for inputs

No raw plain-text option lists should remain in core product flows.

#### Selection UI
The following must render as real selectors, not plain stacked text:
- focus domains
- task size preferences
- day intensity
- other similar settings/options
...
- spacing
- tap affordance
- calm but visible contrast

#### Hierarchy Enforcement
Enforce clear hierarchy for:
- screen headers
- section headers
...
The rendered app should not feel like a text dump or internal tool.

#### Surface Quality
Grouped content should feel intentionally grouped.
Fix:
- weak flat panels
- inconsistent padding
- raw bordered boxes that feel like browser forms
- lack of separation between sections

#### Today Screen Priority
Today must feel like:
- a structured execution surface
- not a debug readout
- not a wall of stacked text

It should preserve density where needed, but with clearer grouping, spacing, and action affordances.

#### Debug-Feeling Artifact Removal
Raw metadata and internal-feeling labels should be restyled or reduced.
Examples of problematic current patterns:
- LOCAL ONLY
- raw block/task counters
- raw status text lines
- debug-feeling fallback labels

#### Interaction Clarity
Buttons must look like buttons.
Tasks must look actionable.
Selection controls must look interactive.
Metadata must not compete visually with primary actions.

#### Product Constraint
Do not redesign Ambitions into a different product.
This is a design-system enforcement and UI-quality pass, not a new product direction.
...
- exact post-Phase-11.5 expansion sequencing after UI correction
