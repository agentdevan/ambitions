# Cross Surface Continuity System
...
PXOS connects surfaces clearly. Capture can become a step, plan item, goal seed, note-to-review, or discarded item. Goals generate meaningful recommended steps for Today. Today execution creates receipts/proof for Goals. Plan provides capacity/time context for Today. You controls preferences, trust, setup, and personalization.

The system explains cross-surface changes when they matter and lets the user trace why something appeared.

Examples: captured idea -> Ready to Place -> Goal seed -> Goal Detail -> Today recommended step; goal milestone -> Plan capacity check -> Today Step -> Step Session -> completion receipt -> Goal proof rail; missed step -> soft closure prompt -> Moved or Still Counts -> Plan adjustment -> receipt; schedule setup -> better Plan capacity -> better Today recommendation; vacation/away override -> Plan changes availability -> Today avoids treating away time as free time.
