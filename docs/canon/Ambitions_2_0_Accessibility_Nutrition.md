...
This document is not a public claim. It is the internal source of truth for what must be checked, how claim status is recorded, and what Batch 85 must prove before `You -> Accessibility` can publish a summary.
...
- Release notes, App Store copy, website copy, onboarding, Profile/You copy, and support docs must not imply verified accessibility support before Batch 85 signoff.
...
The user-facing summary may appear only after Batch 85 verification.
...
## Batch 85 Verification Handoff Criteria

Batch 85 may build and publish the user-facing `You -> Accessibility` summary only when:
...
If any evidence is incomplete, Batch 85 must keep the user-facing summary conservative or defer publication.
