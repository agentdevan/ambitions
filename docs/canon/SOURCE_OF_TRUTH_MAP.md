...
Purpose: Make Ambitions documentation easier to read, safer for Codex, and harder to drift. This document does not replace the existing canon. It maps which existing document owns each kind of truth and identifies consolidation gaps that need dedicated specs.
...
8. `docs/canon/Ambitions_2_0_Roadmap.md`, `docs/canon/Ambitions_2_0_Batch_Plan.md`, and `docs/codex/batches/*` for execution sequencing.
9. QA, review, and release docs for acceptance evidence.
10. Archived docs only when explicitly marked as historical context and only where they do not conflict with active canon.
...
| Goal/Plan/Task lifecycle | `GOAL_PLAN_TASK_LIFECYCLE.md` | Product Architecture, Systems Architecture, Visual System | New consolidated state-machine doc. |
...
| Visual direction | `Ambitions_2_0_Visual_System.md` | `component-contract-matrix.md`, `panel-density-size-spec.md`, `DESIGN_TOKENS.md` | Design tokens are now extracted for implementation naming. |
...
| Completion and QA gates | `IMPLEMENTATION_ACCEPTANCE_GATES.md` | Visual Review Checklist, RC Maturity Plan | New consolidated gate doc. |
...
2. **Object model spread.** Life Area, North Star, Goal, Plan, Path, Milestone, Step, Task, Proof, Receipt, Review, Decision, and Archive are defined across multiple docs. They now need one domain reference.
3. **Lifecycle implied, not fully executable.** Goal Weather, Goal Lifecycle Rail, Proof Spine, Decision Trail, Plan Treaty, and Completion Archive are defined, but a compact state-machine doc was missing.
...
5. **Design tokens not literal enough.** Visual direction is strong, but token names and implementation-level mapping need a dedicated token contract.
6. **Onboarding is under-specified.** The constitution defines onboarding principles, but not a full screen-by-screen flow.
7. **Trust/memory is strategically important enough to stand alone.** It existed, but was embedded inside broader docs.
8. **Empty/error/recovery states need a screen-by-screen acceptance matrix.** UX writing rules existed, but they were not exhaustive by surface.
9. **Acceptance gates need one explicit implementation standard.** QA material existed, but future Codex prompts need a single definition of done.
...
The following new documents complete the immediate consolidation set:
