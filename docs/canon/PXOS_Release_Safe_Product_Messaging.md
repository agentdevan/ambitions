...
PXOS messaging must pass REC and release-claim boundaries. It may describe future canon, future train readiness, and product direction. It must not claim App Store readiness, TestFlight readiness, production readiness, physical-device proof, calendar/reminder integration, widgets implemented, App Intents implemented, AI implemented, backend/sync/cloud implemented, PXOS shipped, AmbitionsOS implemented, release-ready product experience, production personalization, production model behavior, or verified platform integration.

Any future product messaging must state evidence source, proof gaps, what is current, what is future, and what remains human/operator verified.
