...
Canonical semantics belong to `Core/Domain/`; scheduling, OS reconciliation, mutation, and inspection belong to `Core/LocalRuntimeOS/Scheduling/`, `ExternalWrites/`, `Commands/`, and `Inspection/`; You presents controls and `Quality/` proves permission denial, quiet hours, privacy payload, delivery failure/retry, noncompletion, recurrence linkage, Trash/restore, replay, offline, and accessibility. Tests bind requests to rule/subject identifiers;
