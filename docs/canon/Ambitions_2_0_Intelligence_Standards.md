...
- Batch 73: Today 2.0 / Daily Operating Contract must expose the current lens, one recommended step, urgent outside-lens items, one protected must-do, one intentionally-not-today item, one recovery fallback, Save the Day, Action Closure entry, and why-this-now without becoming a dashboard.
...
- Batches 83-84: Goals and Goal Detail 2.0 must support living goal containers, deliverables, scope changes, visible roadmaps, completed work, remaining work, Previous/Active/Future/Waiting/Blocked/Parked/Protected/Completed/Cancelled / Dropped states, Goal Lifecycle Rail, Goal Atlas preview/connected goals, Next Visible Step, Goal Weather, Proof Spine as Proof Rail expression, Decision Trail, Timeline View, Milestone Cards, Kanban-lite Task Lane, Completion Archive summaries, proof-weighted progress, assumptions, Mission Control lanes, and Life Graph Breadcrumb.
- Batches 85-86: Plan 2.0 / Reality Reflow must show context blocks, calendar-aware fit, priority conflicts, displaced items, protected high-priority work, calmly deferred lower-priority work, Weekly Plan Strip, Next Visible Step selection, Goal Weather/proof-density believability inputs, Plan Treaty, Personal Capacity Envelope, Constraint Gravity, Decision Debt, Opportunity Windows, Decision Trail notes for major scope changes, and no silent rescheduling.
...
- Batches 90-94: Sync/export/import, App Intents, widgets, and Live Activities must use External Continuity Contract, Action Closure receipts, stale-state truth, no duplicate command logic, and calm ambient Goal/Plan snapshots such as Next Visible Step, Active Goal Timeline, Goal Portfolio, Next Milestone, Protected Goal, Weekly Plan Strip, and next proof action.
- Batches 95-97: Path Intelligence and Path Builder must connect long-range paths, path forks, Domain Path Packs, proof requirements, prerequisites, fallback paths, future-self simulations, limited-time/pause scenarios, Goal Atlas expansion, milestone roadmap nodes, lifecycle timelines, Decision Trail, and Weekly Plan Strip rollups to living goal containers and daily actions.
