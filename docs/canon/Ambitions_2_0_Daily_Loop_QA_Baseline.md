...
| Public accessibility claims remain locked | R04 | Blocking | R01 added a claims lock over internal evidence and R03 did not add manual accessibility proof; manual VoiceOver, Dynamic Type, Reduce Motion, contrast, motor, and external-surface accessibility review are still needed before R04 public copy can promote any claim. |
| Device QA, App Store materials, and RC lock need human/device gates | R04-R05 / human-device gate | Blocking | R03 records simulator/source device-readiness evidence but does not prove physical-device QA or TestFlight upload readiness; privacy/demo materials and explicit human approval gate remain. |
