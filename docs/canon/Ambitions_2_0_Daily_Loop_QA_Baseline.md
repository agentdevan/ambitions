...
| Path editing and confirmed roadmap mutation remain maturity work | M08-M12 | Medium | M07 added Goal Detail Path Builder UI with phases, dependencies, fork prompts, proof checks, Today/Plan handoff copy, accessible list fallback, and a compact performance budget, but it does not add broad path editing, automatic roadmap changes, persistence/export/sync changes, or real-device/manual UI proof. |
