...
Batch 61 verifies repo truth after Batch 60 and records the current state without implementing product changes.
...
- Batch 60 is recorded as complete for planning and Batch 61 is active in `docs/codex/BATCH_REGISTRY.md`.
...
- Rich panel/design-system readiness exists through shared primitives and widget UI foundations, but Batch 63 still needs to align it to the new 2.0 visual direction.
- Accessibility support is present across primitives and screens, but user-facing Accessibility Nutrition Facts are not implemented or manually verified.

### What appears missing

- The locked Ambitions 2.0 top-level shell does not exist yet. Current top-level tabs are Today, Goals, Plan, Insights, and Profile.
- Apple-first sync is not implemented; the repo currently exposes a local-only sync capability.
- Full long-range Path Builder UI, Plan-owned calendar permission UX, and a unified Canonical Now State/Event Ledger are not complete.
...
- Do not redesign panels before Batch 63.
...
| Batch registry and post-Batch-60 status | Verified complete | Registry/docs: `docs/codex/BATCH_REGISTRY.md` records Batch 60 complete for planning, Batch 61 active, Batch 62 queued. | Batch 61 can proceed as repo-truth audit; Batch 62 remains queued. | If wrong, work could skip a required hardening or canon step. | 61 | Leave registry history intact; only update completion status after explicit closeout decision. |
| Current tab shell | Stale / legacy | Code: `Native/Ambitions/App/AppTab.swift` `AppTab.allCases` returns Today, Goals, Plan, Insights, Profile; `AmbitionsRootView` builds those tabs. Docs: 2.0 canon locks Today, Goals, Capture, Plan, You. | Current IA conflicts with locked Ambitions 2.0 shell. | If treated as current truth, Codex may preserve wrong top-level tabs. | 62 | Batch 62 should replace shell IA without migrating feature internals prematurely. |
...
| Capture surface | Partially implemented | Code/tests: `CapturesScreen`, `CapturesViewModel`, `DefaultCaptureService`, `CaptureServiceTests`; shell evidence: `AppTab.captures` maps to Plan and is not top-level. | Capture functionality exists, but locked singular top-level Capture tab is missing. | If wrong, Batch 62/69 could either duplicate Capture or leave it buried. | 62, 69 | Batch 62 should create IA ownership; Batch 69 should implement Capture 2.0 behavior. |
...
| You/Profile surface | Stale / legacy | Code: `ProfileScreen`, `ProfileFeatureService`, `AppTab.profile`; docs: 2.0 locks top-level tab label to You. | Current Profile tab must become You later and absorb reviews/accessibility/trust surfaces. | If treated as final, Insights demotion and Accessibility Nutrition placement drift. | 62, 76 | Batch 62 should handle top-level label/route; Batch 76 should build You content. |
| Insights surface | Stale / legacy | Code/tests: `InsightsScreen`, `InsightsMonthlyReviewScreen`, `InsightsHistoryScreen`, `InsightsFeatureServiceTests`; shell: top-level `AppTab.insights`. Docs: Insights must be demoted. | Existing Insights is real but conflicts with locked 2.0 navigation. | If left top-level, the app violates the future canon shell. | 62, 77 | Demote IA in Batch 62; redistribute insights in Batch 77. |
...
| Navigation / routing | Partially implemented | Code/tests: `AppNavigation`, `PlanRouteTarget`, `InsightsRouteTarget`, `AppExternalRouting`, `ShellCommandRouter`, `AppShellNavigationTests`, `ExternalRoutingTests`. | Routing supports current shell and external actions but not final 2.0 IA. | If overclaimed, Batch 62 may miss stale route targets. | 62, 68, 79 | Batch 62 should update shell routes; Batch 68/79 should harden command/external routing. |
...
| Design system / theme system | Partially implemented | Code/tests: `Sources/Theme/AmbitionTheme.swift`, `InformationPrimitives.swift`, `SurfacePrimitives.swift`, `ControlPrimitives.swift`, `AppearancePreferenceTests`. | Shared theme and primitives exist, but they are not yet fully aligned to the 2.0 visual system. | If overclaimed, rich-panel work may become inconsistent. | 63 | Batch 63 should align tokens, panel hierarchy, and light/dark warmth. |
| Rich panel readiness | Partially implemented | Code: `AppUI/Sources/WidgetFoundation.swift`, `WidgetFamilies.swift`, `WidgetSurface`, `WidgetCard`, `AsyncStateCard`; docs: 2.0 visual system requires widget-like panels. | Foundation exists for rich panels across app and widgets. | If wrong, Batch 63 may overuse unavailable primitives. | 63 | Convert app surfaces to canon panels only in scheduled UI batches. |
...
| Accessibility support | Partially implemented | Code: `.accessibilityIdentifier` usage across feature screens; `ambitionPanelAccessibility` in `SurfacePrimitives`; tests include shell/feature accessibility identifiers. | Accessibility hooks exist, but full verified accessibility nutrition is not complete. | If overclaimed, public accessibility claims may be inaccurate. | 64, 85 | Batch 64 should add internal nutrition layer; Batch 85 should verify public summary. |
| Accessibility Nutrition docs/readiness | Partially implemented | Docs: `docs/canon/Ambitions_2_0_Accessibility_Nutrition.md`, `docs/canon/Ambitions_Accessibility_Nutrition_Labels_Audit.md`; code lacks You-facing summary. | Canon and historical audit exist; user-facing facts are not shipped. | If wrong, Batch 85 could publish unverified claims. | 64, 85 | Keep as docs/readiness until implementation and manual verification. |
...
| Memory / Event Ledger | Partially implemented | Code/tests: `ProgressEvidence`, `GoalFeedbackEvent`, `GoalTeachingSignal`, repositories, `MemoryLensService`, `MemoryLensServiceTests`; no unified `EventLedger` type found. | Memory fragments exist, but a canonical ledger is not complete. | If wrong, explanation, learning, and sync may diverge. | 65 | Batch 65 should define and migrate to canonical ledger boundaries. |
...
| Privacy / memory controls | Partially implemented | Code/docs: `PrivacyInfo.xcprivacy`, Profile/Trust Center surfaces, local-first docs, memory lens services. | Privacy foundation exists, but Memory/Event Ledger and sync controls are not final. | If overclaimed, future sync/memory UX may violate trust posture. | 65, 76, 78, 85 | Tie memory controls to ledger, You, sync, and Accessibility Nutrition work. |
...
| --- | --- | --- |
| Locked 2.0 tabs are Today, Goals, Capture, Plan, You, but current shell is Today, Goals, Plan, Insights, Profile. | Docs: `Ambitions_2_0_Product_Architecture.md`; code: `AppTab.allCases`, `AmbitionsRootView`. | Resolve in Batch 62 only. Do not rename/migrate tabs in Batch 61. |
| Insights must be demoted, but it remains a top-level tab with monthly review/history routes. | Code: `InsightsScreen`, `InsightsMonthlyReviewScreen`, `InsightsHistoryScreen`, `AppTab.insights`. | Demote in Batch 62; redistribute review/explanation surfaces in Batches 76-77. |
