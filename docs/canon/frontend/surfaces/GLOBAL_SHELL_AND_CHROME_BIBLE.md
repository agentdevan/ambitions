...
This surface contains safe areas, destination title behavior, contextual headers, bottom destination dock, selected/unselected tab states, tab icon rules, Context Crown, back navigation, disclosure rows, chevrons, modals, sheets, overlays, drawers, receipts, confirmations, keyboard/composer behavior, trust indicators. The top-level visual hierarchy is `Global Shell` first, one primary next action second, source/trust/proof affordances third, and secondary metadata last.

## Primary Object

- Destination: Shell
- Primary object: Global Shell
- User job: Orientation
...
Destination roots; top-right utility controls; receipt surfaces; modal and sheet families; contextual source details.
