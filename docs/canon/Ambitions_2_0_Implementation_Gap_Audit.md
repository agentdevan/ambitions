...
| External surfaces | Widgets, Live Activity, notifications, and App Intents exist; D22 now adds a shared contract registry, privacy snapshot policy, safe stale/unavailable labels, command/receipt/confirmation requirements, and fallback-route rules. Platform-specific rendering and device verification are not yet complete. | D23-D25 platform-specific alignment and D26 validation. |
...
| D23 | Widgets Alignment | Next: align widgets after Now State stability with lightweight snapshots. |
