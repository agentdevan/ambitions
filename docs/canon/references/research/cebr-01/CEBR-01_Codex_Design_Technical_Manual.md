...
> **Research design reference.** This manual is non-normative. Before
> implementation, use the product-canon compiler to resolve the owning
> requirements, then inspect current source and tests. Ordinary work needs no
> task start, pack, approval, attestation, or finalization ceremony.
...
At repository revision `7dbf554876c686e62525a85d762fec04b67a4af4`, the Private Life Runtime required typed command validation, deterministic local simulation, atomic local commit, projection invalidation or materialization, truthful Receipts, replay, rollback, and external effects only after local commitment. Scheduling law required honest capacity, explicit infeasibility, inspectable reflow, protected boundaries, user control, and deterministic change sets. Current normative specifications take precedence over this snapshot.
...
All names in this section are provisional and non-normative. Codex must resolve final naming against current product canon and live source before editing.
...
No numeric target is valid until performance calibration defines device, OS, build, graph scale, horizon, recurrence scale, warm/cold state, percentile, energy, memory, and storage criteria.

## 21. Codex candidate file map

These are candidate placements only. Current product canon and live source may route differently.
...
These are research decompositions, not an active implementation train.
...
Before implementing one of these decompositions, resolve its exact owning
requirements, current source owners, focused tests, dependency impact, and
rollback boundary. Do not infer current implementation status from this
research document.
...
2. Query the exact owning requirements with
   `python3 scripts/ambitions-canon.py query`.
3. Inspect current source, project configuration, tests, and dependency impact.
4. Define deterministic input, accepted and rejected effects, persistence,
   replay, rollback, privacy, concurrency, accessibility, and degraded behavior
   for the slice.
5. Add focused tests before or with the implementation.
6. Preserve existing source owners; do not create parallel runtime authority.
7. Run the changed-scope build and behavioral checks, plus migration,
   data-integrity, privacy/security, concurrency, accessibility, and performance
   checks when the slice affects them.
8. Report only behavior demonstrated by the current build, tests, and rendered
   evidence.
...
- **C1.** `docs/canon/MANIFEST.toml` at `7dbf554876c686e62525a85d762fec04b67a4af4` — Historical canon-manifest and compiler snapshot.
