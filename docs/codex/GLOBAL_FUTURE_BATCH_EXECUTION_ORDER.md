...
- PXOS is future user-facing canon only. PX01-PX03 are complete as future canon; PX04 is next pending dry-run selection; PX05-PX20 are queued/blocked and not started.
...
run through REC closure evidence. PX01-PX03 are complete as future canon. PX04 is the next global batch only after the mandatory dry-run selection says `Execution allowed: YES`.
...
| 009 | PX04 | PXOS | Docs/canon | Capture expression depends on PX01. | PX01-PX03 Green, PXOS, Trust/Privacy, current global 4.0 preauthorization or `Start PXOS Future-Canon Train` | Parallel-safe after PX03 with PX05-PX08 only if approved | Dry-run pending | Adequate docs evidence | privacy/trust, product language | Continue only on Green |
...
| PX04-PX20 | Queued / Blocked | Current global preauthorization or `Start PXOS Future-Canon Train`, PXOS gates, source truth | Future canon only; not implemented | No PXOS implementation or release-readiness claim |
