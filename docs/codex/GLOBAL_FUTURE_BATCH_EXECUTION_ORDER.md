...
- PXOS is future user-facing canon only. PX01 is next pending dry-run selection; PX02-PX20 are queued/blocked and not started.
...
run through REC closure evidence. PX01 is the next global batch only after the
mandatory dry-run selection says `Execution allowed: YES`.
...
| 006 | PX01 | PXOS | Docs/canon | PXOS parent canon and surface hierarchy block all later PXOS work. | Current global 4.0 preauthorization or `Start PXOS Future-Canon Train`, Source Truth, Product Decision Lock | Serial-only | Dry-run pending | Adequate docs evidence | source-truth-reconciler, product decision lock, surface hierarchy | Continue only if train control and global dry-run permit |
