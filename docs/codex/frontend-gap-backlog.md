...
| 4 | `TIME-LIFESHAPE-PROOF-01` | Prove LifeShape Field over calendar/dashboard; resolve `.plan` visual-context debt where safe. | Optional; consider only for LifeShape canvas/accessibility gains. | Red if Time still reads as card dashboard or calendar clone. | Time focused tests, screenshot matrix, stale-language scan. | Restore Time-only changes/proof. |
...
| 10 | `FRONTEND-CONTROL-PLANE-HONESTY-01` | Repair generated dashboard language that says Green while proof is out of scope. | Not applicable. | Yellow until reports separate routing health from product proof. | frontend scripts, prompt audit. | Restore reports/control-plane files. |
