...
| SwiftPM `.iOS(.v26)` under tools 6.0 | Failed as expected | Temporary manifest with `// swift-tools-version: 6.0` failed with `error: 'v26' is unavailable` and note `introduced in PackageDescription 6.2` | This is the key boundary result for the target bump. |
