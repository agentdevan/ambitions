# AOS10 Commitment Time Kernel Prompt
...
LDI hook: Commitment Time should map capacity fit, eligibility/deadline constraints, source-check tasks, proof tasks, review tasks, recovery needs, and no fantasy schedules.
...
- `git rev-parse HEAD`
- `git log -1 --oneline`
- `rg -n "AOS10|Commitment Time Kernel|AmbitionsOS|release ready|App Store ready|TestFlight ready" docs .codex Native README.md AGENTS.md || true`
...
- Required skills: `aos-train-orchestrator`, `aos-invariant-enforcer`, `runtime-contract-reviewer`, `validation-evidence-auditor`, `release-claim-truth-enforcer`, plus `Commitment Time Kernel`-specific reviewers.
...
Stop on Red, missing predecessor gate, dirty target files owned by someone else, unclear owner, untrusted validation, missing rollback path, release-claim ambiguity, privacy ambiguity, or a request to broaden beyond AOS10.
...
`Run AOS10 Commitment Time Kernel`
