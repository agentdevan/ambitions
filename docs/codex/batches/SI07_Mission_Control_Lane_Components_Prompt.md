# SI07 Mission Control Lane Components Prompt
...
- Batch ID: SI07
- Title: Mission Control Lane Components
- Train: SI01-SI18 Signature Interface Implementation Train
- Global order after SI insertion: 054
...
Build reusable lanes for Goal Detail and Mission Control.
...
MissionControlLane, lane header, lane item, density controls, drill-down affordance.
...
Lane selection, focus movement, expansion/collapse, reduced-motion lane state.

Motion must orient, confirm, or reduce uncertainty. Haptics must be system-native and optional.

## Accessibility Requirements

VoiceOver lane grouping, labels/values, Dynamic Type rows, tap targets.

No icon-only meaning, no color-only meaning, no private content leak, and no Dynamic Type collapse.

## Visual Acceptance Criteria

Lanes are distinct, subordinate to goal path, and scan-friendly.
...
No equal-weight board or project-management columns.
...
Rollback by reverting the SI07 commit. Repair Red by narrowing scope, restoring boundaries, adding missing evidence, splitting oversized files, or stopping with a Red repair report. Do not weaken product canon, tests, accessibility, compatibility, release truth, or SI gates.

## Claims

This batch may claim only the SI07 scope completed after validation, evidence, commit, and push.
...
Run SI07 Mission Control Lane Components
