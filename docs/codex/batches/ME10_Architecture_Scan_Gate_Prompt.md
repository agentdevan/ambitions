...
- Batch ID: `ME10`
- Name: Architecture Scan Gate
- Mode: audit only
- Owner file or files: `architecture scan outputs and Lane 2 owner files`
- Extraction target: Classify architecture scan results and stop on unowned large-file growth.
...
- `git rev-parse HEAD`
- `git log -1 --oneline`
- `wc -l architecture scan outputs and Lane 2 owner files` when the owner path is concrete
- `rg -n "TODO|compat|route|accessibilityIdentifier|Start here|What Ambitions Knows|failed|Profile|You" architecture scan outputs and Lane 2 owner files || true` when the owner path is concrete
...
`Run ME10 Architecture Scan Gate`
