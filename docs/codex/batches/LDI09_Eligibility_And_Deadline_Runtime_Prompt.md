# LDI09 Eligibility And Deadline Runtime Prompt
...
- Batch ID: LDI09
- Title: Eligibility And Deadline Runtime
...
Implements eligibility/deadline logic with source-state boundaries.
...
Boundary: age/date/window/deadline/minimum lead time logic.
...
Implement only the smallest usable slice that advances Eligibility And Deadline Runtime. Any runtime behavior must be deterministic, local-first, evidence-producing, and compatible with Ambitions' five destination IA.
...
`Run LDI09 Eligibility And Deadline Runtime`

## Next Safe Prompt / Next Gate

After Green or accepted Yellow, ask global train for the next eligible batch. Expected next gate: LDI10.
