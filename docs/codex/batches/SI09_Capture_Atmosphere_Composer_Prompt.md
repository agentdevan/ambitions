# SI09 Capture Atmosphere Composer Prompt
...
- Batch ID: SI09
- Title: Capture Atmosphere Composer
- Train: SI01-SI18 Signature Interface Implementation Train
- Global order after SI insertion: 056
...
Build Capture signature atmosphere/composer primitive.
...
CaptureAtmosphereComposer, input field, route reveal strip, privacy posture label.

Likely owner families: Native/Ambitions/Features/Captures/**; Sources/Components/**; Sources/Previews/**; Native/AmbitionsTests/**; docs/audits/**. Any actual owner change must be named in the batch report before edits.

## Interaction Requirements

Input focus, route reveal after text, mic/add feedback, reduced-motion alternative.

Motion must orient, confirm, or reduce uncertainty. Haptics must be system-native and optional.

## Accessibility Requirements

Text field labels, mic/add labels, privacy state, Dynamic Type composer.

No icon-only meaning, no color-only meaning, no private content leak, and no Dynamic Type collapse.

## Visual Acceptance Criteria

Atmosphere is restrained and composer is the primary object.
...
No chatbot wrapper, notes inbox, or neon decoration.
...
rg -n "SI09|Capture Atmosphere Composer|Signature Interface|release ready|App Store ready|TestFlight ready" README.md docs .codex Native Sources AppUI || true
~~~

Strong Capture tests/build, privacy scan, SI visual/accessibility proof.

## Preview And Visual Evidence

Preview empty, typing, ready to place, needs decision, grow into goal, private.

Do not claim human visual approval unless a human actually approves. Do not claim physical-device proof unless performed.

## Green Criteria

- Scope completed exactly for SI09.
...
Rollback by reverting the SI09 commit. Repair Red by narrowing scope, restoring boundaries, adding missing evidence, splitting oversized files, or stopping with a Red repair report. Do not weaken product canon, tests, accessibility, compatibility, release truth, or SI gates.

## Claims

This batch may claim only the SI09 scope completed after validation, evidence, commit, and push.
...
Run SI09 Capture Atmosphere Composer
