# LDI22 Governance And Maintenance Console Prompt
...
Defines governance metrics without personal data.
...
Boundary: pack/source/safety/sync/governance metrics, no user personal data.
...
Implement only the smallest usable slice that advances Governance And Maintenance Console. Any runtime behavior must be deterministic, local-first, evidence-producing, and compatible with Ambitions' five destination IA.
...
`Run LDI22 Governance And Maintenance Console`

## Next Safe Prompt / Next Gate

After Green or accepted Yellow, ask global train for the next eligible batch. Expected next gate: LDI closeout / next explicit user decision.
