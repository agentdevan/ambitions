...
LDI hook: Governance registry should map pack/source/safety/sync governance metrics without user personal data.
...
- `git rev-parse HEAD`
- `git log -1 --oneline`
- `rg -n "AOS23|Governance Kernel|AmbitionsOS|release ready|App Store ready|TestFlight ready" docs .codex Native README.md AGENTS.md || true`
...
Stop on Red, missing predecessor gate, dirty target files owned by someone else, unclear owner, untrusted validation, missing rollback path, release-claim ambiguity, privacy ambiguity, or a request to broaden beyond AOS23.
...
`Run AOS23 Governance Kernel Registry`
