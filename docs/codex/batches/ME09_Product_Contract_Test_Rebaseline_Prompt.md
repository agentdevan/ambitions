...
- Batch ID: `ME09`
- Name: Product Contract Test Rebaseline
- Mode: test rebaseline only
- Owner file or files: `tests covering ME01-ME08 owner files`
- Extraction target: Rebaseline behavior-preservation tests only; do not weaken expectations.
...
- `git rev-parse HEAD`
- `git log -1 --oneline`
- `wc -l tests covering ME01-ME08 owner files` when the owner path is concrete
- `rg -n "TODO|compat|route|accessibilityIdentifier|Start here|What Ambitions Knows|failed|Profile|You" tests covering ME01-ME08 owner files || true` when the owner path is concrete
...
`Run ME09 Product Contract Test Rebaseline`
