# AOS14 Recommendation Start Here Kernel Prompt
...
- Batch ID: `AOS14`
- Name: Recommendation Start Here Kernel
- Owning kernel: Recommendation Kernel
- Affected 3.0 primitive: Today / Goals / Capture / Time / You only where the train gate names a concrete surface
- Affected surface: Today, Goal Detail
- Dependency gate: depends on AOS04, AOS12, and AOS13
- Implementation boundary: Start Here recommendation contract only; no confidence scoring
...
LDI hook: Recommendation Start Here should map Today Bridge and Action Closure steps: source-check, proof, clarification, review, recovery, and Still Counts without guarantee language.
...
- `git rev-parse HEAD`
- `git log -1 --oneline`
- `rg -n "AOS14|Recommendation Kernel|AmbitionsOS|release ready|App Store ready|TestFlight ready" docs .codex Native README.md AGENTS.md || true`
...
Primary owner: Recommendation Kernel. Before edits, produce a decision record listing the exact files this batch will touch, why the owner owns them, and which large-file, compatibility, privacy, performance, and release gates apply. If the owner file is not obvious, stop after discovery and write the decision record instead of guessing.

## Required Implementation Boundary

This batch may read typed local state relevant to Recommendation Kernel; it may produce typed contracts, tests, fixtures, projections, reports, or user-reviewable deltas only. Model output must not mutate the Life Graph. Source-sensitive facts must remain unverified until source evidence is attached. User-facing behavior is allowed only when the batch explicitly owns it and focused tests cover it.
...
- Required skills: `aos-train-orchestrator`, `aos-invariant-enforcer`, `runtime-contract-reviewer`, `validation-evidence-auditor`, `release-claim-truth-enforcer`, plus `Recommendation Kernel`-specific reviewers.
...
Stop on Red, missing predecessor gate, dirty target files owned by someone else, unclear owner, untrusted validation, missing rollback path, release-claim ambiguity, privacy ambiguity, or a request to broaden beyond AOS14.
...
`Run AOS14 Recommendation Start Here Kernel`
