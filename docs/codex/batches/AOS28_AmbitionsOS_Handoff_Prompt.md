# AOS28 AmbitionsOS Handoff Prompt
...
LDI hook: Handoff should identify whether LDI can start after AOS30 or whether explicit user decision is needed for earlier gates.
...
- `git rev-parse HEAD`
- `git log -1 --oneline`
- `rg -n "AOS28|Governance Kernel|AmbitionsOS|release ready|App Store ready|TestFlight ready" docs .codex Native README.md AGENTS.md || true`
...
Stop on Red, missing predecessor gate, dirty target files owned by someone else, unclear owner, untrusted validation, missing rollback path, release-claim ambiguity, privacy ambiguity, or a request to broaden beyond AOS28.
