# LDI12 Capacity And Commitment-Time Bridge Prompt
...
- Batch ID: LDI12
- Title: Capacity And Commitment-Time Bridge
...
Fits plans into real life and blocks fantasy schedules.
...
Boundary: capacity fit, no fantasy schedules, no silent rescheduling.
...
Implement only the smallest usable slice that advances Capacity And Commitment-Time Bridge. Any runtime behavior must be deterministic, local-first, evidence-producing, and compatible with Ambitions' five destination IA.
...
`Run LDI12 Capacity And Commitment-Time Bridge`

## Next Safe Prompt / Next Gate

After Green or accepted Yellow, ask global train for the next eligible batch. Expected next gate: LDI13.
