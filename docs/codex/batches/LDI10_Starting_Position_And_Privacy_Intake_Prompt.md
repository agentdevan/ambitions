# LDI10 Starting Position And Privacy Intake Prompt
...
- Batch ID: LDI10
- Title: Starting Position And Privacy Intake
...
Defines minimal intake and sensitivity controls.
...
Boundary: ask-only-needed intake, local storage boundary, sensitive controls.
...
Implement only the smallest usable slice that advances Starting Position And Privacy Intake. Any runtime behavior must be deterministic, local-first, evidence-producing, and compatible with Ambitions' five destination IA.
...
`Run LDI10 Starting Position And Privacy Intake`

## Next Safe Prompt / Next Gate

After Green or accepted Yellow, ask global train for the next eligible batch. Expected next gate: LDI11.
