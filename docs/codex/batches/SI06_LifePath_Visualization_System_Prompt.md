# SI06 LifePath Visualization System Prompt
...
- Batch ID: SI06
- Title: LifePath Visualization System
- Train: SI01-SI18 Signature Interface Implementation Train
- Global order after SI insertion: 053
...
Invent Ambitions goal path visual grammar.
...
Goals show a life path object, not a progress bar or project chart.
...
- Kanban/project-board routing
- persistence schema
- unrelated Goal behavior
- .github/workflows/**
...
LifePathView, path node, proof marker, risk marker, alternate route indicator.

Likely owner families: Native/Ambitions/Features/Goals/**; Sources/Components/**; Sources/Previews/**; Native/AmbitionsTests/**; docs/audits/**. Any actual owner change must be named in the batch report before edits.

## Interaction Requirements

Node focus, path emphasis, route branch reveal, reduced-motion non-animated states.

Motion must orient, confirm, or reduce uncertainty. Haptics must be system-native and optional.

## Accessibility Requirements

Non-visual summary, VoiceOver node order, non-color proof/risk meaning.

No icon-only meaning, no color-only meaning, no private content leak, and no Dynamic Type collapse.

## Visual Acceptance Criteria

Novel Ambitions path grammar.
...
No roadmap, timeline, kanban, OKR, or project-management visual language.
...
Strong Goals tests/build, SI scans, preview proof.

## Preview And Visual Evidence

Preview early path, active path, proof-rich, risk, alternate route, private.

Do not claim human visual approval unless a human actually approves. Do not claim physical-device proof unless performed.

## Green Criteria

- Scope completed exactly for SI06.
...
Rollback by reverting the SI06 commit. Repair Red by narrowing scope, restoring boundaries, adding missing evidence, splitting oversized files, or stopping with a Red repair report. Do not weaken product canon, tests, accessibility, compatibility, release truth, or SI gates.

## Claims

This batch may claim only the SI06 scope completed after validation, evidence, commit, and push.
...
Run SI06 LifePath Visualization System
