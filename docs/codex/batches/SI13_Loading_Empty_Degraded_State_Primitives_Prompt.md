# SI13 Loading Empty Degraded State Primitives Prompt
...
- Batch ID: SI13
- Title: Loading Empty Degraded State Primitives
- Train: SI01-SI18 Signature Interface Implementation Train
- Global order after SI insertion: 060
...
Build reusable loading, empty, and degraded primitives.
...
AmbitionsLoadingState, empty state, degraded state, stale source label, recovery prompt module.

Likely owner families: Sources/Components/**; Native/Ambitions/Features/**; Sources/Previews/**; Native/AmbitionsTests/**; docs/audits/**. Any actual owner change must be named in the batch report before edits.

## Interaction Requirements

Loading-to-ready, retry/review entry, disabled action pending validation, reduced-motion skeleton.

Motion must orient, confirm, or reduce uncertainty. Haptics must be system-native and optional.

## Accessibility Requirements

VoiceOver state announcement, non-color status, Dynamic Type layout, no shame copy.

No icon-only meaning, no color-only meaning, no private content leak, and no Dynamic Type collapse.

## Visual Acceptance Criteria

States are calm, honest, and action-oriented.
...
No spinner-only default where context is required; no fake intelligence.
...
rg -n "SI13|Loading Empty Degraded State Primitives|Signature Interface|release ready|App Store ready|TestFlight ready" README.md docs .codex Native Sources AppUI || true
~~~

Strong build/tests, copy scan, loading/degraded review.

## Preview And Visual Evidence

Preview each state family and Dynamic Type.

Do not claim human visual approval unless a human actually approves. Do not claim physical-device proof unless performed.

## Green Criteria

- Scope completed exactly for SI13.
...
Rollback by reverting the SI13 commit. Repair Red by narrowing scope, restoring boundaries, adding missing evidence, splitting oversized files, or stopping with a Red repair report. Do not weaken product canon, tests, accessibility, compatibility, release truth, or SI gates.

## Claims

This batch may claim only the SI13 scope completed after validation, evidence, commit, and push.
...
Run SI13 Loading Empty Degraded State Primitives
