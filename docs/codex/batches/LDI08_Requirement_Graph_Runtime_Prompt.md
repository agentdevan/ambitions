# LDI08 Requirement Graph Runtime Prompt
...
- Batch ID: LDI08
- Title: Requirement Graph Runtime
...
Implements requirement graph contracts and tests within owned seams.
...
Boundary: hard/soft requirements, blockers, dependencies, proof needed.
...
Implement only the smallest usable slice that advances Requirement Graph Runtime. Any runtime behavior must be deterministic, local-first, evidence-producing, and compatible with Ambitions' five destination IA.
...
`Run LDI08 Requirement Graph Runtime`

## Next Safe Prompt / Next Gate

After Green or accepted Yellow, ask global train for the next eligible batch. Expected next gate: LDI09.
