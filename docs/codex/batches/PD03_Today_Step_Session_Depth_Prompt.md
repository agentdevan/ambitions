# PD03 Today Step Session Depth Prompt
...
Make Step Session the execution environment for a step.

## Product Depth Ownership

Owns: Focused step view; optional/secondary timer; context reminder; goal connection; pause/stop/close controls; completion and closure entry; receipt generation path.
...
- PD02 Green
- ME Today/TodayPanels gates Green
- PXOS visual/cognitive load gates Green
- Proof/receipt gates Green
...
- completion and closure entry
- receipt generation path.

## Batch-Specific Acceptance Criteria

- Step Session is step-first, not timer-first.
- Pause, stop, close, and completion controls are visible and non-shaming.
- Proof mutation is explicit and receipt-backed.
- Session exit returns to Today or Step Detail without route breakage.
...
It may claim only the completed, validated scope of `PD03` after evidence and commit.
...
`Run PD03 Today Step Session Depth`
