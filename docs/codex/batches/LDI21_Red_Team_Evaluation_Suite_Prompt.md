# LDI21 Red-Team Evaluation Suite Prompt
...
Builds permanent abuse-resistance fixture suite.
...
Boundary: 45 fixture families and validation matrix.
...
Implement only the smallest usable slice that advances Red-Team Evaluation Suite. Any runtime behavior must be deterministic, local-first, evidence-producing, and compatible with Ambitions' five destination IA.
...
`Run LDI21 Red-Team Evaluation Suite`

## Next Safe Prompt / Next Gate

After Green or accepted Yellow, ask global train for the next eligible batch. Expected next gate: LDI22.
