...
- Batch ID: `AOS03`
- Name: Graph Delta Review Projection Store
- Owning kernel: Life Graph Kernel / Runtime Contract
- Affected 3.0 primitive: Today / Goals / Capture / Time / You only where the train gate names a concrete surface
- Affected surface: all projections
- Dependency gate: depends on AOS02
- Implementation boundary: graph delta review and projection store contracts only after AOS02
...
- `git rev-parse HEAD`
- `git log -1 --oneline`
- `rg -n "AOS03|Life Graph Kernel|AmbitionsOS|release ready|App Store ready|TestFlight ready" docs .codex Native README.md AGENTS.md || true`
...
Primary owner: Life Graph Kernel / Runtime Contract. Before edits, produce a decision record listing the exact files this batch will touch, why the owner owns them, and which large-file, compatibility, privacy, performance, and release gates apply. If the owner file is not obvious, stop after discovery and write the decision record instead of guessing.

## Required Implementation Boundary

This batch may read typed local state relevant to Life Graph Kernel / Runtime Contract; it may produce typed contracts, tests, fixtures, projections, reports, or user-reviewable deltas only. Model output must not mutate the Life Graph. Source-sensitive facts must remain unverified until source evidence is attached. User-facing behavior is allowed only when the batch explicitly owns it and focused tests cover it.
...
- Required skills: `aos-train-orchestrator`, `aos-invariant-enforcer`, `runtime-contract-reviewer`, `validation-evidence-auditor`, `release-claim-truth-enforcer`, plus `Life Graph Kernel / Runtime Contract`-specific reviewers.
...
Stop on Red, missing predecessor gate, dirty target files owned by someone else, unclear owner, untrusted validation, missing rollback path, release-claim ambiguity, privacy ambiguity, or a request to broaden beyond AOS03.
...
`Run AOS03 Graph Delta Review Projection Store`
