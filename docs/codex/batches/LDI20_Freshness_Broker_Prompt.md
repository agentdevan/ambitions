...
Defines minimal non-personal source operation surface.
...
Boundary: manifest, changed claim IDs, signed diffs, generic update notifications, no user-data backend.
...
Implement only the smallest usable slice that advances Freshness Broker. Any runtime behavior must be deterministic, local-first, evidence-producing, and compatible with Ambitions' five destination IA.
...
`Run LDI20 Freshness Broker`

## Next Safe Prompt / Next Gate

After Green or accepted Yellow, ask global train for the next eligible batch. Expected next gate: LDI21.
