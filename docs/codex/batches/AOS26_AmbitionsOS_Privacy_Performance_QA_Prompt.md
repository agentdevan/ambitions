# AOS26 AmbitionsOS Privacy Performance QA Prompt
...
LDI hook: QA should include privacy/performance risks for source packs, recompiler, sync/archive, receipts, and sensitive previews/logs.
...
- `git rev-parse HEAD`
- `git log -1 --oneline`
- `rg -n "AOS26|Evaluation Kernel|AmbitionsOS|release ready|App Store ready|TestFlight ready" docs .codex Native README.md AGENTS.md || true`
...
Primary owner: Evaluation Kernel / Privacy Safety / Performance Energy. Before edits, produce a decision record listing the exact files this batch will touch, why the owner owns them, and which large-file, compatibility, privacy, performance, and release gates apply. If the owner file is not obvious, stop after discovery and write the decision record instead of guessing.

## Required Implementation Boundary

This batch may read typed local state relevant to Evaluation Kernel / Privacy Safety / Performance Energy; it may produce typed contracts, tests, fixtures, projections, reports, or user-reviewable deltas only. Model output must not mutate the Life Graph. Source-sensitive facts must remain unverified until source evidence is attached. User-facing behavior is allowed only when the batch explicitly owns it and focused tests cover it.
...
- Required skills: `aos-train-orchestrator`, `aos-invariant-enforcer`, `runtime-contract-reviewer`, `validation-evidence-auditor`, `release-claim-truth-enforcer`, plus `Evaluation Kernel / Privacy Safety / Performance Energy`-specific reviewers.
...
Stop on Red, missing predecessor gate, dirty target files owned by someone else, unclear owner, untrusted validation, missing rollback path, release-claim ambiguity, privacy ambiguity, or a request to broaden beyond AOS26.
...
`Run AOS26 AmbitionsOS Privacy Performance QA`
