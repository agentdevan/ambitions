...
- Batch ID: `ME05`
- Name: PlanFeatureService Extraction
- Mode: extraction only
- Owner file or files: `Native/Ambitions/Features/Plan/PlanFeatureService.swift`
- Extraction target: Extract Plan service responsibilities without changing Plan behavior or calendar posture.
...
- `git rev-parse HEAD`
- `git log -1 --oneline`
- `wc -l Native/Ambitions/Features/Plan/PlanFeatureService.swift` when the owner path is concrete
- `rg -n "TODO|compat|route|accessibilityIdentifier|Start here|What Ambitions Knows|failed|Profile|You" Native/Ambitions/Features/Plan/PlanFeatureService.swift || true` when the owner path is concrete
...
`Run ME05 PlanFeatureService Extraction`
