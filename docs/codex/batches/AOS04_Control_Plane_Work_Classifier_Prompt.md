...
LDI hook: Control Plane classifier should map Capture Understanding, Dream Seriousness Router, safety/legality triage, and lane candidate contracts without user-facing exposure.
...
- `git rev-parse HEAD`
- `git log -1 --oneline`
- `rg -n "AOS04|Control Plane|AmbitionsOS|release ready|App Store ready|TestFlight ready" docs .codex Native README.md AGENTS.md || true`
...
- Required skills: `aos-train-orchestrator`, `aos-invariant-enforcer`, `runtime-contract-reviewer`, `validation-evidence-auditor`, `release-claim-truth-enforcer`, plus `Control Plane`-specific reviewers.
...
Stop on Red, missing predecessor gate, dirty target files owned by someone else, unclear owner, untrusted validation, missing rollback path, release-claim ambiguity, privacy ambiguity, or a request to broaden beyond AOS04.
...
`Run AOS04 Control Plane Work Classifier`
