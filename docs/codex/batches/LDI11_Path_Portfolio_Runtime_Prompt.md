# LDI11 Path Portfolio Runtime Prompt
...
Generates path portfolio candidates without guarantees.
...
Boundary: primary/conservative/aggressive/exploration/fallback/North Star paths.
...
Implement only the smallest usable slice that advances Path Portfolio Runtime. Any runtime behavior must be deterministic, local-first, evidence-producing, and compatible with Ambitions' five destination IA.
...
`Run LDI11 Path Portfolio Runtime`

## Next Safe Prompt / Next Gate

After Green or accepted Yellow, ask global train for the next eligible batch. Expected next gate: LDI12.
