...
- Batch ID: `ME11`
- Name: Maintainability Regression Repair
- Mode: repair only
- Owner file or files: `files named by failed ME evidence`
- Extraction target: Repair classified ME regression only; no new extraction.
...
- `git rev-parse HEAD`
- `git log -1 --oneline`
- `wc -l files named by failed ME evidence` when the owner path is concrete
- `rg -n "TODO|compat|route|accessibilityIdentifier|Start here|What Ambitions Knows|failed|Profile|You" files named by failed ME evidence || true` when the owner path is concrete
...
`Run ME11 Maintainability Regression Repair`
