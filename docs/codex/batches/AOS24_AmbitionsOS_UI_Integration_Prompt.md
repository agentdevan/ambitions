# AOS24 AmbitionsOS UI Integration Prompt
...
LDI hook: UI integration must not expose LDI user-facing intelligence without SI/PD/AOS/LDI gates and must not create a sixth destination.
...
- `git rev-parse HEAD`
- `git log -1 --oneline`
- `rg -n "AOS24|Experience Kernel|AmbitionsOS|release ready|App Store ready|TestFlight ready" docs .codex Native README.md AGENTS.md || true`
...
- Required skills: `aos-train-orchestrator`, `aos-invariant-enforcer`, `runtime-contract-reviewer`, `validation-evidence-auditor`, `release-claim-truth-enforcer`, plus `Experience Kernel / Control Plane`-specific reviewers.
...
Stop on Red, missing predecessor gate, dirty target files owned by someone else, unclear owner, untrusted validation, missing rollback path, release-claim ambiguity, privacy ambiguity, or a request to broaden beyond AOS24.
...
`Run AOS24 AmbitionsOS UI Integration`
