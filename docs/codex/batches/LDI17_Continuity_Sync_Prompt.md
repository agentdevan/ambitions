...
Defines future continuity sync contracts without entitlements.
...
Boundary: CloudKit-private sync plan, iCloud states, local-only fallback. No entitlement/signing change in docs-only batch.
...
Implement only the smallest usable slice that advances Continuity Sync. Any runtime behavior must be deterministic, local-first, evidence-producing, and compatible with Ambitions' five destination IA.
...
`Run LDI17 Continuity Sync`

## Next Safe Prompt / Next Gate

After Green or accepted Yellow, ask global train for the next eligible batch. Expected next gate: LDI18.
