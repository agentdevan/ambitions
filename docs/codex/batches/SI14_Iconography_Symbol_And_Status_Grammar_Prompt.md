# SI14 Iconography Symbol And Status Grammar Prompt
...
- Batch ID: SI14
- Title: Iconography Symbol And Status Grammar
- Train: SI01-SI18 Signature Interface Implementation Train
- Global order after SI insertion: 061
- Type: SwiftUI/design-system implementation
...
Build status/source/proof/recovery icon and symbol grammar.
...
- Custom symbol assets without justification
- icon-only controls
- route/raw changes
- .github/workflows/**
...
AmbitionsStatusSymbol, proof/source/privacy/pressure symbol maps, label pairing rules.

Likely owner families: Sources/Components/**; Sources/Accessibility/**; Sources/Previews/**; Native/AmbitionsTests/**; docs/audits/**. Any actual owner change must be named in the batch report before edits.

## Interaction Requirements

Symbol state changes, focus feedback, reduced-motion unaffected semantics.

Motion must orient, confirm, or reduce uncertainty. Haptics must be system-native and optional.

## Accessibility Requirements

Icon labels, non-color backup, VoiceOver labels, Dynamic Type adjacent labels.

No icon-only meaning, no color-only meaning, no private content leak, and no Dynamic Type collapse.

## Visual Acceptance Criteria

Symbols are consistent, restrained, and support reading.
...
No random decorative icons or cartoonish clutter.
...
rg -n "SI14|Iconography Symbol And Status Grammar|Signature Interface|release ready|App Store ready|TestFlight ready" README.md docs .codex Native Sources AppUI || true
~~~

Strong build/tests, symbol scan, accessibility scan.

## Preview And Visual Evidence

Preview status/proof/recovery/privacy/pressure states.

Do not claim human visual approval unless a human actually approves. Do not claim physical-device proof unless performed.

## Green Criteria

- Scope completed exactly for SI14.
...
Rollback by reverting the SI14 commit. Repair Red by narrowing scope, restoring boundaries, adding missing evidence, splitting oversized files, or stopping with a Red repair report. Do not weaken product canon, tests, accessibility, compatibility, release truth, or SI gates.

## Claims

This batch may claim only the SI14 scope completed after validation, evidence, commit, and push.
...
Run SI14 Iconography Symbol And Status Grammar
