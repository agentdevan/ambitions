...
- Batch ID: `ME04`
- Name: TodayPanels Extraction
- Mode: extraction only
- Owner file or files: `Native/Ambitions/Features/Today/TodayPanels.swift`
- Extraction target: Extract focused panels/components while preserving copy and accessibility identifiers.
...
- `git rev-parse HEAD`
- `git log -1 --oneline`
- `wc -l Native/Ambitions/Features/Today/TodayPanels.swift` when the owner path is concrete
- `rg -n "TODO|compat|route|accessibilityIdentifier|Start here|What Ambitions Knows|failed|Profile|You" Native/Ambitions/Features/Today/TodayPanels.swift || true` when the owner path is concrete
...
`Run ME04 TodayPanels Extraction`
