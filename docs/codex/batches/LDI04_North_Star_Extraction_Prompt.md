# LDI04 North Star Extraction Prompt
...
Turns impossible or unsafe-literal dreams into safe meaning without validating literal harm.
...
Boundary: impossible/fantasy/symbolic/unsafe-literal dream conversion contracts and examples.
...
Implement only the smallest usable slice that advances North Star Extraction. Any runtime behavior must be deterministic, local-first, evidence-producing, and compatible with Ambitions' five destination IA.
...
`Run LDI04 North Star Extraction`

## Next Safe Prompt / Next Gate

After Green or accepted Yellow, ask global train for the next eligible batch. Expected next gate: LDI05.
