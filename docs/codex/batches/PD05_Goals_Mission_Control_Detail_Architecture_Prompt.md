# PD05 Goals Mission Control Detail Architecture Prompt
...
- Batch ID: `PD05`
- Name: Goals Mission Control Detail Architecture
- Train: Product Depth
- Type: Implementation
...
Turn Goal Detail into the real depth hub for a goal.

## Product Depth Ownership

Owns: Mission Control lanes; path lane; proof lane; decision lane; risk/blocker lane; next-step lane; alternate-path lane; goal health summary.
...
- PX03 Green
- PX14 Green
- ME Goals owner gate Green
- CS goal route/name seam check if relevant
...
- Goal Detail is a depth hub, not a goal card stack.
- Mission Control lanes are visually distinct and owned.
- Next step, proof, risk, path, and decisions are available without becoming an OKR surface.
- Route/name compatibility is checked before any navigation change.
...
It may claim only the completed, validated scope of `PD05` after evidence and commit.
...
`Run PD05 Goals Mission Control Detail Architecture`
