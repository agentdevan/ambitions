...
- Batch ID: `ME12`
- Name: Maintainability Handoff
- Mode: handoff only
- Owner file or files: `docs/audits, docs/codex, .codex/reports`
- Extraction target: Write handoff and next-decision record only.
...
- `git rev-parse HEAD`
- `git log -1 --oneline`
- `wc -l docs/audits, docs/codex, .codex/reports` when the owner path is concrete
- `rg -n "TODO|compat|route|accessibilityIdentifier|Start here|What Ambitions Knows|failed|Profile|You" docs/audits, docs/codex, .codex/reports || true` when the owner path is concrete
...
`Run ME12 Maintainability Handoff`
