# SI11 Personal System Center Components Prompt
...
- Batch ID: SI11
- Title: Personal System Center Components
- Train: SI01-SI18 Signature Interface Implementation Train
- Global order after SI insertion: 058
...
Build reusable You top/header/grouped navigation primitives.
...
SystemProfileHeader, setup completeness, trust summary, grouped navigation variants.

Likely owner families: Native/Ambitions/Features/Profile/**; Sources/Components/**; Sources/Previews/**; Native/AmbitionsTests/**; docs/audits/**. Any actual owner change must be named in the batch report before edits.

## Interaction Requirements

Row feedback, disclosure, setup-needed states, reduced-motion row state.

Motion must orient, confirm, or reduce uncertainty. Haptics must be system-native and optional.

## Accessibility Requirements

VoiceOver row grouping, Dynamic Type section layout, private summaries.

No icon-only meaning, no color-only meaning, no private content leak, and no Dynamic Type collapse.

## Visual Acceptance Criteria

Grouped lists serve control and navigation only where that is the object.
...
No social profile or cluttered settings junk drawer.
...
rg -n "SI11|Personal System Center Components|Signature Interface|release ready|App Store ready|TestFlight ready" README.md docs .codex Native Sources AppUI || true
~~~

Strong Profile/You tests/build, SI navigation/accessibility scans.

## Preview And Visual Evidence

Preview setup incomplete, trust healthy, private, disabled source.

Do not claim human visual approval unless a human actually approves. Do not claim physical-device proof unless performed.

## Green Criteria

- Scope completed exactly for SI11.
...
Rollback by reverting the SI11 commit. Repair Red by narrowing scope, restoring boundaries, adding missing evidence, splitting oversized files, or stopping with a Red repair report. Do not weaken product canon, tests, accessibility, compatibility, release truth, or SI gates.

## Claims

This batch may claim only the SI11 scope completed after validation, evidence, commit, and push.
...
Run SI11 Personal System Center Components
