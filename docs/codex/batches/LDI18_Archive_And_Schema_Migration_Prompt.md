# LDI18 Archive And Schema Migration Prompt
...
Boundary: encrypted archive design, schema migration ladder, import/export rules.
...
Implement only the smallest usable slice that advances Archive And Schema Migration. Any runtime behavior must be deterministic, local-first, evidence-producing, and compatible with Ambitions' five destination IA.
...
`Run LDI18 Archive And Schema Migration`

## Next Safe Prompt / Next Gate

After Green or accepted Yellow, ask global train for the next eligible batch. Expected next gate: LDI19.
