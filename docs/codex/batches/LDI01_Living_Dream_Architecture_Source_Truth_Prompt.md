# LDI01 Living Dream Architecture Source Truth Prompt
...
- Batch ID: LDI01
- Title: Living Dream Architecture Source Truth
...
Creates/updates LDI source truth and governance references.
...
Boundary: docs/canon + codex governance only.
...
Implement only the smallest usable slice that advances Living Dream Architecture Source Truth. Any runtime behavior must be deterministic, local-first, evidence-producing, and compatible with Ambitions' five destination IA.
...
`Run LDI01 Living Dream Architecture Source Truth`

## Next Safe Prompt / Next Gate

After Green or accepted Yellow, ask global train for the next eligible batch. Expected next gate: LDI02.
