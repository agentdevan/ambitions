...
- Batch ID: `ME01`
- Name: Maintainability Baseline And Ownership Map
- Mode: audit only
- Owner file or files: `all Lane 2 candidate files`
- Extraction target: Build the ownership map and risk baseline. Do not extract code.
...
- `git rev-parse HEAD`
- `git log -1 --oneline`
- `wc -l all Lane 2 candidate files` when the owner path is concrete
- `rg -n "TODO|compat|route|accessibilityIdentifier|Start here|What Ambitions Knows|failed|Profile|You" all Lane 2 candidate files || true` when the owner path is concrete
...
`Run ME01 Maintainability Baseline And Ownership Map`
