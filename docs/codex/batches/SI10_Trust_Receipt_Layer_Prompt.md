# SI10 Trust Receipt Layer Prompt
...
- Batch ID: SI10
- Title: Trust Receipt Layer
- Train: SI01-SI18 Signature Interface Implementation Train
- Global order after SI insertion: 057
...
Build reusable trust/proof/receipt layer.
...
TrustReceiptToast, inline receipt, ProofPreview, SourceFreshnessLabel, Why this affordance.
...
Toast timing, undo/review entry, receipt dismissal, reduced-motion equivalent.

Motion must orient, confirm, or reduce uncertainty. Haptics must be system-native and optional.

## Accessibility Requirements

Receipt labels, redaction, source freshness, Dynamic Type, non-color proof meaning.

No icon-only meaning, no color-only meaning, no private content leak, and no Dynamic Type collapse.

## Visual Acceptance Criteria

Receipts are subtle, source-grounded, and reversible where relevant.
...
No fake certainty or intrusive modal default.
...
Rollback by reverting the SI10 commit. Repair Red by narrowing scope, restoring boundaries, adding missing evidence, splitting oversized files, or stopping with a Red repair report. Do not weaken product canon, tests, accessibility, compatibility, release truth, or SI gates.

## Claims

This batch may claim only the SI10 scope completed after validation, evidence, commit, and push.
