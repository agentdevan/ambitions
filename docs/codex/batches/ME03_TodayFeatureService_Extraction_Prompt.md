# ME03 TodayFeatureService Extraction Prompt
...
- Batch ID: `ME03`
- Name: TodayFeatureService Extraction
- Mode: extraction only
- Owner file or files: `Native/Ambitions/Features/Today/TodayFeatureService.swift`
- Extraction target: Extract Today service responsibilities without changing Today visible behavior.
...
- `git rev-parse HEAD`
- `git log -1 --oneline`
- `wc -l Native/Ambitions/Features/Today/TodayFeatureService.swift` when the owner path is concrete
- `rg -n "TODO|compat|route|accessibilityIdentifier|Start here|What Ambitions Knows|failed|Profile|You" Native/Ambitions/Features/Today/TodayFeatureService.swift || true` when the owner path is concrete
...
`Run ME03 TodayFeatureService Extraction`
