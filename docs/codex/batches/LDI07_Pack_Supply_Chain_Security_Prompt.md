# LDI07 Pack Supply Chain Security Prompt
...
- Batch ID: LDI07
- Title: Pack Supply Chain Security
...
Adds pack integrity, validation, rollback, and tamper-handling proof.
...
Boundary: signed packs, checksum, provenance, rollback, corruption handling, no executable logic.
...
Implement only the smallest usable slice that advances Pack Supply Chain Security. Any runtime behavior must be deterministic, local-first, evidence-producing, and compatible with Ambitions' five destination IA.
...
`Run LDI07 Pack Supply Chain Security`

## Next Safe Prompt / Next Gate

After Green or accepted Yellow, ask global train for the next eligible batch. Expected next gate: LDI08.
