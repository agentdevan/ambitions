...
> Candidate references: AMB28-retired_ia_or_terminology_reference-50558884, AMB28-same_source_file_targeted_by_multiple_active_batches-22647572, AMB28-same_source_file_targeted_by_multiple_active_batches-65376188, AMB28-same_surface_multiple_active_batches-13212827, AMB28-same_surface_multiple_active_batches-26899932, AMB28-same_surface_multiple_active_batches-34058953, AMB28-same_surface_multiple_active_batches-66075429
...
- Batch ID: `PD02`
- Name: Today Step Detail Depth
- Train: Product Depth
- Type: Implementation
- Primary surface ownership: Today
- Required approval phrase: `Start Product Depth Train`

## Purpose

Make Step Detail a real lightweight drill-down, not a card-expanded task row.

## Product Depth Ownership

Owns: Step Detail screen/sheet; step title, why-now, context, duration, goal link, source/evidence label, primary action, closure entry points, lightweight proof/receipt access.
...
- PX02 Green
- PX07/PX08 relevant gates Green or accepted Yellow
- ME Today owner gate Green
- Accessibility and copy gates Green
...
- Step Detail screen/sheet
- step title, why-now, context, duration, goal link, source/evidence label, primary action, closure entry points, lightweight proof/receipt access.

## Batch-Specific Acceptance Criteria

- Today top level stays visual and compact.
- Rail row opens Step Detail; detail is behind tap, not expanded into a top-level task row.
- Start now launches Step Session where available and stays honest if blocked.
- Step Detail includes source/proof labels without dense history overload.
...
It may claim only the completed, validated scope of `PD02` after evidence and commit.
...
`Run PD02 Today Step Detail Depth`
...
LDI hook: Today depth may include source-check, proof, clarification, review, and recovery steps as step states, not separate task lists.
