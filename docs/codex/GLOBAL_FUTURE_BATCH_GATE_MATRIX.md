...
| Product Depth Gate | Drill-down/depth work | Deepens existing surfaces and has PXOS + ME/CS prerequisites. | Formal PD prompt missing but no implementation starts. | Widens app or starts Product Depth without approval. | `product-depth-strategist`, `deep-not-wide-product-reviewer` |
...
- Product Depth: PXOS, Product Depth, Top-Level Composition, ME, CS, Accessibility, Visual, Copy, Validation Strength.
