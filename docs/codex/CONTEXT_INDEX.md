...
SI starts only through global dry-run selection or `Start Signature Interface Train`. SI01-SI17 are complete as bounded Signature Interface evidence; SI18 remains queued/blocked and not implemented.
...
LDI is future source truth/governance only until its implementation batches run. SI18 remains the next active global batch after SI17.
