...
SI starts only through global dry-run selection or `Start Signature Interface Train`. SI01-SI07 are complete as bounded Signature Interface evidence; SI08-SI18 remain queued/blocked and not implemented.
...
LDI is future source truth/governance only until its implementation batches run. SI08 remains the next active global batch after SI07.
