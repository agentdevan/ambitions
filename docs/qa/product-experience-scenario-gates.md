...
|---|---|---|---|---|
| `origin_high_ambition_low_operating_structure_user_can_start` | A high-agency user with more ambition than operating structure can see what matters and start with a fitting Step. | Partial | `docs/truth/PRODUCT_ORIGIN_TRUTH.md`, `Native/Ambitions/Surfaces/Today/Projection/TodayExecutionProjector.swift`, `Native/Ambitions/DesignSystem/ProductObjects/TodayStartHereSurface.swift` | End-to-end first-session scenario, rendered Today proof, accessibility proof |
| `origin_many_goals_many_obligations_today_remains_clear` | Many simultaneous goals and obligations do not turn Today into a cluttered list or dashboard. | Partial | `docs/truth/PRODUCT_DESIGN_TRUTH.md`, `Native/Ambitions/Surfaces/Today/Projection/TodayExecutionProjector.swift` | Overloaded-day scenario test, rendered first-viewport proof, visual review |
...
| `foundation_offline_core_runs_without_account` | Core app runs without account or network. | Partial | `docs/truth/IMPLEMENTATION_TRUTH.md`, `Native/Ambitions/Core/Persistence/`, `Native/Ambitions/Core/LocalRuntimeOS/Scheduling/ProtectedStepPlacementPolicy.swift`, `Native/AmbitionsTests/Runtime/P1FLocalSearchFoundationTests.swift`, `Native/AmbitionsTests/Time/P1DTimeFoundationTests.swift`, `Native/AmbitionsTests/LocalRuntimeOS/Scheduling/ProtectedStepPlacementPolicyTests.swift` | Network-disabled device workflow, no-account launch matrix, release evidence |
...
| `goal_without_deadline_generates_flexible_path` | Goal without deadline gets flexible path. | Partial | `Native/Ambitions/Core/LocalRuntimeOS/Planning/DeterministicGoalPlanner.swift` | Flexible scheduled path proof |
...
| `today_shows_only_relevant_schedule_slice` | Today shows the current slice, not the entire long-term path. | Partial | `Native/Ambitions/Surfaces/Today/Projection/TodayExecutionProjector.swift` | Today scenario test and screenshot proof |
| `time_shows_whether_goal_fits` | Time shows whether a goal fits. | Partial | `Native/Ambitions/Surfaces/Time/Projection/TimePressureProjection.swift`, `Native/Ambitions/DesignSystem/ProductObjects/LifeShapeFieldCapacity.swift` | Fit/over-capacity scenario test |
...
| `near_term_step_edits_normally` | Near-term Step edits normally. | Partial | `Native/Ambitions/Core/LocalRuntimeOS/Commands/CommandRouter.swift` | Edit UI/runtime/persistence proof |
...
| Gate ID | User-visible behavior | Current status | Evidence path if found | Required future proof |
|---|---|---|---|---|
| `automatic_future_adjustment_occurs_only_where_allowed` | Automatic adjustment follows automation permissions. | Partial | `Native/Ambitions/Core/LocalRuntimeOS/Scheduling/ProtectedStepPlacementPolicy.swift`, `Native/AmbitionsTests/LocalRuntimeOS/Scheduling/ProtectedStepPlacementPolicyTests.swift` | Full automation settings policy, rendered review UI, receipt/relaunch proof |
| `automatic_adjustment_does_not_move_next_seven_days_silently` | Next seven days are not silently moved. | Partial | `Native/Ambitions/Core/LocalRuntimeOS/Scheduling/ProtectedStepPlacementPolicy.swift`, `Native/Ambitions/Core/LocalRuntimeOS/Commands/PolicyGuardedCommandExecutor.swift`, `Native/AmbitionsTests/LocalRuntimeOS/Scheduling/ProtectedStepPlacementPolicyTests.swift`, `Native/AmbitionsTests/LocalRuntimeOS/Commands/PolicyGuardedCommandExecutorTests.swift` | Rendered approval/review UI, accessibility proof, device/no-network proof |
| `user_caused_adjustment_shows_impact_summary` | User-caused adjustment shows impact summary. | Partial | `Native/Ambitions/Surfaces/Time/Projection/TimeReflowDecisionProjector.swift`, `Native/Ambitions/Core/LocalRuntimeOS/Scheduling/ProtectedStepPlacementPolicy.swift`, `Native/AmbitionsTests/LocalRuntimeOS/Scheduling/ProtectedStepPlacementPolicyTests.swift` | User-caused change UI, receipt proof, accessibility proof |
...
| `ignored_plan_suggests_contextual_recovery` | Ignored plan suggests contextual recovery. | Partial | `Native/Ambitions/Surfaces/Time/Projection/TimeCalendarRecoveryProjection.swift` | Multi-day ignored-plan scenario proof |
...
| Gate ID | User-visible behavior | Current status | Evidence path if found | Required future proof |
|---|---|---|---|---|
| `priority_supports_low_normal_high` | Priority supports Low, Normal, High. | Partial | `Native/Ambitions/Core/LocalRuntimeOS/Commands/AmbitionsCommandTaxonomy.swift`; `Native/Ambitions/Core/LocalRuntimeOS/Scheduling/PriorityPlacementPolicy.swift`; `Native/AmbitionsTests/LocalRuntimeOS/Scheduling/PriorityPlacementPolicyTests.swift` | Rendered priority UI and accessibility proof |
...
| `priority_can_be_overridden_by_user` | User can override priority. | Partial | `Native/Ambitions/Core/LocalRuntimeOS/Scheduling/PriorityPlacementPolicy.swift`; `Native/AmbitionsTests/LocalRuntimeOS/Scheduling/PriorityPlacementPolicyTests.swift` | Rendered edit control, persistence/relaunch proof, and accessibility proof |
| `priority_does_not_include_must_fit_as_goal_type` | `Must Fit` is not a goal type. | Partial | `Native/Ambitions/Core/LocalRuntimeOS/Scheduling/PriorityPlacementPolicy.swift`; `Native/AmbitionsTests/LocalRuntimeOS/Scheduling/PriorityPlacementPolicyTests.swift`; vocabulary drift scan | Broader goal-type audit and rendered priority proof |
...
| `life_capital_supports_capability_credential_resource_relationship_accomplishment_proof_pattern` | Life Capital supports all canon content types. | Partial | `Native/Ambitions/Core/Domain/SourceAtlasPackModels+06-SourceAtlasPack.swift`, `Native/Ambitions/Surfaces/You/Projection/YouFeatureServiceLifeContextRuntimeProjection.swift` | Dedicated Life Capital model/tests |
...
| `life_capital_patterns_show_broad_stats` | Patterns show broad stats. | Partial | `Native/Ambitions/Surfaces/You/Projection/YouFeatureServiceLearningControlsProjection.swift` | Pattern rows and planning effect proof |
...
| `review_surfaces_accomplishments_and_proof` | Review surfaces accomplishments and proof. | Partial | `Native/Ambitions/Surfaces/You/Projection/YouFeatureServiceReviewProjection.swift` | Review UI/export proof |

## Source Atlas

| Gate ID | User-visible behavior | Current status | Evidence path if found | Required future proof |
|---|---|---|---|---|
| `source_atlas_invisible_by_default` | Source Atlas is invisible by default. | Partial | `docs/truth/PRODUCT_DESIGN_TRUTH.md`, `Native/Ambitions/Surfaces/You/Projection/YouFeatureServiceSourceAtlasPrimaryProjection.swift` | Surface audit proving no marketplace/center |
...
| `source_inspection_available_when_user_asks_why` | Source inspection appears when asked why. | Partial | `Native/Ambitions/Trust/Projection/InspectionStageScene.swift`, `Native/Ambitions/Trust/` | Inspection UI proof |
...
| `review_includes_accomplishments` | Review includes accomplishments. | Partial | `Native/Ambitions/Surfaces/You/Projection/YouFeatureServiceReviewProjection.swift` | Rendered review content proof |
| `review_includes_broad_patterns` | Review includes broad patterns. | Partial | `Native/Ambitions/Surfaces/You/Projection/YouHistoryPatternProjection.swift` | Pattern review proof |
...
| `automation_never_forces` | Automation never forces. | Partial | `Native/Ambitions/Surfaces/Time/Projection/TimeReflowDecisionProjector.swift`, `docs/truth/PRODUCT_EXPERIENCE_CANON.md` | Policy tests and destructive-action proof |
| `automation_respects_protected_near_term_placement` | Automation respects protected near-term placement. | Partial | `Native/Ambitions/Core/LocalRuntimeOS/Scheduling/ProtectedStepPlacementPolicy.swift`, `Native/Ambitions/Core/LocalRuntimeOS/Commands/PolicyGuardedCommandExecutor.swift`, `Native/AmbitionsTests/LocalRuntimeOS/Scheduling/ProtectedStepPlacementPolicyTests.swift`, `Native/AmbitionsTests/LocalRuntimeOS/Commands/PolicyGuardedCommandExecutorTests.swift` | Rendered review UI, receipt persistence, accessibility proof, device/no-network proof |
| `automation_shows_user_caused_impact` | User-caused automation impact is visible. | Partial | `Native/Ambitions/Surfaces/Time/Projection/TimeReflowDecisionProjector.swift` | User-caused impact UI proof |
