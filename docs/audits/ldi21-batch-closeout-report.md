...
## Files Changed
- `Native/Ambitions/Domain/Planning/LivingPlanRedTeamEvaluator.swift` (added)
- `Native/AmbitionsTests/Domain/LivingPlanRedTeamEvaluatorTests.swift` (added)
...
## EFC Applicability
Invoked. The Red-Team Evaluation Suite inspects goals deterministically and surfaces weaknesses natively via explicit mutation receipts rather than silently overwriting user plans with cloud-based intelligence.
...
## Rollback Notes
If needed, can revert changes to `Native/Ambitions/Domain/Planning/LivingPlanRedTeamEvaluator.swift` and `Native/AmbitionsTests/Domain/LivingPlanRedTeamEvaluatorTests.swift`.
