...
## Files Changed
- `Native/Ambitions/Domain/AmbitionsOSRuntimeTailGate.swift` (added)
- `Native/AmbitionsTests/Domain/AmbitionsOSRuntimeTailGateTests.swift` (added)
...
## EFC Applicability
Invoked. The AOS Runtime Tail Gate uses deterministically evaluated properties to explicitly request confirmation when critical runtime, privacy, evaluation, and experience obligations are not yet fully proven.
...
## Rollback Notes
If needed, can revert changes to `Native/Ambitions/Domain/AmbitionsOSRuntimeTailGate.swift` and `Native/AmbitionsTests/Domain/AmbitionsOSRuntimeTailGateTests.swift`.
