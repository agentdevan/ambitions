...
## Files Changed
- `Native/Ambitions/Domain/AmbitionsOSHandoffTailGate.swift` (added)
- `Native/AmbitionsTests/Domain/AmbitionsOSHandoffTailGateTests.swift` (added)
...
## EFC Applicability
Invoked. The Handoff Tail Gate ensures that all external handoff obligations—such as dependencies on Source Atlas, intelligence control gates, and cross-train checks—are evaluated before proceeding.
...
## Rollback Notes
If needed, can revert changes to `Native/Ambitions/Domain/AmbitionsOSHandoffTailGate.swift` and `Native/AmbitionsTests/Domain/AmbitionsOSHandoffTailGateTests.swift`.
