...
## Files Changed
- `Native/Ambitions/Domain/Planning/LivingPlanSchemaMigration.swift` (added)
- `Native/AmbitionsTests/Domain/LivingPlanSchemaMigrationTests.swift` (added)
...
## EFC Applicability
Invoked. Schema migrations are deterministically tracked through the same receipt framework as plan recompiles, preventing silent mutation of living plans during schema bumps and surfacing confirmation when destructive changes occur.
...
## Rollback Notes
If needed, can revert changes to `Native/Ambitions/Domain/Planning/LivingPlanSchemaMigration.swift` and `Native/AmbitionsTests/Domain/LivingPlanSchemaMigrationTests.swift`.
