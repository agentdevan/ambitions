# EB21 Concierge Setup And Planning Defaults Onboarding Report

Date: 2026-05-03
Batch: EB21 Concierge Setup And Planning Defaults Onboarding
Global order: 071
Starting HEAD: 2ffba909
...
EB21 is closed as a gate/evidence batch, not product implementation. The prompt
allows implementation later for concierge setup and planning defaults, but does
not name an exact production owner file, behavior change, focused test lane,
migration, preview, or rollback plan for product Swift in this run.

This batch therefore records the concierge setup and planning-defaults contract
...
- Trust, Privacy, And User Control: setup questions must expose purpose,
  source, edit/delete, skip, and receipt paths before any durable memory is
  created.
- Accessibility And Cognitive Load: concierge setup must be skippable,
  low-load, non-shaming, Dynamic Type-aware, VoiceOver-readable, and Reduce
  Motion safe.
- Planning Kernel: planning defaults must remain user-controlled suggestions
  and must not request calendar permission during onboarding.

## Concierge Setup And Planning Defaults Contract

Future EB21 implementation may proceed only if it preserves these constraints:

- Keep setup useful immediately; do not turn onboarding into an account,
  dashboard, habit, or AI-theater flow.
- Explain why each setup question matters and keep every answer editable.
- Keep planning defaults inside Plan, Today, You, or owned setup drill-downs.
- Never request calendar permission during onboarding.
- Do not create durable memory until source, confidence, edit, delete, and
  receipt controls are present.
- Provide skip/later/recovery paths without shame language.
- Record preview/fixture evidence for normal setup, skipped setup, low-data,
...
- `scripts/eb-no-5-version-drift-scan.sh || true`: PASS; no active EB21 5.0
...
- `scripts/release-claim-safety-scan.sh || true`: completed by advisory scan
  convention; no EB21 unsupported release claim was introduced.
...
- Focused onboarding/UI tests: not required unless product behavior changes.
...
The EB21 prompt still said `Global order after EB insertion: 067`, but current
repo global order after DAV insertion makes EB21 global order 071. This batch
...
- Existing docs QA/canon-language advisory backlog may remain outside EB21.
- EB21 is Green only as gate/evidence. Product implementation remains future
  Yellow until a later batch names files and tests.

## Next Eligible Batch

EB22 Privacy Setup And Trust Onboarding, global order 072, after EB21
