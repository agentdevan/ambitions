...
| `Native/Ambitions/Core/Persistence/SwiftDataRepositories+05-SwiftDataAmbitionGraphProjectionRecordRepository.swift` | Projection storage | `ProjectionEngine`, `ProjectionStoreSQLite`, and `Commands` for command execution records | P-MAP, P-REJECT, P-STORAGE | Unsafe legacy projection/command record repository writes through SwiftData. |
