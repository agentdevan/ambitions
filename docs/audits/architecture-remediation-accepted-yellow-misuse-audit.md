...
- Legacy runtime: `python3 scripts/ambitions-legacy-runtime-production-use-guard.py --json` now reports `currentLegacyRuntimeFiles=64` and `legacyRuntimeFileCeiling=64` after the AMB-1730 standalone PlanningEngine, goal clarification/contradiction, TimeEngine, step planning/scheduling, and MemoryLens/SearchRecall owner-move batches. M02 administrative completion does not mean legacy runtime authority is gone.
...
| AMB-1666 | Needs Repair | `invalid_required_scope_incomplete` | Repair through AMB-1730; eliminate remaining legacy runtime production authority. AMB-1730 reduced the count from `111` to `64`, but remaining live authority still blocks Green. |
