...
- Legacy runtime: `python3 scripts/ambitions-legacy-runtime-production-use-guard.py --json` now reports `currentLegacyRuntimeFiles=67` and `legacyRuntimeFileCeiling=67` after the AMB-1730 standalone PlanningEngine, goal clarification/contradiction, TimeEngine, and step planning/scheduling owner-move batches. M02 administrative completion does not mean legacy runtime authority is gone.
...
| AMB-1666 | Needs Repair | `invalid_required_scope_incomplete` | Repair through AMB-1730; eliminate remaining legacy runtime production authority. AMB-1730 reduced the count from `111` to `67`, but remaining live authority still blocks Green. |
