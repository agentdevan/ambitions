...
- Legacy runtime: `python3 scripts/ambitions-legacy-runtime-production-use-guard.py --json` now reports `currentLegacyRuntimeFiles=0` and `legacyRuntimeFileCeiling=0` after the AMB-1730 all-remaining owner pass. Focused Swift/Xcode validation was skipped by user instruction, so AMB-1730 should remain open until build/typecheck proof is re-enabled and passes.
...
| AMB-1666 | Needs Repair | `invalid_required_scope_incomplete` | Repair through AMB-1730; eliminate remaining legacy runtime production authority. AMB-1730 reduced the count from `111` to `0`; build/typecheck proof remains skipped by user instruction, so closure is not claimed. |
