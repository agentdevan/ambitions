...
Deleted tracked workflow files:
...
Hosted workflow runs, hosted-CI logs, and Actions artifacts are not valid current proof sources.

## Remaining Historical Mentions

Residual mentions of GitHub Actions, hosted CI, Actions artifacts, `.github/workflows/**`, or `ios-validate.yml` may remain only when clearly historical, archival, removed-policy, or forbidden-current-proof language.
...
This batch makes no hosted-CI proof claim. It intentionally removes hosted workflow proof as a current validation source and replaces it with local/Codex-operated validation.
...
Follow-up: classify residual hosted-CI mentions after running `rg -n "\.github/workflows|GitHub Actions|hosted CI|Actions artifact|ios-validate\.yml" README.md docs .codex || true` on a local checkout.
Recheck condition: all remaining active-doc hits must either be removed or explicitly classified as historical/forbidden-current-proof language; `test ! -d .github/workflows` must pass.
