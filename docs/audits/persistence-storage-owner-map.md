...
| `BlobStoreFileSystem.swift` | Blob/cache storage | `Core/LocalRuntimeOS/Storage` | `CaptureRouting`, `TrustSystem`, `PrivacySecurity` when scoped | Attachment/proof blobs, checksums, content type, file-protection class records | No queryable domain authority, no event ordering, no migration approval. |
...
| `goalDraft` | `GoalDraftRecord` | `CaptureRouting`, `Transactions` | Store draft identity, promotion lookup, fallback draft payload | Capture/draft writes must be command-receipt gated before Green. |
...
| `capture` | `CaptureRecord` | `CaptureRouting`, `Transactions` | Store capture lookup, linked goal reference, fallback capture payload | No global Capture routing Green from records alone. |
...
| --- | --- | --- | --- |
| Repository contracts in `PersistenceContracts+02-AFEPQueryBudgetCatalog.swift` | Split by family into `ObjectState`, `Commands`, `CaptureRouting`, `PlanningEngine`, `Scheduling`, `TrustSystem`, `Projections`, `ExternalWrites`, `MigrationRepair`, and `Storage` | `save`, `append`, `delete`, import, and unit-of-work APIs are legacy adapter contracts, not business-decision authority. | P-REJECT |
