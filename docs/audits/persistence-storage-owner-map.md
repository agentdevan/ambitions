...
| --- | --- | --- | --- | --- |
| `goalThread` | `GoalRecord` | `Commands`, `TransactionKernel`, `EventJournal`; planning policy remains under `PlanningEngine` when scoped | Store IDs, lifecycle columns, encoded planning/progress payloads, fallback goal snapshot | Legacy goal repository writes remain Yellow until P-REJECT. |
