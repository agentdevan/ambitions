...
| Schema version ledger and migration scaffold coverage are PK-proven, but migration safety is not. | Open Yellow | PK09-PK13 | PK07 names current storage versions; PK08 scaffolds blocked migration planning gates. No migration-safe or data-loss-proof claim until unknown-value degradation, invariant checks, backup, import dry run, and rollback are proven. |
...
| Package/module moves can destabilize builds. | Scanner Yellow | PK09-PK41 | PK01 boundary scaffold and PK02 scanner exist; current scanner findings are Yellow and no package-split claim is allowed until later focused builds pass. |
