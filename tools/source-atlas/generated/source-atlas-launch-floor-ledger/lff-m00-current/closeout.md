...
| --- | ---: | --- | --- | --- |
| 1M+ public/reference shards before public launch | missing | `not_measurable_fail_closed` | canonical shard corpus manifest is missing | Create canonical shard corpus manifest with publicReferenceShards >= 1,000,000.<br>Partition shards into pack/R2/mobile index layout with checksum/readback proof. |
| 500+ goal domains | 14 | `not_met` | goal_domains counter is 14, below required 500 | Expand active governed frontier taxonomy to at least 500 lawful public/reference goal domains.<br>Keep every domain mapped to source/legal/API lane governance before promotion. |
| 5,000+ subdomains | missing | `not_measurable_fail_closed` | canonical subdomain taxonomy is missing | Add canonical subdomain taxonomy with at least 5,000 governed subdomains.<br>Attach each subdomain to parent domain, source lane, legal posture, API posture, and freshness SLA. |
...
- `LFT-002` 500-domain active public/reference taxonomy: Expand active governed frontier taxonomy to at least 500 lawful public/reference goal domains. Keep every domain mapped to source/legal/API lane governance before promotion.
- `LFT-003` 5,000-subdomain governed taxonomy: Add canonical subdomain taxonomy with at least 5,000 governed subdomains. Attach each subdomain to parent domain, source lane, legal posture, API posture, and freshness SLA.
...
- `status` git status --short: confirm scoped working tree before and after LFF-M00 evidence generation
- `launch_floor_pytest` python3 -m pytest tools/source-atlas/foundry/tests/test_source_atlas_launch_floor_ledger.py tools/source-atlas/foundry/tests/test_source_atlas_completion_audit_train_129.py: prove fail-closed launch-floor ledger and completion-audit wiring
- `launch_floor_ledger` python3 tools/source-atlas/source-atlas-foundry.py source-atlas-launch-floor-ledger --output-root tools/source-atlas/generated/source-atlas-launch-floor-ledger/lff-m00-current --emit-evidence docs/qa/source-atlas/source-atlas-launch-floor-ledger-current.json --markdown docs/qa/source-atlas/source-atlas-launch-floor-ledger-current.md: regenerate current launch-floor ledger without source/R2/native mutation
