...
Status: Source Green for bounded autonomous Source Atlas production orchestrator
Source Atlas status ceiling: Yellow overall Source Atlas; bounded configured public/reference production orchestrator only
Overall readiness: bounded_autonomous_production_operational
...
- Production domains ready: 14
- Remote R2 uploads/readbacks reconciled: 14
- Goal-domain gauntlet cases: 14
- Autonomous cycle actions: 19
- Local cycle actions executed: 19
- Local monitor checks executed: 14
- Remote mutations: 0
...
| --- | --- | --- | --- | --- |
| production_target | configured_frontiers_ready | yes | production target ledger | none |
| internal_legal_terms | source_specific_internal_terms_review_ready | yes | production finish-line gate | none |
| live_transport_and_r2 | current_remote_r2_upload_readback_reconciled | yes | production sweep | none |
| ambitions_runtime_boundary | bounded_configured_runtime_green_not_release_green | yes | production finish-line gate | none |
| arbitrary_goal_domains | configured_domains_route_to_production_unknown_domains_candidate_only | yes | arbitrary-domain handling gate | none |
| representative_goal_domain_gauntlet | configured_cases_pass_unknown_cases_candidate_only | yes | goal-domain gauntlet | none |
| autonomous_control_loop | monitoring_ready_writes_held | yes | autonomous control loop | none |
| autonomous_cycle_runner | recurring_cycle_actions_emitted | yes | autonomous cycle runner | none |
| autonomous_cycle_execution | local_cycle_actions_executed_no_mutation | yes | autonomous cycle executor | none |
| claim_ceiling | release_legal_universal_and_final_output_claims_blocked | yes | all gate reports | none |
...
- outside_legal_approval: not_claimed (outside legal approval requires an explicit current outside legal artifact)
- release_green: not_claimed (Release Green requires umbrella release proof, current device proof, accessibility proof, rollback proof, and owner approval)
- literal_universal_coverage: forbidden_claim (Source Atlas supports governed arbitrary public/reference domain expansion, not literal universal coverage)
- future_remote_r2_write: execute_gated (future R2 writes require explicit execute, budget, approval, and credentials; this orchestrator reconciles current proof only)
