...
def _counters(
    ...
    return {
        ...
        "recordCounts": {
            ...
            "missingShardReviewGateDecisions": _record_count(missing_shard_review_gate, "gateDecisions"),
            "missingShardReviewGateApprovedEvents": _record_count(missing_shard_review_gate, "approvedEvents"),
            "missingShardReviewGateBlockedEvents": _record_count(missing_shard_review_gate, "blockedEvents"),
            "missingShardReviewGatePlannedRegistryMutations": _record_count(
                missing_shard_review_gate,
                "plannedRegistryMutations",
            ),
            "missingShardReviewGateActiveRegistryMutations": _record_count(
                missing_shard_review_gate,
...
def _current_capabilities(counters: dict[str, Any], target_statuses: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    if counts["missingShardReviewGateDecisions"] is not None:
        review_gate_proven = (
            counts["missingShardReviewGateActiveRegistryMutations"] == 0
            and counts["missingShardReviewGateCoverageCounterMutations"] == 0
        )
        capabilities.append(
            {
                "capabilityID": "missing_shard_review_legal_api_gate",
                "status": "proven_current" if review_gate_proven else "not_proven",
                "evidence": (
                    f"{counts['missingShardReviewGateDecisions']} gate decisions, "
                    f"{counts['missingShardReviewGateApprovedEvents'] or 0} approved for dry-run planning, "
                    f"{counts['missingShardReviewGateBlockedEvents'] or 0} blocked, "
                    f"{counts['missingShardReviewGatePlannedRegistryMutations'] or 0} planned dry-run mutations, "
                    f"{counts['missingShardReviewGateActiveRegistryMutations'] or 0} active mutations, "
                    f"{counts['missingShardReviewGateCoverageCounterMutations'] or 0} coverage counter mutations"
                ),
            }
        )
