...
IRS_SOURCE_ID = "irs.when_to_file"
CFPB_SOURCE_ID = "cfpb.adult_financial_education"
USAGOV_SOURCE_ID = "usa.gov.benefits"
SOURCE_IDS = [IRS_SOURCE_ID, CFPB_SOURCE_ID, USAGOV_SOURCE_ID]
DOMAIN = "finance_public_reference"


def test_finance_public_reference_fixture_adapters_are_governed_and_pack_candidate_ready(tmp_path: Path):
    result = _harvest_finance(tmp_path)
    ...
    irs_normalized = read_json(Path(result["runRoot"]) / "normalized" / f"{IRS_SOURCE_ID}.json")
    assert irs_normalized["sourceID"] == IRS_SOURCE_ID
    assert irs_normalized["adapterID"] == ADAPTER_IDS[IRS_SOURCE_ID]
    assert irs_normalized["termsValidation"]["packable"] is True
    assert irs_normalized["termsValidation"]["r2Ready"] is True
    assert {claim["claimType"] for claim in irs_normalized["claims"]} == {"tax_deadline_reference"}
    assert all(claim["reviewRequirement"] is False for claim in irs_normalized["claims"])
    assert irs_normalized["apiLanes"]["fixtureMode"] == "no_network_static_irs_tax_deadline_public_reference"

    cfpb_normalized = read_json(Path(result["runRoot"]) / "normalized" / f"{CFPB_SOURCE_ID}.json")
    assert cfpb_normalized["sourceID"] == CFPB_SOURCE_ID
    assert cfpb_normalized["adapterID"] == ADAPTER_IDS[CFPB_SOURCE_ID]
    assert cfpb_normalized["termsValidation"]["packable"] is True
    assert cfpb_normalized["termsValidation"]["r2Ready"] is True
    assert {claim["claimType"] for claim in cfpb_normalized["claims"]} == {"public_financial_education"}
    assert all(claim["reviewRequirement"] is False for claim in cfpb_normalized["claims"])
    assert cfpb_normalized["apiLanes"]["fixtureMode"] == "no_network_static_cfpb_financial_education_public_reference"
    ...
    assert {claim["claimType"] for claim in usagov_normalized["claims"]} == {"official_benefit_program_reference"}
    assert all(claim["reviewRequirement"] is False for claim in usagov_normalized["claims"])
    assert usagov_normalized["apiLanes"]["fixtureMode"] == "no_network_static_usagov_benefits_public_reference"


def test_finance_public_reference_frontier_reaches_claim_graph_ready_with_gold_set(tmp_path: Path):
    harvest = _harvest_finance(tmp_path)
    ...
    finance = next(report for report in result["frontierReports"] if report["frontier_id"] == DOMAIN)
    assert finance["status"] == "claim_graph_ready"
    assert finance["status_ceiling"] == "pack_staging_ready"
    assert finance["packable_claim_count"] == 3
    assert finance["blocked_claim_count"] == 0
    assert finance["missing_claim_classes"] == []
    assert finance["authority_coverage"]["complete"] is True
    assert finance["legal_posture_completeness"]["complete"] is True
    assert finance["provenance_completeness"]["complete"] is True
    assert finance["gold_set_status"] == "passed"
    assert finance["gold_set"]["goldSetID"] == "gold.finance_public_reference.irs_cfpb_usagov_public_reference.v1"
    assert finance["gold_set"]["requiredCount"] == 3
    assert finance["gold_set"]["matchedCount"] == 3
    assert finance["gold_set"]["missing"] == []
    ...
    assert "not as tax advice" in encoded
    assert "not as investment" in encoded
    assert "not a benefits eligibility guarantee" in encoded


def test_finance_public_reference_pack_production_dry_run_emits_public_r2_plan(tmp_path: Path):
    harvest = _harvest_finance(tmp_path)
    ...
    assert all("/finance_public_reference/" in key for key in result["objectKeys"].values())
    ...
    assert {row["license_id"] for row in attribution} == {"irs_public_web", "cfpb_public_web", "usagov_public_web"}
...
def test_finance_public_reference_adapters_output_no_private_or_final_user_artifacts(tmp_path: Path):
    result = _harvest_finance(tmp_path)
    encoded = ""
...
def test_finance_public_reference_adapters_are_registered_once():
    ...
    assert {adapter.adapter_id for adapter in matching} == {
        "irs_when_to_file_tax_deadline_adapter",
        "cfpb_adult_financial_education_adapter",
        "usagov_benefits_reference_adapter",
    }


def _harvest_finance(tmp_path: Path) -> dict[str, object]:
    return run_governed_harvest(
        GovernedHarvestOptions(
            output_root=tmp_path / "governed-harvest",
            run_id="finance-public-reference-fixture",
            mode="fixture",
