...
from foundry.catalog_direct_source_approval_chain import (
...
def test_catalog_direct_source_approval_chain_blocks_without_review_evidence(tmp_path: Path):
    candidate = _resolution_candidate("education")
    paths = _write_chain_inputs(tmp_path, candidate)

    result = run_catalog_direct_source_approval_chain(
        CatalogDirectSourceApprovalChainOptions(
            templates_path=paths["templates"],
            resolution_candidates_path=paths["resolution_candidates"],
            decision_inputs_path=paths["decision_inputs"],
            terms_proposals_path=paths["terms_proposals"],
            draft_governance_packets_path=paths["draft_packets"],
    ...
    assert result["status"] == "Source Green for catalog direct-source approval chain proof tooling"
    assert result["sourceAtlasStatusCeiling"] == "Yellow overall Source Atlas; direct-source approval chain proof tooling only"
    assert result["recordCounts"]["directSourceReviewTemplates"] == 1
    assert result["recordCounts"]["reviewEvidenceRecords"] == 0
    assert result["recordCounts"]["completedDirectSourceReviews"] == 0
    assert result["recordCounts"]["blockedDirectSourceReviews"] == 1
    ...
    assert _check(result, "missing_review_evidence_blocks_without_approval")
    assert _check(result, "chain_emits_no_claims_or_packs")


def test_catalog_direct_source_approval_chain_completed_fixture_reaches_candidate_registry_dry_run(tmp_path: Path):
    candidate = _resolution_candidate("education")
    paths = _write_chain_inputs(tmp_path, candidate)
    ...
    assert result["valid"], result["issues"]
    assert result["recordCounts"]["reviewEvidenceRecords"] == 1
    assert result["recordCounts"]["completedDirectSourceReviews"] == 1
    assert result["recordCounts"]["completedSourceReviewCompletionPackets"] == 1
...
def test_catalog_direct_source_approval_chain_rejects_private_review_evidence(tmp_path: Path):
    candidate = _resolution_candidate("education")
    paths = _write_chain_inputs(tmp_path, candidate)
    evidence = _completed_review_evidence(candidate)
    evidence["goal_text"] = "Use this for my private schedule"
    ...
    assert not _check(result, "direct_source_review_completion_valid")
    assert result["recordCounts"]["completedDirectSourceReviews"] == 0
    assert result["recordCounts"]["activeRegistryMutations"] == 0
    assert any("goal_text" in issue or "first_person_private_context" in issue for issue in result["issues"])


def _write_chain_inputs(tmp_path: Path, candidate: dict[str, object]) -> dict[str, Path]:
    ...
    return {
        "resolution_candidates": candidates_path,
        "templates": Path(templates["outputPaths"]["directSourceReviewPackets"]),
        "decision_inputs": _write_decision_inputs(tmp_path, [packet]),
        "terms_proposals": _write_terms_proposals(tmp_path, [_terms_proposal_for(packet)]),
        "draft_packets": _write_draft_packets(tmp_path, [_draft_packet_for(packet)]),
    }
...
def _write_resolution_candidates(tmp_path: Path, candidates: list[dict[str, object]]) -> Path:
    ...
    write_json(path, {"kind": "ambitions.sourceAtlas.catalogDirectSourceResolutionCandidates.v1", "createdAt": CREATED_AT, "resolutionCandidates": candidates})
    return path
...
def _write_review_evidence(tmp_path: Path, records: list[dict[str, object]]) -> Path:
    ...
    write_json(path, {"kind": "ambitions.sourceAtlas.catalogDirectSourceReviewEvidence.v1", "createdAt": CREATED_AT, "directSourceReviewEvidence": records})
    return path
...
def _resolution_candidate(suffix: str) -> dict[str, object]:
    return {
