...
def test_catalog_approval_decision_assembler_blocks_without_completion(tmp_path: Path):
    ...
    assert result["status"] == "Source Green for catalog approval decision assembler tooling"
    assert result["sourceAtlasStatusCeiling"] == "Yellow overall Source Atlas; decision assembler tooling only"
    ...
    assert result["recordCounts"]["blockedDecisionAssemblies"] == 2
    ...
    assert _check(result, "decision_assembler_emits_no_claims")

    finalizer_decision = read_json(Path(result["outputRoot"]) / "catalog-approval-finalizer-decision.json")
    assert finalizer_decision["kind"] == "ambitions.sourceAtlas.catalogApprovalFinalizerDecisionUnavailable.v1"
    assert read_json(Path(result["outputRoot"]) / "completed-decision-artifacts.json")["completedDecisionArtifacts"] == []


def test_catalog_approval_decision_assembler_valid_completion_emits_finalizer_compatible_decision(tmp_path: Path):
    ...
    assert result["recordCounts"]["blockedDecisionAssemblies"] == 0
    assert _check(result, "completed_decision_artifacts_match_finalizer_contract")

    decision = read_json(Path(result["outputRoot"]) / "catalog-approval-finalizer-decision.json")
    assert decision["kind"] == "ambitions.sourceAtlas.catalogApprovalFinalizerDecision.v1"
    assert decision["decision_status"] == "approved"
    assert decision["selected_proposal_ids"] == [packet["proposal_id"]]
    assert decision["approved_entries"][0]["intake_id"] == packet["intake_id"]

    proposals_path = _write_terms_proposals(tmp_path, [_terms_proposal_for(packet)])
    finalizer = compile_catalog_approval_finalizer(
        CatalogApprovalFinalizerOptions(
            terms_proposals_path=proposals_path,
            output_root=tmp_path / "finalizer",
            decision_artifact=Path(result["outputRoot"]) / "catalog-approval-finalizer-decision.json",
            created_at=CREATED_AT,
        )
    )
    assert finalizer["valid"], finalizer["issues"]
    assert finalizer["recordCounts"]["completedApprovalArtifacts"] == 1
    assert finalizer["recordCounts"]["approvedEntries"] == 1


def test_catalog_approval_decision_assembler_rejects_incomplete_completion(tmp_path: Path):
    packet = _decision_input_packet("education")
    inputs_path = _write_decision_inputs(tmp_path, [packet])
    completion = _completion_for(packet)
    del completion["completed_entries"][0]["legal_terms_entry"]["source_specific_restrictions"]
    completion_path = tmp_path / "completion.json"
    ...
    assert not _check(result, "provided_completion_valid")
    assert any("legal_terms_entry.source_specific_restrictions required" in issue for issue in result["issues"])
    finalizer_decision = read_json(Path(result["outputRoot"]) / "catalog-approval-finalizer-decision.json")
    assert finalizer_decision["kind"] == "ambitions.sourceAtlas.catalogApprovalFinalizerDecisionUnavailable.v1"


def test_catalog_approval_decision_assembler_rejects_outside_legal_claim_without_artifact(tmp_path: Path):
    packet = _decision_input_packet("education")
    inputs_path = _write_decision_inputs(tmp_path, [packet])
    completion = _completion_for(packet)
    completion["outside_legal_status"] = "approved"
    completion["outside_legal_approval_artifact"] = ""
    completion_path = tmp_path / "completion.json"
    write_json(completion_path, completion)
...
def test_catalog_approval_decision_assembler_rejects_private_completion(tmp_path: Path):
    packet = _decision_input_packet("education")
    inputs_path = _write_decision_inputs(tmp_path, [packet])
    completion = _completion_for(packet)
    ...
    assert not result["valid"]
    assert not _check(result, "completion_privacy_scan_passed")
    assert any("goal_text" in issue or "first_person_private_context" in issue for issue in result["issues"])


def test_catalog_approval_decision_assembler_stable_blocked_ordering(tmp_path: Path):
    ...
    first = compile_catalog_approval_decision_assembler(
        CatalogApprovalDecisionAssemblerOptions(decision_inputs_path=inputs_path, output_root=tmp_path / "first", created_at=CREATED_AT)
    )
    second = compile_catalog_approval_decision_assembler(
        CatalogApprovalDecisionAssemblerOptions(decision_inputs_path=inputs_path, output_root=tmp_path / "second", created_at=CREATED_AT)
    )

    first_blocked = read_json(Path(first["outputRoot"]) / "blocked-decision-assemblies.json")["blockedDecisionAssemblies"]
    second_blocked = read_json(Path(second["outputRoot"]) / "blocked-decision-assemblies.json")["blockedDecisionAssemblies"]
    assert first_blocked == second_blocked
    assert first_blocked == sorted(first_blocked, key=lambda item: (item["domain_guess"], item["intake_id"], item["proposal_id"]))
