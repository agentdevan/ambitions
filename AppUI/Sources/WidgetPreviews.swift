...
public extension DailyTargetsWidgetViewModel {
    ...
    static let empty = Self(snapshot: .init(metadata: WidgetFixtures.metadata(family: .dailyTargets), state: .empty(.init(title: "No targets yet", message: "Plan output can appear here once the day is assembled.", icon: "target", actionTitle: "Adjust plan"))))
    static let ready = Self(snapshot: .init(metadata: WidgetFixtures.metadata(family: .dailyTargets), state: .ready(.init(title: "Daily targets", subtitle: "Three useful moves for today.", completionLabel: "2 of 3", targets: [.init(id: "1", title: "Ship widget framework", detail: "Architecture and previews", progress: 0.7, trailingValue: "70%", statusLabel: "Focus", state: .selected), .init(id: "2", title: "Review trend summary", detail: "Spot weak consistency pockets", progress: 0.45, trailingValue: "45%", statusLabel: "Next"), .init(id: "3", title: "Log routine evidence", detail: "Evening check-in", progress: 0.2, trailingValue: "20%", statusLabel: "Later")], actions: WidgetFixtures.defaultActions))))
...
public extension GoalsListWidgetViewModel {
    ...
    static let empty = Self(snapshot: .init(metadata: WidgetFixtures.metadata(family: .goalsList), state: .empty(.init(title: "No active goals", message: "This stays empty until intake and plan data are connected.", icon: "target", actionTitle: "Adjust intake"))))
    static let ready = Self(snapshot: .init(metadata: WidgetFixtures.metadata(family: .goalsList), state: .ready(.init(title: "Goal portfolio", subtitle: "Ordered by what needs attention.", goals: [.init(id: "g1", title: "Launch premium Today screen", subtitle: "UI architecture and interaction polish", progressLabel: "68%", statusLabel: "On track"), .init(id: "g2", title: "Stabilize goal setup", subtitle: "Improve the handoff from plan to action", progressLabel: "42%", statusLabel: "Watching"), .init(id: "g3", title: "Reduce setup friction", subtitle: "You and settings cleanup", progressLabel: "31%", statusLabel: "Queued")], actions: [.init(kind: .refinePlan, title: "Adjust", icon: "slider.horizontal.3"), .init(kind: .expand, title: "Expand", icon: "arrow.up.left.and.arrow.down.right")]))))
...
public extension MilestonePromptWidgetViewModel {
    ...
    static let ready = Self(snapshot: .init(metadata: WidgetFixtures.metadata(family: .milestonePrompt), state: .ready(.init(title: "Milestone prompt", subtitle: "Reflect before the next planning pass.", prompt: "You completed two deep-work sessions faster than expected. Capture what made the cadence sustainable.", confidenceLabel: "Looks useful", actions: [.init(kind: .markHelpful, title: "Helpful", icon: "hand.thumbsup.fill", state: .success), .init(kind: .dismiss, title: "Dismiss", icon: "xmark.circle"), .init(kind: .openDetail, title: "Open detail", icon: "arrow.right.circle")]))))
...
public extension RecentActivityWidgetViewModel {
    ...
    static let ready = Self(snapshot: .init(metadata: WidgetFixtures.metadata(family: .recentActivity), state: .ready(.init(title: "Recent activity", subtitle: "Latest meaningful signals, not raw event spam.", activities: [.init(id: "a1", title: "Completed widget contract draft", subtitle: "Today", timestamp: "9:14 AM", icon: "checkmark.circle.fill", badge: "Done"), .init(id: "a2", title: "Skipped oversized focus step", subtitle: "Adjusted", timestamp: "Yesterday", icon: "forward.fill", badge: "Adjusted"), .init(id: "a3", title: "Logged evening reflection", subtitle: "Routine", timestamp: "Yesterday", icon: "book.closed.fill", badge: nil)], actions: [.init(kind: .quickLog, title: "Log", icon: "plus.circle"), .init(kind: .expand, title: "Expand", icon: "rectangle.expand.vertical")]))))
}
