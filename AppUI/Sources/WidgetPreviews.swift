...
public extension RitualSummaryWidgetViewModel {
    static let loading = Self(snapshot: .init(metadata: WidgetFixtures.metadata(family: .ritualSummary), state: .loading))
    static let empty = Self(snapshot: .init(metadata: WidgetFixtures.metadata(family: .ritualSummary), state: .empty(.init(title: "No ritual snapshot", message: "Use preview fixtures until logging and persistence settle down.", icon: "repeat", actionTitle: "Quick log"))))
    static let ready = Self(snapshot: .init(metadata: WidgetFixtures.metadata(family: .ritualSummary), state: .ready(.init(title: "Ritual summary", subtitle: "Daily routines with enough structure to stay glanceable.", stats: [.init(id: "r1", title: "Consistency", value: "84%", detail: "Last 14 days", icon: "waveform.path.ecg", state: .success), .init(id: "r2", title: "Sessions", value: "18", detail: "This week", icon: "calendar.badge.clock", state: .selected)], rituals: [.init(id: "ritual1", title: "Morning review", detail: "10-minute alignment", progress: 0.9, trailingValue: "9/10", statusLabel: "Strong", state: .success), .init(id: "ritual2", title: "Evening log", detail: "Close the day with evidence", progress: 0.62, trailingValue: "5/8", statusLabel: "Recover"), .init(id: "ritual3", title: "Movement reset", detail: "Mobility between work blocks", progress: 0.5, trailingValue: "4/8", statusLabel: "Watch", state: .warning)], actions: [.init(kind: .quickLog, title: "Log", icon: "plus.circle.fill", state: .success), .init(kind: .openDetail, title: "Inspect", icon: "chart.bar")]))))
}

public extension RitualRhythmWidgetViewModel {
    static let loading = Self(snapshot: .init(metadata: WidgetFixtures.metadata(family: .ritualRhythm, chrome: .widgetCard), state: .loading))
    static let empty = Self(snapshot: .init(metadata: WidgetFixtures.metadata(family: .ritualRhythm, chrome: .widgetCard), state: .empty(.init(title: "No rhythm yet", message: "This stays encouraging, not punitive, while Ambitions waits for enough evidence.", icon: "waveform.path.ecg", actionTitle: "Quick log"))))
    static let ready = Self(snapshot: .init(metadata: WidgetFixtures.metadata(family: .ritualRhythm, chrome: .widgetCard), state: .ready(.init(title: "7 steady days", subtitle: "Cadence stayed stable despite schedule movement.", stats: [.init(id: "rhythm1", title: "Current rhythm", value: "7", detail: "days", icon: "waveform.path.ecg", state: .celebration), .init(id: "rhythm2", title: "Best rhythm", value: "11", detail: "days", icon: "trophy.fill", state: .success)], recoveryNote: "A lighter step tomorrow keeps momentum better than an all-or-nothing push.", actions: [.init(kind: .markHelpful, title: "Helpful", icon: "heart.fill", state: .celebration), .init(kind: .askForSmallerStep, title: "Smaller step", icon: "arrow.down.circle")]))))
}
...
public extension CelebrationWidgetViewModel {
    ...
    static let ready = Self(snapshot: .init(metadata: WidgetFixtures.metadata(family: .celebration, variant: .expanded, priority: .hero, chrome: .heroCard), state: .ready(.init(title: "Momentum is building", subtitle: "You closed the top focus step and kept the next one clear before the day drifted.", achievements: ["Finished the architecture layer", "Kept the daily rhythm", "Captured why the focus card worked"], actions: [.init(kind: .markHelpful, title: "Helpful", icon: "hand.thumbsup.fill", state: .celebration), .init(kind: .dismiss, title: "Dismiss", icon: "xmark.circle")]))))
}
    ...
    var body: some View {
        ScrollView {
            WidgetFeed(items: [
                ...
                WidgetFeedItem(id: "ritual-ready", priority: .standard, variant: .medium) { RitualSummaryWidget(viewModel: .ready) },
                WidgetFeedItem(id: "rhythm-ready", priority: .standard, variant: .compact) { RitualRhythmWidget(viewModel: .ready) },
