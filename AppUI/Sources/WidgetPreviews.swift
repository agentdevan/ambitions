...
public extension MilestonePromptWidgetViewModel {
    ...
    static let ready = Self(snapshot: .init(metadata: WidgetFixtures.metadata(family: .milestonePrompt), state: .ready(.init(title: "Milestone prompt", subtitle: "Reflect before the next planning pass.", prompt: "You completed two deep-work sessions faster than expected. Capture what made the cadence sustainable.", confidenceLabel: "Strong signal", actions: [.init(kind: .markHelpful, title: "Helpful", icon: "hand.thumbsup.fill", state: .success), .init(kind: .dismiss, title: "Dismiss", icon: "xmark.circle"), .init(kind: .openDetail, title: "Open detail", icon: "arrow.right.circle")]))))
