...
public extension DailyTargetsWidgetViewModel {
    ...
    static let ready = Self(snapshot: .init(metadata: WidgetFixtures.metadata(family: .dailyTargets), state: .ready(.init(title: "Daily targets", subtitle: "Three high-confidence moves for today.", completionLabel: "2 of 3", targets: [.init(id: "1", title: "Ship widget framework", detail: "Architecture and previews", progress: 0.7, trailingValue: "70%", statusLabel: "Focus", state: .selected), .init(id: "2", title: "Review trend summary", detail: "Spot weak consistency pockets", progress: 0.45, trailingValue: "45%", statusLabel: "Next"), .init(id: "3", title: "Log habit evidence", detail: "Evening check-in", progress: 0.2, trailingValue: "20%", statusLabel: "Later")], actions: WidgetFixtures.defaultActions))))
...
public extension FocusNowWidgetViewModel {
    ...
    static let empty = Self(snapshot: .init(metadata: WidgetFixtures.metadata(family: .focusNow, priority: .hero, chrome: .heroCard), state: .empty(.init(title: "No focus card", message: "Once planner confidence is high enough, the top block can collapse into one primary action.", icon: "scope", actionTitle: "Shrink the step"))))
    static let ready = Self(snapshot: .init(metadata: WidgetFixtures.metadata(family: .focusNow, priority: .hero, chrome: .heroCard), state: .ready(.init(headline: "Draft the Today feed contracts", subtitle: "Best next move for the current session.", reason: "This unblocks every downstream widget family and keeps visual logic out of business orchestration.", duration: "35 min", energyLabel: "Confidence", progress: 0.82, supportSteps: ["Define shared widget action payloads", "Reuse WidgetCard and HeroCard surfaces", "Add preview fixtures before persistence"], actions: [.init(kind: .complete, title: "Mark done", icon: "checkmark.circle.fill", state: .success), .init(kind: .askWhyThisMatters, title: "Why it matters", icon: "questionmark.circle"), .init(kind: .askForSmallerStep, title: "Smaller step", icon: "arrow.down.circle")]))))
...
public extension FreeTimeWidgetViewModel {
    ...
    static let empty = Self(snapshot: .init(metadata: WidgetFixtures.metadata(family: .freeTime, chrome: .widgetCard), state: .empty(.init(title: "No free-time suggestion", message: "Leave this calm and optional until scheduling signals are trustworthy.", icon: "clock.badge.questionmark", actionTitle: "Open day"))))
    static let ready = Self(snapshot: .init(metadata: WidgetFixtures.metadata(family: .freeTime, chrome: .widgetCard), state: .ready(.init(title: "Free time", subtitle: "A small opportunistic window opened after lunch.", availableWindow: "26 min", suggestionTitle: "Outline one smaller step", suggestionDetail: "Use the gap for a low-friction planning pass instead of deep work.", actions: [.init(kind: .quickLog, title: "Quick log", icon: "plus.circle"), .init(kind: .skip, title: "Skip", icon: "forward.fill", state: .warning)]))))
}
...
public extension GoalsListWidgetViewModel {
    ...
    static let ready = Self(snapshot: .init(metadata: WidgetFixtures.metadata(family: .goalsList), state: .ready(.init(title: "Goal portfolio", subtitle: "Ordered for attention, not permanence.", goals: [.init(id: "g1", title: "Launch premium Today screen", subtitle: "UI architecture and interaction polish", progressLabel: "68%", statusLabel: "On track"), .init(id: "g2", title: "Stabilize goal orchestration", subtitle: "Improve planner handoff confidence", progressLabel: "42%", statusLabel: "Watching"), .init(id: "g3", title: "Reduce setup friction", subtitle: "Profile and settings cleanup", progressLabel: "31%", statusLabel: "Queued")], actions: [.init(kind: .refinePlan, title: "Refine", icon: "slider.horizontal.3"), .init(kind: .expand, title: "Expand", icon: "arrow.up.left.and.arrow.down.right")]))))
...
public extension MilestonePromptWidgetViewModel {
    ...
    static let empty = Self(snapshot: .init(metadata: WidgetFixtures.metadata(family: .milestonePrompt), state: .empty(.init(title: "No milestone prompt", message: "Keep this optional until adaptation is confident enough to surface a prompt.", icon: "flag.checkered", actionTitle: "View plan"))))
    static let ready = Self(snapshot: .init(metadata: WidgetFixtures.metadata(family: .milestonePrompt), state: .ready(.init(title: "Milestone prompt", subtitle: "Reflect before the next planning pass.", prompt: "You completed two deep-work sessions faster than expected. Capture what made the cadence sustainable.", confidenceLabel: "High confidence", actions: [.init(kind: .markHelpful, title: "Helpful", icon: "hand.thumbsup.fill", state: .success), .init(kind: .dismiss, title: "Dismiss", icon: "xmark.circle"), .init(kind: .openDetail, title: "Open detail", icon: "arrow.right.circle")]))))
...
public extension InsightStatsWidgetViewModel {
    ...
    static let empty = Self(snapshot: .init(metadata: WidgetFixtures.metadata(family: .insightStats), state: .empty(.init(title: "No insight stats", message: "Wait for enough history before implying precision.", icon: "chart.xyaxis.line", actionTitle: "Open insights"))))
    static let ready = Self(snapshot: .init(metadata: WidgetFixtures.metadata(family: .insightStats), state: .ready(.init(title: "Insight stats", subtitle: "Useful rollups for the current review window.", stats: [.init(id: "i1", title: "Focus depth", value: "73%", detail: "weighted quality", icon: "brain.head.profile", state: .selected), .init(id: "i2", title: "Completion", value: "81%", detail: "active steps", icon: "checkmark.circle.fill", state: .success), .init(id: "i3", title: "Drift", value: "14%", detail: "unplanned pivots", icon: "arrow.triangle.branch", state: .warning), .init(id: "i4", title: "Helpfulness", value: "4.7", detail: "prompt rating", icon: "hand.thumbsup.fill", state: .celebration)], summary: "Recent adaptation suggestions were most effective when the step size was under 35 minutes.", actions: [.init(kind: .markHelpful, title: "Helpful", icon: "hand.thumbsup.fill", state: .success), .init(kind: .openDetail, title: "Open", icon: "arrow.right.circle")]))))
}
...
public extension ProfileSummaryWidgetViewModel {
    ...
    static let empty = Self(snapshot: .init(metadata: WidgetFixtures.metadata(family: .profileSummary), state: .empty(.init(title: "No profile summary", message: "Keep this driven by demo fixtures until account and preference repositories are wired.", icon: "person.crop.circle", actionTitle: "Open profile"))))
    static let ready = Self(snapshot: .init(metadata: WidgetFixtures.metadata(family: .profileSummary), state: .ready(.init(title: "Preview User", subtitle: "Quiet systems, steady momentum.", initials: "PU", badges: ["On-device", "Adaptive"], stats: [.init(id: "p1", title: "Goals", value: "6", detail: "active", icon: "target"), .init(id: "p2", title: "Wins", value: "14", detail: "this month", icon: "sparkles", state: .celebration)], actions: [.init(kind: .openDetail, title: "Open profile", icon: "person.crop.circle"), .init(kind: .refinePlan, title: "Adjust defaults", icon: "slider.horizontal.3")]))))
}
...
public extension CelebrationWidgetViewModel {
    ...
    static let ready = Self(snapshot: .init(metadata: WidgetFixtures.metadata(family: .celebration, variant: .expanded, priority: .hero, chrome: .heroCard), state: .ready(.init(title: "Momentum is compounding", subtitle: "You closed the top focus step and protected the next one before context drift took over.", achievements: ["Finished the architecture layer", "Preserved the daily streak", "Captured why the focus card worked"], actions: [.init(kind: .markHelpful, title: "Helpful", icon: "hand.thumbsup.fill", state: .celebration), .init(kind: .dismiss, title: "Dismiss", icon: "xmark.circle")]))))
}
...
public extension SettingsGroupWidgetViewModel {
    ...
    static let ready = Self(snapshot: .init(metadata: WidgetFixtures.metadata(family: .settingsGroup), state: .ready(.init(title: "Settings", subtitle: "Tunable controls grouped in reusable card chrome.", items: [.init(id: "sg1", title: "Focus defaults", subtitle: "Default session length and recovery margins", icon: "timer", valueLabel: "35 min"), .init(id: "sg2", title: "Notifications", subtitle: "Timely nudges only", icon: "bell.badge", valueLabel: "Smart"), .init(id: "sg3", title: "Adaptation tone", subtitle: "How assertive the guidance feels", icon: "dial.medium", valueLabel: "Calm")], footer: "Keep screen assembly in the app target. Only the visual row shell belongs to the design system."))))
}
