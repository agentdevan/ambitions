...
public extension StreakWidgetViewModel {
    ...
    static let empty = Self(snapshot: .init(metadata: WidgetFixtures.metadata(family: .streak, chrome: .widgetCard), state: .empty(.init(title: "No rhythm yet", message: "This stays encouraging, not punitive, while Ambitions waits for enough evidence.", icon: "flame", actionTitle: "Quick log"))))
    static let ready = Self(snapshot: .init(metadata: WidgetFixtures.metadata(family: .streak, chrome: .widgetCard), state: .ready(.init(title: "7 steady days", subtitle: "Cadence stayed stable despite schedule movement.", stats: [.init(id: "s1", title: "Current rhythm", value: "7", detail: "days", icon: "flame.fill", state: .celebration), .init(id: "s2", title: "Best rhythm", value: "11", detail: "days", icon: "trophy.fill", state: .success)], recoveryNote: "A lighter step tomorrow keeps momentum better than an all-or-nothing push.", actions: [.init(kind: .markHelpful, title: "Helpful", icon: "heart.fill", state: .celebration), .init(kind: .askForSmallerStep, title: "Smaller step", icon: "arrow.down.circle")]))))
}
