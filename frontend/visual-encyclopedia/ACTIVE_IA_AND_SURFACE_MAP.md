# Active IA And Surface Map

Status: Active frontend canon

Active top-level IA is exactly `Today / Goals / Capture / Time / You`.

| Destination | Primary Object | User Job | Source Mapping |
|---|---|---|---|
...
`Plan` is not a top-level destination. Existing `plan` code paths are internal compatibility seams for the user-facing Time surface.
Do not recreate a `Plan` tab, `Tasks` tab, or other duplicate top-level destination in frontend canon.
