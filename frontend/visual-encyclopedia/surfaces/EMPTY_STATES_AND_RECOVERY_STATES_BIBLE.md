...
This surface contains empty, no-data, source-unavailable, stale-source, overload, blocked, waiting, Still Counts, recovery, unavailable, error fallback. The top-level visual hierarchy is `Empty and Recovery States` first, one primary next action second, source/trust/proof affordances third, and secondary metadata last.

## Primary Object

- Destination: Cross-surface
- Primary object: Empty and Recovery States
- User job: Resilience
...
Object-local help; source detail; closure/recovery sheets; receipt details.
