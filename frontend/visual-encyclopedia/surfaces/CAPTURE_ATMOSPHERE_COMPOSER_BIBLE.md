...
This surface contains idle state, composer field, add/proof attachment, mic/dictation, keyboard behavior, post-input route reveal, local parse state, capture receipt, hold / needs a place, route rows. The top-level visual hierarchy is `Atmosphere Composer` first, one primary next action second, source/trust/proof affordances third, and secondary metadata last.
...
Save as Proof; Make Commitment; Grow into Goal; Mark Constraint; Reflect; Hold / Needs a Place; quick proof attachment; quick step capture; source upload; error/unavailable states.
