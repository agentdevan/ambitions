...
- Typography: Use native iPhone semantic text hierarchy: compact region label, Reflow Preview Tray title or state label, source/proof caption, and readable action text. Emphasis stays on the active Reflow Preview Tray decision or inspected state.
- Spacing: Keep source, proof, CTA, and state notes visually attached to Reflow Preview Tray, with tight native grouping, thumb-zone reach for actions, and enough separation to prevent equal-weight card stacking.
...
- CTAs: Use a visible primary command only when this region changes Reflow Preview Tray; otherwise prefer named disclosure rows such as Why this?, View source, View receipt, Add proof, Undo, or Close.
...
- Contains: Source freshness, proof basis, receipt expectation, unresolved-direction note, or Why this? affordance for Reflow Preview Tray.
...
## Object Inventory
Reflow Preview Tray, plus any visible Receipt System, Closure System, Recommendation Source System, Proof Trail System, Commitment Staging Tray, Reflow Preview Tray, Personal Runtime, or source/proof objects referenced by this surface.

## Typography Recipe
Native iPhone semantic typography. The Reflow Preview Tray title or active decision owns the strongest weight; explanatory source/proof text remains compact but readable. Dynamic Type must preserve Reflow Preview Tray, the source/proof line, the primary command, and the recovery or cancel path.
