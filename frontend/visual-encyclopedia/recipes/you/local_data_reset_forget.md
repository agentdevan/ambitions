# Surface Recipe: Local Data / Reset / Forget

## Canon Status
intended_canon

## Surface ID
local_data_reset_forget
...
- Purpose: Show how Settings-style section body changes the visible hierarchy, source/proof meaning, and available action for Local Data / Reset / Forget.
...
- Purpose: Show how Trust/local runtime explanation changes the visible hierarchy, source/proof meaning, and available action for Local Data / Reset / Forget.
- Contains: User System Profile; visible ingredients include User System Profile setting or runtime object, local data boundary, privacy/control state, receipt/history link, export/reset disclosure, and source or proof context. Supporting objects: Receipt System, Source Freshness Badge, Why This Sheet, Closure System. Region context: orientation, primary object, source/proof line.
- Primitives: GraphiteRecess ground, LuminousTrace attachment, CelestialField only for semantic orientation, SF Symbols, chevrons, label system.
- Typography: Use native iPhone semantic text hierarchy: compact region label, User System Profile title or state label, source/proof caption, and readable action text. Emphasis stays on the active User System Profile decision or inspected state.
- Spacing: Keep source, proof, CTA, and state notes visually attached to User System Profile, with tight native grouping, thumb-zone reach for actions, and enough separation to prevent equal-weight card stacking.
...
- Contains: Primary and secondary commands for Local Data / Reset / Forget; disabled/destructive states when applicable; cancel and undo where reversible.
...
- Purpose: Show how Warning/offline/first-run state changes the visible hierarchy, source/proof meaning, and available action for Local Data / Reset / Forget.
- Contains: User System Profile; visible ingredients include User System Profile setting or runtime object, local data boundary, privacy/control state, receipt/history link, export/reset disclosure, and source or proof context. Supporting objects: Receipt System, Source Freshness Badge, Why This Sheet, Closure System. Region context: orientation, primary object, source/proof line.
- Primitives: GraphiteRecess ground, LuminousTrace attachment, CelestialField only for semantic orientation, SF Symbols, chevrons, label system.
- Typography: Use native iPhone semantic text hierarchy: compact region label, User System Profile title or state label, source/proof caption, and readable action text. Emphasis stays on the active User System Profile decision or inspected state.
- Spacing: Keep source, proof, CTA, and state notes visually attached to User System Profile, with tight native grouping, thumb-zone reach for actions, and enough separation to prevent equal-weight card stacking.
...
This recipe is final-state design canon for Local Data / Reset / Forget in You. It does not prove implementation or release readiness.

### Good / Bad Example

- Good: Local Data / Reset / Forget stays attached to User System Profile, with trust, proof, receipt, and recovery visible.
- Bad: Local Data / Reset / Forget turns into a generic productivity pattern or hides its trust seam.
...
Local Data / Reset / Forget keeps its trust seam attached to User System Profile and You.
...
Local Data / Reset / Forget exposes local runtime behavior as inspectable state, not hidden automation.
...
Local Data / Reset / Forget previews local reset or forget consequences before the user commits to them.
...
## Purpose
Local Data / Reset / Forget exists to make You understandable through the User System Profile, not through a generic productivity screen.
Minimum specificity: the recipe must name local_data_reset_forget, the user job, and the reason this surface owns that job.

## Surface Hierarchy
Local Data / Reset / Forget sits inside You and is subordinate to the User System Profile; supporting surfaces must not outrank it.
...
Minimum specificity: if Local Data / Reset / Forget cannot show what User System Profile owns, the surface is not final-proof eligible.

## Label-Off Signature
With labels blurred, Local Data / Reset / Forget must still read as User System Profile through local runtime panel, trust controls, reset/forget preview, source/proof edge, and recovery return path.
...
## Visible Regions
Visible regions for Local Data / Reset / Forget: local runtime panel, trust controls, reset/forget preview, source/proof edge, and recovery return path.
...
Disclosure opens deeper source, proof, consequence, or correction detail for Local Data / Reset / Forget; it is never decorative navigation noise.
Minimum specificity: every chevron must have a visible label or accessibility meaning tied to User System Profile.

## Source / Trust Behavior
Source Link Status: intended_only. Local Data / Reset / Forget must show whether source is linked, intended-only, stale, unavailable, or local-only.
...
Minimum specificity: proof cannot be a detached feed, generic toast, or release claim for Local Data / Reset / Forget.

## Transaction Behavior
Transactions on Local Data / Reset / Forget require visible intent, preview when consequence matters, explicit commit, receipt, and undo or recovery.
...
## VoiceOver Order
VoiceOver Order: You, Local Data / Reset / Forget, User System Profile, state, source status, proof or receipt status, primary action, correction path.
...
## Reduce Motion Behavior
Reduce Motion replaces movement in Local Data / Reset / Forget with static before/after labels, source summaries, receipt state, and recovery labels.
...
## Native iPhone Believability Requirements
Local Data / Reset / Forget must feel like a restrained native iPhone surface with thumb-reachable actions, semantic rows, sheets, and stable safe areas.
...
Source-family influence for Local Data / Reset / Forget: visual canon, accessibility, source/proof receipts, local trust, transaction, and You object depth.
Minimum specificity: train references must explain behavior, not imply shipped SwiftUI implementation.

## Source Linkage
Source Link Status: intended_only. Implementation Proof Boundary: Not In Scope. Required gates: transaction, local_first_trust, accessibility.
...
## Implementation Proof Boundary
Implementation Proof Boundary: this recipe is final-state design canon for Local Data / Reset / Forget; it does not prove SwiftUI, device, release, or accessibility implementation.
Minimum specificity: proof requires source inspection, local validation, and release evidence outside this recipe.

## Unresolved Direction
Unresolved direction for Local Data / Reset / Forget: any missing implementation source remains intended_only debt and must stay visible until a scoped implementation batch lands.
...
Validator hint: fail Local Data / Reset / Forget if User System Profile can be replaced by a generic list, calendar, chat, settings, score, or dashboard surface.
...
Minimum specificity: Local Data / Reset / Forget may define design canon only and must not claim production readiness.

## Acceptance Checklist
Good example: one local-control object where privacy, runtime, reset, export, and correction stay inspectable.
Acceptance requires source linkage, proof boundary, VoiceOver/Dynamic Type/Reduce Motion coverage, primary action, correction path, and anti-generic checks for Local Data / Reset / Forget.

## P0 Proof Appendix
- Source Link Status: intended_only; Local Data / Reset / Forget must keep source status inspectable, including local-only and stale source states, and must not hide intended-only debt.
...
The proof appendix exists to prevent a false Green for Local Data / Reset / Forget. It records what can be claimed as design canon and what remains outside implementation proof.
...
Evidence note: Local Data / Reset / Forget must keep generated reports, source-link status, and Not In Scope implementation proof aligned before any Green claim.
If any report upgrades Local Data / Reset / Forget without this boundary, the validator must fail rather than convert intended canon into shipped behavior.

## P0 Canon Appendix
VoiceOver must announce You, Local Data / Reset / Forget, User System Profile, state, Source, Proof, Receipt, Primary Action, and recovery in that order.
...
This appendix is canon authority for Local Data / Reset / Forget, not implementation proof, release proof, or an approval to alter top-level IA.
