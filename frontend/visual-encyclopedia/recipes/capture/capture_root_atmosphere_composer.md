# Surface Recipe: Capture Root / Atmosphere Composer
...
## Final Intended Role
The Atmosphere Composer root. It keeps capture immediate and quiet, then reveals route options only after the user gives Ambitions something to place.

## User Perception
The user should feel they can put something down fast, then choose a clear route when Ambitions has enough context.

## Why This Surface Exists
It exists so the Capture root can show the composer, route reveal, source basis, proof path, and receipt path as one quiet intake surface.
...
- proof, attachment, and source line
- local-only and error state
...
This recipe is final-state design canon for Capture Root / Atmosphere Composer in Capture. It does not prove implementation or release readiness.

### Good / Bad Example

- Good: Capture Root / Atmosphere Composer stays attached to Atmosphere Composer, with trust, proof, receipt, and recovery visible.
- Bad: Capture Root / Atmosphere Composer turns into a generic productivity pattern or hides its trust seam.
...
Capture Root / Atmosphere Composer keeps its trust seam attached to Atmosphere Composer and Capture.
...
Capture Root / Atmosphere Composer exposes local runtime behavior as inspectable state, not hidden automation.
...
Capture Root / Atmosphere Composer previews local reset or forget consequences before the user commits to them.
...
## Purpose
Capture Root / Atmosphere Composer exists to make Capture understandable through the Atmosphere Composer, not through a generic productivity screen.
Minimum specificity: the recipe must name capture_root_atmosphere_composer, the user job, and the reason this surface owns that job.

## Surface Hierarchy
Capture Root / Atmosphere Composer sits inside Capture and is subordinate to the Atmosphere Composer; supporting surfaces must not outrank it.
...
Minimum specificity: if Capture Root / Atmosphere Composer cannot show what Atmosphere Composer owns, the surface is not final-proof eligible.

## Label-Off Signature
With labels blurred, Capture Root / Atmosphere Composer must still read as Atmosphere Composer through capture field, route reveal seam, needs-a-place holding state, source/proof edge, and recovery return path.
...
## Visible Regions
Visible regions for Capture Root / Atmosphere Composer: capture field, route reveal seam, needs-a-place holding state, source/proof edge, and recovery return path.
...
Disclosure opens deeper source, proof, consequence, or correction detail for Capture Root / Atmosphere Composer; it is never decorative navigation noise.
Minimum specificity: every chevron must have a visible label or accessibility meaning tied to Atmosphere Composer.

## Source / Trust Behavior
Source Link Status: linked. Capture Root / Atmosphere Composer must show whether source is linked, intended-only, stale, unavailable, or local-only.
...
Minimum specificity: proof cannot be a detached feed, generic toast, or release claim for Capture Root / Atmosphere Composer.

## Transaction Behavior
Transactions on Capture Root / Atmosphere Composer require visible intent, preview when consequence matters, explicit commit, receipt, and undo or recovery.
...
## VoiceOver Order
VoiceOver Order: Capture, Capture Root / Atmosphere Composer, Atmosphere Composer, state, source status, proof or receipt status, primary action, correction path.
...
## Reduce Motion Behavior
Reduce Motion replaces movement in Capture Root / Atmosphere Composer with static before/after labels, source summaries, receipt state, and recovery labels.
...
## Native iPhone Believability Requirements
Capture Root / Atmosphere Composer must feel like a restrained native iPhone surface with thumb-reachable actions, semantic rows, sheets, and stable safe areas.
...
Source-family influence for Capture Root / Atmosphere Composer: visual canon, accessibility, source/proof receipts, local trust, transaction, and Capture object depth.
...
## Implementation Proof Boundary
Implementation Proof Boundary: this recipe is final-state design canon for Capture Root / Atmosphere Composer; it does not prove SwiftUI, device, release, or accessibility implementation.
Minimum specificity: proof requires source inspection, local validation, and release evidence outside this recipe.

## Unresolved Direction
Unresolved direction for Capture Root / Atmosphere Composer: any missing implementation source remains linked debt and must stay visible until a scoped implementation batch lands.
...
Validator hint: fail Capture Root / Atmosphere Composer if Atmosphere Composer can be replaced by a generic list, calendar, chat, settings, score, or dashboard surface.
...
Minimum specificity: Capture Root / Atmosphere Composer may define design canon only and must not claim production readiness.

## Acceptance Checklist
Good example: one capture-first object that preserves the thought, shows routes, and holds uncertainty safely.
Acceptance requires source linkage, proof boundary, VoiceOver/Dynamic Type/Reduce Motion coverage, primary action, correction path, and anti-generic checks for Capture Root / Atmosphere Composer.

## P0 Proof Appendix
- Source Link Status: linked; Capture Root / Atmosphere Composer must keep source status inspectable, including local-only and stale source states, and must not hide intended-only debt.
...
The proof appendix exists to prevent a false Green for Capture Root / Atmosphere Composer. It records what can be claimed as design canon and what remains outside implementation proof.
...
Evidence note: Capture Root / Atmosphere Composer must keep generated reports, source-link status, and Not In Scope implementation proof aligned before any Green claim.
If any report upgrades Capture Root / Atmosphere Composer without this boundary, the validator must fail rather than convert intended canon into shipped behavior.

## P0 Canon Appendix
VoiceOver must announce Capture, Capture Root / Atmosphere Composer, Atmosphere Composer, state, Source, Proof, Receipt, Primary Action, and recovery in that order.
...
This appendix is canon authority for Capture Root / Atmosphere Composer, not implementation proof, release proof, or an approval to alter top-level IA.
