# Surface Recipe: Capture Receipt
...
- Purpose: Show how Composer atmosphere field changes the visible hierarchy, source/proof meaning, and available action for Capture Receipt.
...
- Purpose: Show how Input affordance changes the visible hierarchy, source/proof meaning, and available action for Capture Receipt.
- Contains: Atmosphere Composer; visible ingredients include captured text or attachment, routing destination, needs-place state, source/proof attachment, local-only note, receipt expectation, and correction or undo path. Supporting objects: Receipt System, Source Freshness Badge, Why This Sheet, Closure System. Region context: orientation, primary object, source/proof line.
- Primitives: GraphiteRecess ground, LuminousTrace attachment, CelestialField only for semantic orientation, SF Symbols, chevrons, label system.
- Typography: Use native iPhone semantic text hierarchy: compact region label, Atmosphere Composer title or state label, source/proof caption, and readable action text. Emphasis stays on the active Atmosphere Composer decision or inspected state.
- Spacing: Keep source, proof, CTA, and state notes visually attached to Atmosphere Composer, with tight native grouping, thumb-zone reach for actions, and enough separation to prevent equal-weight card stacking.
...
- Purpose: Show how Route reveal or placement options changes the visible hierarchy, source/proof meaning, and available action for Capture Receipt.
- Contains: Atmosphere Composer; visible ingredients include captured text or attachment, routing destination, needs-place state, source/proof attachment, local-only note, receipt expectation, and correction or undo path. Supporting objects: Receipt System, Source Freshness Badge, Why This Sheet, Closure System. Region context: orientation, primary object, source/proof line.
- Primitives: GraphiteRecess ground, LuminousTrace attachment, CelestialField only for semantic orientation, SF Symbols, chevrons, label system.
- Typography: Use native iPhone semantic text hierarchy: compact region label, Atmosphere Composer title or state label, source/proof caption, and readable action text. Emphasis stays on the active Atmosphere Composer decision or inspected state.
- Spacing: Keep source, proof, CTA, and state notes visually attached to Atmosphere Composer, with tight native grouping, thumb-zone reach for actions, and enough separation to prevent equal-weight card stacking.
...
- Purpose: Show how Local-only/error state changes the visible hierarchy, source/proof meaning, and available action for Capture Receipt.
- Contains: Atmosphere Composer; visible ingredients include captured text or attachment, routing destination, needs-place state, source/proof attachment, local-only note, receipt expectation, and correction or undo path. Supporting objects: Receipt System, Source Freshness Badge, Why This Sheet, Closure System. Region context: orientation, primary object, source/proof line.
- Primitives: GraphiteRecess ground, LuminousTrace attachment, CelestialField only for semantic orientation, SF Symbols, chevrons, label system.
- Typography: Use native iPhone semantic text hierarchy: compact region label, Atmosphere Composer title or state label, source/proof caption, and readable action text. Emphasis stays on the active Atmosphere Composer decision or inspected state.
- Spacing: Keep source, proof, CTA, and state notes visually attached to Atmosphere Composer, with tight native grouping, thumb-zone reach for actions, and enough separation to prevent equal-weight card stacking.
...
This recipe is final-state design canon for Capture Receipt in Capture. It does not prove implementation or release readiness.

### Good / Bad Example

- Good: Capture Receipt stays attached to Atmosphere Composer, with trust, proof, receipt, and recovery visible.
- Bad: Capture Receipt turns into a generic productivity pattern or hides its trust seam.
...
Capture Receipt keeps its trust seam attached to Atmosphere Composer and Capture.
...
Capture Receipt exposes local runtime behavior as inspectable state, not hidden automation.
...
Capture Receipt previews local reset or forget consequences before the user commits to them.
...
## Purpose
Capture Receipt exists to make Capture understandable through the Atmosphere Composer, not through a generic productivity screen.
Minimum specificity: the recipe must name capture_receipt, the user job, and the reason this surface owns that job.

## Surface Hierarchy
Capture Receipt sits inside Capture and is subordinate to the Atmosphere Composer; supporting surfaces must not outrank it.
...
Minimum specificity: if Capture Receipt cannot show what Atmosphere Composer owns, the surface is not final-proof eligible.

## Label-Off Signature
With labels blurred, Capture Receipt must still read as Atmosphere Composer through capture field, route reveal seam, needs-a-place holding state, source/proof edge, and recovery return path.
...
## Visible Regions
Visible regions for Capture Receipt: capture field, route reveal seam, needs-a-place holding state, source/proof edge, and recovery return path.
...
Disclosure opens deeper source, proof, consequence, or correction detail for Capture Receipt; it is never decorative navigation noise.
Minimum specificity: every chevron must have a visible label or accessibility meaning tied to Atmosphere Composer.

## Source / Trust Behavior
Source Link Status: intended_only. Capture Receipt must show whether source is linked, intended-only, stale, unavailable, or local-only.
...
Minimum specificity: proof cannot be a detached feed, generic toast, or release claim for Capture Receipt.

## Transaction Behavior
Transactions on Capture Receipt require visible intent, preview when consequence matters, explicit commit, receipt, and undo or recovery.
...
## VoiceOver Order
VoiceOver Order: Capture, Capture Receipt, Atmosphere Composer, state, source status, proof or receipt status, primary action, correction path.
...
## Reduce Motion Behavior
Reduce Motion replaces movement in Capture Receipt with static before/after labels, source summaries, receipt state, and recovery labels.
...
## Native iPhone Believability Requirements
Capture Receipt must feel like a restrained native iPhone surface with thumb-reachable actions, semantic rows, sheets, and stable safe areas.
...
Source-family influence for Capture Receipt: visual canon, accessibility, source/proof receipts, local trust, transaction, and Capture object depth.
...
## Implementation Proof Boundary
Implementation Proof Boundary: this recipe is final-state design canon for Capture Receipt; it does not prove SwiftUI, device, release, or accessibility implementation.
Minimum specificity: proof requires source inspection, local validation, and release evidence outside this recipe.

## Unresolved Direction
Unresolved direction for Capture Receipt: any missing implementation source remains intended_only debt and must stay visible until a scoped implementation batch lands.
...
Validator hint: fail Capture Receipt if Atmosphere Composer can be replaced by a generic list, calendar, chat, settings, score, or dashboard surface.
...
Minimum specificity: Capture Receipt may define design canon only and must not claim production readiness.

## Acceptance Checklist
Good example: one capture-first object that preserves the thought, shows routes, and holds uncertainty safely.
Acceptance requires source linkage, proof boundary, VoiceOver/Dynamic Type/Reduce Motion coverage, primary action, correction path, and anti-generic checks for Capture Receipt.

## P0 Proof Appendix
- Source Link Status: intended_only; Capture Receipt must keep source status inspectable, including local-only and stale source states, and must not hide intended-only debt.
...
The proof appendix exists to prevent a false Green for Capture Receipt. It records what can be claimed as design canon and what remains outside implementation proof.
...
Evidence note: Capture Receipt must keep generated reports, source-link status, and Not In Scope implementation proof aligned before any Green claim.
If any report upgrades Capture Receipt without this boundary, the validator must fail rather than convert intended canon into shipped behavior.

## P0 Canon Appendix
VoiceOver must announce Capture, Capture Receipt, Atmosphere Composer, state, Source, Proof, Receipt, Primary Action, and recovery in that order.
...
This appendix is canon authority for Capture Receipt, not implementation proof, release proof, or an approval to alter top-level IA.
