# Surface Recipe: Capture Post-Input Route Reveal

## Canon Status
intended_canon

## Surface ID
capture_post_input_route_reveal
...
- Purpose: Show how Composer atmosphere field changes the visible hierarchy, source/proof meaning, and available action for Capture Post-Input Route Reveal.
...
- Purpose: Show how Input affordance changes the visible hierarchy, source/proof meaning, and available action for Capture Post-Input Route Reveal.
- Contains: Atmosphere Composer; visible ingredients include captured text or attachment, routing destination, needs-place state, source/proof attachment, local-only note, receipt expectation, and correction or undo path. Supporting objects: Receipt System, Source Freshness Badge, Why This Sheet, Closure System. Region context: orientation, primary object, source/proof line.
- Primitives: GraphiteRecess ground, LuminousTrace attachment, CelestialField only for semantic orientation, SF Symbols, chevrons, label system.
- Typography: Use native iPhone semantic text hierarchy: compact region label, Atmosphere Composer title or state label, source/proof caption, and readable action text. Emphasis stays on the active Atmosphere Composer decision or inspected state.
- Spacing: Keep source, proof, CTA, and state notes visually attached to Atmosphere Composer, with tight native grouping, thumb-zone reach for actions, and enough separation to prevent equal-weight card stacking.
...
- Purpose: Show how Route reveal or placement options changes the visible hierarchy, source/proof meaning, and available action for Capture Post-Input Route Reveal.
- Contains: Atmosphere Composer; visible ingredients include captured text or attachment, routing destination, needs-place state, source/proof attachment, local-only note, receipt expectation, and correction or undo path. Supporting objects: Receipt System, Source Freshness Badge, Why This Sheet, Closure System. Region context: orientation, primary object, source/proof line.
- Primitives: GraphiteRecess ground, LuminousTrace attachment, CelestialField only for semantic orientation, SF Symbols, chevrons, label system.
- Typography: Use native iPhone semantic text hierarchy: compact region label, Atmosphere Composer title or state label, source/proof caption, and readable action text. Emphasis stays on the active Atmosphere Composer decision or inspected state.
- Spacing: Keep source, proof, CTA, and state notes visually attached to Atmosphere Composer, with tight native grouping, thumb-zone reach for actions, and enough separation to prevent equal-weight card stacking.
...
- Purpose: Show how Local-only/error state changes the visible hierarchy, source/proof meaning, and available action for Capture Post-Input Route Reveal.
- Contains: Atmosphere Composer; visible ingredients include captured text or attachment, routing destination, needs-place state, source/proof attachment, local-only note, receipt expectation, and correction or undo path. Supporting objects: Receipt System, Source Freshness Badge, Why This Sheet, Closure System. Region context: orientation, primary object, source/proof line.
- Primitives: GraphiteRecess ground, LuminousTrace attachment, CelestialField only for semantic orientation, SF Symbols, chevrons, label system.
- Typography: Use native iPhone semantic text hierarchy: compact region label, Atmosphere Composer title or state label, source/proof caption, and readable action text. Emphasis stays on the active Atmosphere Composer decision or inspected state.
- Spacing: Keep source, proof, CTA, and state notes visually attached to Atmosphere Composer, with tight native grouping, thumb-zone reach for actions, and enough separation to prevent equal-weight card stacking.
...
This recipe is final-state design canon for Capture Post-Input Route Reveal in Capture. It does not prove implementation or release readiness.

### Good / Bad Example

- Good: Capture Post-Input Route Reveal stays attached to Atmosphere Composer, with trust, proof, receipt, and recovery visible.
- Bad: Capture Post-Input Route Reveal turns into a generic productivity pattern or hides its trust seam.

### Acceptance Checklist

- the surface stays anchored to Atmosphere Composer
...
- recipe path: frontend/visual-encyclopedia/recipes/capture/capture_post_input_route_reveal.md
...
Capture Post-Input Route Reveal keeps its trust seam attached to Atmosphere Composer and Capture.
...
Capture Post-Input Route Reveal exposes local runtime behavior as inspectable state, not hidden automation.
...
Capture Post-Input Route Reveal previews local reset or forget consequences before the user commits to them.

### Trust Boundary

Capture remains local-first unless the active truth explicitly says otherwise.
...
## Purpose
Capture Post-Input Route Reveal exists to make Capture understandable through the Atmosphere Composer, not through a generic productivity screen.
Minimum specificity: the recipe must name capture_post_input_route_reveal, the user job, and the reason this surface owns that job.

## Surface Hierarchy
Capture Post-Input Route Reveal sits inside Capture and is subordinate to the Atmosphere Composer; supporting surfaces must not outrank it.
...
Minimum specificity: if Capture Post-Input Route Reveal cannot show what Atmosphere Composer owns, the surface is not final-proof eligible.

## Label-Off Signature
With labels blurred, Capture Post-Input Route Reveal must still read as Atmosphere Composer through capture field, route reveal seam, needs-a-place holding state, source/proof edge, and recovery return path.
...
## Visible Regions
Visible regions for Capture Post-Input Route Reveal: capture field, route reveal seam, needs-a-place holding state, source/proof edge, and recovery return path.
...
Disclosure opens deeper source, proof, consequence, or correction detail for Capture Post-Input Route Reveal; it is never decorative navigation noise.
Minimum specificity: every chevron must have a visible label or accessibility meaning tied to Atmosphere Composer.

## Source / Trust Behavior
Source Link Status: intended_only. Capture Post-Input Route Reveal must show whether source is linked, intended-only, stale, unavailable, or local-only.
...
Minimum specificity: proof cannot be a detached feed, generic toast, or release claim for Capture Post-Input Route Reveal.

## Transaction Behavior
Transactions on Capture Post-Input Route Reveal require visible intent, preview when consequence matters, explicit commit, receipt, and undo or recovery.
...
## VoiceOver Order
VoiceOver Order: Capture, Capture Post-Input Route Reveal, Atmosphere Composer, state, source status, proof or receipt status, primary action, correction path.
...
## Reduce Motion Behavior
Reduce Motion replaces movement in Capture Post-Input Route Reveal with static before/after labels, source summaries, receipt state, and recovery labels.
...
## Native iPhone Believability Requirements
Capture Post-Input Route Reveal must feel like a restrained native iPhone surface with thumb-reachable actions, semantic rows, sheets, and stable safe areas.
...
Source-family influence for Capture Post-Input Route Reveal: visual canon, accessibility, source/proof receipts, local trust, transaction, and Capture object depth.
Minimum specificity: train references must explain behavior, not imply shipped SwiftUI implementation.

## Source Linkage
Source Link Status: intended_only. Implementation Proof Boundary: Not In Scope. Required gates: object_depth, transaction, anti_generic, accessibility.
...
## Implementation Proof Boundary
Implementation Proof Boundary: this recipe is final-state design canon for Capture Post-Input Route Reveal; it does not prove SwiftUI, device, release, or accessibility implementation.
Minimum specificity: proof requires source inspection, local validation, and release evidence outside this recipe.

## Unresolved Direction
Unresolved direction for Capture Post-Input Route Reveal: any missing implementation source remains intended_only debt and must stay visible until a scoped implementation batch lands.
...
Validator hint: fail Capture Post-Input Route Reveal if Atmosphere Composer can be replaced by a generic list, calendar, chat, settings, score, or dashboard surface.
...
Minimum specificity: Capture Post-Input Route Reveal may define design canon only and must not claim production readiness.

## Acceptance Checklist
Good example: one capture-first object that preserves the thought, shows routes, and holds uncertainty safely.
Acceptance requires source linkage, proof boundary, VoiceOver/Dynamic Type/Reduce Motion coverage, primary action, correction path, and anti-generic checks for Capture Post-Input Route Reveal.

## P0 Proof Appendix
- Source Link Status: intended_only; Capture Post-Input Route Reveal must keep source status inspectable, including local-only and stale source states, and must not hide intended-only debt.
...
The proof appendix exists to prevent a false Green for Capture Post-Input Route Reveal. It records what can be claimed as design canon and what remains outside implementation proof.
...
Evidence note: Capture Post-Input Route Reveal must keep generated reports, source-link status, and Not In Scope implementation proof aligned before any Green claim.
If any report upgrades Capture Post-Input Route Reveal without this boundary, the validator must fail rather than convert intended canon into shipped behavior.

## P0 Canon Appendix
VoiceOver must announce Capture, Capture Post-Input Route Reveal, Atmosphere Composer, state, Source, Proof, Receipt, Primary Action, and recovery in that order.
...
This appendix is canon authority for Capture Post-Input Route Reveal, not implementation proof, release proof, or an approval to alter top-level IA.
