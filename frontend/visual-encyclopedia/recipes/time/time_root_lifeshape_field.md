...
## Purpose
Time Root / LifeShape Field exists to make Time understandable through the LifeShape Field, not through a generic productivity screen.
Minimum specificity: the recipe must name time_root_lifeshape_field, the user job, and the reason this surface owns that job.

## Surface Hierarchy
Time Root / LifeShape Field sits inside Time and is subordinate to the LifeShape Field; supporting surfaces must not outrank it.
...
Minimum specificity: if Time Root / LifeShape Field cannot show what LifeShape Field owns, the surface is not final-proof eligible.

## Label-Off Signature
With labels blurred, Time Root / LifeShape Field must still read as LifeShape Field through capacity field, protected-time boundary, fit/reflow preview, source/proof edge, and recovery return path.
...
## Visible Regions
Visible regions for Time Root / LifeShape Field: capacity field, protected-time boundary, fit/reflow preview, source/proof edge, and recovery return path.
...
Disclosure opens deeper source, proof, consequence, or correction detail for Time Root / LifeShape Field; it is never decorative navigation noise.
Minimum specificity: every chevron must have a visible label or accessibility meaning tied to LifeShape Field.

## Source / Trust Behavior
Source Link Status: linked. Time Root / LifeShape Field must show whether source is linked, intended-only, stale, unavailable, or local-only.
...
Minimum specificity: proof cannot be a detached feed, generic toast, or release claim for Time Root / LifeShape Field.

## Transaction Behavior
Transactions on Time Root / LifeShape Field require visible intent, preview when consequence matters, explicit commit, receipt, and undo or recovery.
...
## VoiceOver Order
VoiceOver Order: Time, Time Root / LifeShape Field, LifeShape Field, state, source status, proof or receipt status, primary action, correction path.
...
## Reduce Motion Behavior
Reduce Motion replaces movement in Time Root / LifeShape Field with static before/after labels, source summaries, receipt state, and recovery labels.
...
## Native iPhone Believability Requirements
Time Root / LifeShape Field must feel like a restrained native iPhone surface with thumb-reachable actions, semantic rows, sheets, and stable safe areas.
...
Source-family influence for Time Root / LifeShape Field: visual canon, accessibility, source/proof receipts, local trust, transaction, and Time object depth.
Minimum specificity: train references must explain behavior, not imply shipped SwiftUI implementation.

## Source Linkage
Source Link Status: linked. Implementation Proof Boundary: Not In Scope. Required gates: ia, object_depth, anti_generic, accessibility, native_believability.
...
## Implementation Proof Boundary
Implementation Proof Boundary: this recipe is final-state design canon for Time Root / LifeShape Field; it does not prove SwiftUI, device, release, or accessibility implementation.
Minimum specificity: proof requires source inspection, local validation, and release evidence outside this recipe.

## Unresolved Direction
Unresolved direction for Time Root / LifeShape Field: any missing implementation source remains linked debt and must stay visible until a scoped implementation batch lands.
...
Validator hint: fail Time Root / LifeShape Field if LifeShape Field can be replaced by a generic list, calendar, chat, settings, score, or dashboard surface.
...
Minimum specificity: Time Root / LifeShape Field may define design canon only and must not claim production readiness.

## Acceptance Checklist
Good example: one capacity object showing pressure, protected time, before/after fit, and undoable reflow.
Acceptance requires source linkage, proof boundary, VoiceOver/Dynamic Type/Reduce Motion coverage, primary action, correction path, and anti-generic checks for Time Root / LifeShape Field.

## P0 Proof Appendix
- Source Link Status: linked; Time Root / LifeShape Field must keep source status inspectable, including local-only and stale source states, and must not hide intended-only debt.
...
The proof appendix exists to prevent a false Green for Time Root / LifeShape Field. It records what can be claimed as design canon and what remains outside implementation proof.
...
Evidence note: Time Root / LifeShape Field must keep generated reports, source-link status, and Not In Scope implementation proof aligned before any Green claim.
If any report upgrades Time Root / LifeShape Field without this boundary, the validator must fail rather than convert intended canon into shipped behavior.

## P0 Canon Appendix
VoiceOver must announce Time, Time Root / LifeShape Field, LifeShape Field, state, Source, Proof, Receipt, Primary Action, and recovery in that order.
...
This appendix is canon authority for Time Root / LifeShape Field, not implementation proof, release proof, or an approval to alter top-level IA.
