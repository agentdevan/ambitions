# Surface Recipe: Today Start Here Region

## Canon Status
intended_canon

## Surface ID
today_start_here_region
...
## Final Intended Role
The attached decision region for the next grounded action. It must emerge from the active Reality Meridian segment rather than sit as a detached recommendation card.
...
It exists so the Today root can present the next grounded action, its source basis, and its recovery path as part of Reality Meridian rather than a detached card.
...
This recipe is final-state design canon for Today Start Here Region in Today. It does not prove implementation or release readiness.

### Good / Bad Example

- Good: Today Start Here Region stays attached to Reality Meridian, with trust, proof, receipt, and recovery visible.
- Bad: Today Start Here Region turns into a generic productivity pattern or hides its trust seam.
...
Today Start Here Region keeps its trust seam attached to Reality Meridian and Today.
...
Today Start Here Region exposes local runtime behavior as inspectable state, not hidden automation.
...
Today Start Here Region previews local reset or forget consequences before the user commits to them.
...
## Purpose
Today Start Here Region exists to make Today understandable through the Reality Meridian, not through a generic productivity screen.
Minimum specificity: the recipe must name today_start_here_region, the user job, and the reason this surface owns that job.

## Surface Hierarchy
Today Start Here Region sits inside Today and is subordinate to the Reality Meridian; supporting surfaces must not outrank it.
...
Minimum specificity: if Today Start Here Region cannot show what Reality Meridian owns, the surface is not final-proof eligible.

## Label-Off Signature
With labels blurred, Today Start Here Region must still read as Reality Meridian through orientation rail, current state seam, source/proof edge, primary action shelf, and recovery return path.
...
## Visible Regions
Visible regions for Today Start Here Region: orientation rail, current state seam, source/proof edge, primary action shelf, and recovery return path.
...
Disclosure opens deeper source, proof, consequence, or correction detail for Today Start Here Region; it is never decorative navigation noise.
Minimum specificity: every chevron must have a visible label or accessibility meaning tied to Reality Meridian.

## Source / Trust Behavior
Source Link Status: intended_only. Today Start Here Region must show whether source is linked, intended-only, stale, unavailable, or local-only.
...
Minimum specificity: proof cannot be a detached feed, generic toast, or release claim for Today Start Here Region.

## Transaction Behavior
Transactions on Today Start Here Region require visible intent, preview when consequence matters, explicit commit, receipt, and undo or recovery.
...
## VoiceOver Order
VoiceOver Order: Today, Today Start Here Region, Reality Meridian, state, source status, proof or receipt status, primary action, correction path.
...
## Reduce Motion Behavior
Reduce Motion replaces movement in Today Start Here Region with static before/after labels, source summaries, receipt state, and recovery labels.
...
## Native iPhone Believability Requirements
Today Start Here Region must feel like a restrained native iPhone surface with thumb-reachable actions, semantic rows, sheets, and stable safe areas.
...
Source-family influence for Today Start Here Region: visual canon, accessibility, source/proof receipts, local trust, transaction, and Today object depth.
...
## Implementation Proof Boundary
Implementation Proof Boundary: this recipe is final-state design canon for Today Start Here Region; it does not prove SwiftUI, device, release, or accessibility implementation.
Minimum specificity: proof requires source inspection, local validation, and release evidence outside this recipe.

## Unresolved Direction
Unresolved direction for Today Start Here Region: any missing implementation source remains intended_only debt and must stay visible until a scoped implementation batch lands.
...
Validator hint: fail Today Start Here Region if Reality Meridian can be replaced by a generic list, calendar, chat, settings, score, or dashboard surface.
...
Minimum specificity: Today Start Here Region may define design canon only and must not claim production readiness.

## Acceptance Checklist
Good example: one current-day object with source, proof, action, and recovery visibly attached.
Acceptance requires source linkage, proof boundary, VoiceOver/Dynamic Type/Reduce Motion coverage, primary action, correction path, and anti-generic checks for Today Start Here Region.

## P0 Proof Appendix
- Source Link Status: intended_only; Today Start Here Region must keep source status inspectable, including local-only and stale source states, and must not hide intended-only debt.
...
The proof appendix exists to prevent a false Green for Today Start Here Region. It records what can be claimed as design canon and what remains outside implementation proof.
...
Evidence note: Today Start Here Region must keep generated reports, source-link status, and Not In Scope implementation proof aligned before any Green claim.
If any report upgrades Today Start Here Region without this boundary, the validator must fail rather than convert intended canon into shipped behavior.

## P0 Canon Appendix
VoiceOver must announce Today, Today Start Here Region, Reality Meridian, state, Source, Proof, Receipt, Primary Action, and recovery in that order.
...
This appendix is canon authority for Today Start Here Region, not implementation proof, release proof, or an approval to alter top-level IA.
