# Surface Recipe: Reflection / Recovery Detail

## Canon Status
intended_canon

## Surface ID
reflection_recovery_detail
...
## Final Intended Role
A Goals support surface that preserves Constellation Atlas orientation and proof-backed ambition continuity.
...
- Ambition/commitment/proof relationships
- Recommended step or blocker path
- Proof trail/source line
- Review/recovery/archive state
...
- Purpose: Show how Life area or goal-thread focus changes the visible hierarchy, source/proof meaning, and available action for Reflection / Recovery Detail.
...
- Purpose: Show how Recommended step or blocker path changes the visible hierarchy, source/proof meaning, and available action for Reflection / Recovery Detail.
- Contains: Constellation Atlas; visible ingredients include goal-thread title, life-area relationship, next-step or proof gap, progress/proof marker, source basis, receipt path, recovery cue, and detail disclosure. Supporting objects: Receipt System, Source Freshness Badge, Why This Sheet, Closure System. Region context: orientation, primary object, source/proof line.
- Primitives: GraphiteRecess ground, LuminousTrace attachment, CelestialField only for semantic orientation, SF Symbols, chevrons, label system.
- Typography: Use native iPhone semantic text hierarchy: compact region label, Constellation Atlas title or state label, source/proof caption, and readable action text. Emphasis stays on the active Constellation Atlas decision or inspected state.
- Spacing: Keep source, proof, CTA, and state notes visually attached to Constellation Atlas, with tight native grouping, thumb-zone reach for actions, and enough separation to prevent equal-weight card stacking.
...
- Purpose: Show how Review/recovery/archive state changes the visible hierarchy, source/proof meaning, and available action for Reflection / Recovery Detail.
- Contains: Constellation Atlas; visible ingredients include goal-thread title, life-area relationship, next-step or proof gap, progress/proof marker, source basis, receipt path, recovery cue, and detail disclosure. Supporting objects: Receipt System, Source Freshness Badge, Why This Sheet, Closure System. Region context: orientation, primary object, source/proof line.
- Primitives: GraphiteRecess ground, LuminousTrace attachment, CelestialField only for semantic orientation, SF Symbols, chevrons, label system.
- Typography: Use native iPhone semantic text hierarchy: compact region label, Constellation Atlas title or state label, source/proof caption, and readable action text. Emphasis stays on the active Constellation Atlas decision or inspected state.
- Spacing: Keep source, proof, CTA, and state notes visually attached to Constellation Atlas, with tight native grouping, thumb-zone reach for actions, and enough separation to prevent equal-weight card stacking.
...
## Object Inventory
Constellation Atlas, plus any visible Receipt System, Closure System, Recommendation Source System, Proof Trail System, Commitment Staging Tray, Reflow Preview Tray, Personal Runtime, or source/proof objects referenced by this surface.
...
This recipe is final-state design canon for Reflection / Recovery Detail in Goals. It does not prove implementation or release readiness.

### Good / Bad Example

- Good: Reflection / Recovery Detail stays attached to Constellation Atlas, with trust, proof, receipt, and recovery visible.
- Bad: Reflection / Recovery Detail turns into a generic productivity pattern or hides its trust seam.
...
- recipe path: frontend/visual-encyclopedia/recipes/goals/reflection_recovery_detail.md
...
Reflection / Recovery Detail keeps its trust seam attached to Constellation Atlas and Goals.
...
Reflection / Recovery Detail exposes local runtime behavior as inspectable state, not hidden automation.
...
Reflection / Recovery Detail previews local reset or forget consequences before the user commits to them.
...
## Purpose
Reflection / Recovery Detail exists to make Goals understandable through the Constellation Atlas, not through a generic productivity screen.
Minimum specificity: the recipe must name reflection_recovery_detail, the user job, and the reason this surface owns that job.

## Surface Hierarchy
Reflection / Recovery Detail sits inside Goals and is subordinate to the Constellation Atlas; supporting surfaces must not outrank it.
...
Minimum specificity: if Reflection / Recovery Detail cannot show what Constellation Atlas owns, the surface is not final-proof eligible.

## Label-Off Signature
With labels blurred, Reflection / Recovery Detail must still read as Constellation Atlas through life-area map, goal thread object, proof trail seam, source/proof edge, and recovery return path.
...
## Visible Regions
Visible regions for Reflection / Recovery Detail: life-area map, goal thread object, proof trail seam, source/proof edge, and recovery return path.
...
Disclosure opens deeper source, proof, consequence, or correction detail for Reflection / Recovery Detail; it is never decorative navigation noise.
Minimum specificity: every chevron must have a visible label or accessibility meaning tied to Constellation Atlas.

## Source / Trust Behavior
Source Link Status: intended_only. Reflection / Recovery Detail must show whether source is linked, intended-only, stale, unavailable, or local-only.
...
Minimum specificity: proof cannot be a detached feed, generic toast, or release claim for Reflection / Recovery Detail.

## Transaction Behavior
Transactions on Reflection / Recovery Detail require visible intent, preview when consequence matters, explicit commit, receipt, and undo or recovery.
...
## VoiceOver Order
VoiceOver Order: Goals, Reflection / Recovery Detail, Constellation Atlas, state, source status, proof or receipt status, primary action, correction path.
...
## Reduce Motion Behavior
Reduce Motion replaces movement in Reflection / Recovery Detail with static before/after labels, source summaries, receipt state, and recovery labels.
...
## Native iPhone Believability Requirements
Reflection / Recovery Detail must feel like a restrained native iPhone surface with thumb-reachable actions, semantic rows, sheets, and stable safe areas.
...
Source-family influence for Reflection / Recovery Detail: visual canon, accessibility, source/proof receipts, local trust, transaction, and Goals object depth.
Minimum specificity: train references must explain behavior, not imply shipped SwiftUI implementation.

## Source Linkage
Source Link Status: intended_only. Implementation Proof Boundary: Not In Scope. Required gates: transaction, accessibility, anti_generic.
...
## Implementation Proof Boundary
Implementation Proof Boundary: this recipe is final-state design canon for Reflection / Recovery Detail; it does not prove SwiftUI, device, release, or accessibility implementation.
Minimum specificity: proof requires source inspection, local validation, and release evidence outside this recipe.

## Unresolved Direction
Unresolved direction for Reflection / Recovery Detail: any missing implementation source remains intended_only debt and must stay visible until a scoped implementation batch lands.
...
Validator hint: fail Reflection / Recovery Detail if Constellation Atlas can be replaced by a generic list, calendar, chat, settings, score, or dashboard surface.
...
Minimum specificity: Reflection / Recovery Detail may define design canon only and must not claim production readiness.

## Acceptance Checklist
Good example: one relationship object where goals, life areas, proof, and recovery stay connected.
Acceptance requires source linkage, proof boundary, VoiceOver/Dynamic Type/Reduce Motion coverage, primary action, correction path, and anti-generic checks for Reflection / Recovery Detail.

## P0 Proof Appendix
- Source Link Status: intended_only; Reflection / Recovery Detail must keep source status inspectable, including local-only and stale source states, and must not hide intended-only debt.
...
The proof appendix exists to prevent a false Green for Reflection / Recovery Detail. It records what can be claimed as design canon and what remains outside implementation proof.
...
Evidence note: Reflection / Recovery Detail must keep generated reports, source-link status, and Not In Scope implementation proof aligned before any Green claim.
If any report upgrades Reflection / Recovery Detail without this boundary, the validator must fail rather than convert intended canon into shipped behavior.

## P0 Canon Appendix
VoiceOver must announce Goals, Reflection / Recovery Detail, Constellation Atlas, state, Source, Proof, Receipt, Primary Action, and recovery in that order.
...
This appendix is canon authority for Reflection / Recovery Detail, not implementation proof, release proof, or an approval to alter top-level IA.
