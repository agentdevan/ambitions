...
## Final Intended Role
A proof-continuity surface that shows evidence, gaps, and transfer meaning across goals and pivots.
...
It exists so Goals can show proof continuity, gaps, and transfer meaning as a visible surface rather than a rank or score system.
...
This recipe is final-state design canon for Proof Trail in Goals. It does not prove implementation or release readiness.

### Good / Bad Example

- Good: Proof Trail stays attached to Constellation Atlas, with trust, proof, receipt, and recovery visible.
- Bad: Proof Trail turns into a generic productivity pattern or hides its trust seam.
...
- recipe path: frontend/visual-encyclopedia/recipes/goals/proof_trail.md
...
Proof Trail keeps its trust seam attached to Constellation Atlas and Goals.
...
Proof Trail exposes local runtime behavior as inspectable state, not hidden automation.
...
Proof Trail previews local reset or forget consequences before the user commits to them.
...
## Purpose
Proof Trail exists to make Goals understandable through the Constellation Atlas, not through a generic productivity screen.
Minimum specificity: the recipe must name proof_trail, the user job, and the reason this surface owns that job.

## Surface Hierarchy
Proof Trail sits inside Goals and is subordinate to the Constellation Atlas; supporting surfaces must not outrank it.
...
Minimum specificity: if Proof Trail cannot show what Constellation Atlas owns, the surface is not final-proof eligible.

## Label-Off Signature
With labels blurred, Proof Trail must still read as Constellation Atlas through life-area map, goal thread object, proof trail seam, source/proof edge, and recovery return path.
...
## Visible Regions
Visible regions for Proof Trail: life-area map, goal thread object, proof trail seam, source/proof edge, and recovery return path.
...
Disclosure opens deeper source, proof, consequence, or correction detail for Proof Trail; it is never decorative navigation noise.
Minimum specificity: every chevron must have a visible label or accessibility meaning tied to Constellation Atlas.

## Source / Trust Behavior
Source Link Status: intended_only. Proof Trail must show whether source is linked, intended-only, stale, unavailable, or local-only.
...
Minimum specificity: proof cannot be a detached feed, generic toast, or release claim for Proof Trail.

## Transaction Behavior
Transactions on Proof Trail require visible intent, preview when consequence matters, explicit commit, receipt, and undo or recovery.
...
## VoiceOver Order
VoiceOver Order: Goals, Proof Trail, Constellation Atlas, state, source status, proof or receipt status, primary action, correction path.
...
## Reduce Motion Behavior
Reduce Motion replaces movement in Proof Trail with static before/after labels, source summaries, receipt state, and recovery labels.
...
## Native iPhone Believability Requirements
Proof Trail must feel like a restrained native iPhone surface with thumb-reachable actions, semantic rows, sheets, and stable safe areas.
...
Source-family influence for Proof Trail: visual canon, accessibility, source/proof receipts, local trust, transaction, and Goals object depth.
Minimum specificity: train references must explain behavior, not imply shipped SwiftUI implementation.

## Source Linkage
Source Link Status: intended_only. Implementation Proof Boundary: Not In Scope. Required gates: proof_source_receipt, transaction, accessibility.
...
## Implementation Proof Boundary
Implementation Proof Boundary: this recipe is final-state design canon for Proof Trail; it does not prove SwiftUI, device, release, or accessibility implementation.
Minimum specificity: proof requires source inspection, local validation, and release evidence outside this recipe.

## Unresolved Direction
Unresolved direction for Proof Trail: any missing implementation source remains intended_only debt and must stay visible until a scoped implementation batch lands.
...
Validator hint: fail Proof Trail if Constellation Atlas can be replaced by a generic list, calendar, chat, settings, score, or dashboard surface.
...
Minimum specificity: Proof Trail may define design canon only and must not claim production readiness.

## Acceptance Checklist
Good example: one relationship object where goals, life areas, proof, and recovery stay connected.
Acceptance requires source linkage, proof boundary, VoiceOver/Dynamic Type/Reduce Motion coverage, primary action, correction path, and anti-generic checks for Proof Trail.

## P0 Proof Appendix
- Source Link Status: intended_only; Proof Trail must keep source status inspectable, including local-only and stale source states, and must not hide intended-only debt.
...
The proof appendix exists to prevent a false Green for Proof Trail. It records what can be claimed as design canon and what remains outside implementation proof.
...
Evidence note: Proof Trail must keep generated reports, source-link status, and Not In Scope implementation proof aligned before any Green claim.
If any report upgrades Proof Trail without this boundary, the validator must fail rather than convert intended canon into shipped behavior.

## P0 Canon Appendix
VoiceOver must announce Goals, Proof Trail, Constellation Atlas, state, Source, Proof, Receipt, Primary Action, and recovery in that order.
...
This appendix is canon authority for Proof Trail, not implementation proof, release proof, or an approval to alter top-level IA.
