# Surface Recipe: Goal Thread Detail
...
## Final Intended Role
A deep object surface for a goal thread, linking ambition, commitments, recommended steps, blockers, proof, and recovery.
...
- Purpose: Show how Life area or goal-thread focus changes the visible hierarchy, source/proof meaning, and available action for Goal Thread Detail.
...
- Purpose: Show how Recommended step or blocker path changes the visible hierarchy, source/proof meaning, and available action for Goal Thread Detail.
- Contains: Constellation Atlas; visible ingredients include goal-thread title, life-area relationship, next-step or proof gap, progress/proof marker, source basis, receipt path, recovery cue, and detail disclosure. Supporting objects: Receipt System, Source Freshness Badge, Why This Sheet, Closure System. Region context: orientation, primary object, source/proof line.
- Primitives: GraphiteRecess ground, LuminousTrace attachment, CelestialField only for semantic orientation, SF Symbols, chevrons, label system.
- Typography: Use native iPhone semantic text hierarchy: compact region label, Constellation Atlas title or state label, source/proof caption, and readable action text. Emphasis stays on the active Constellation Atlas decision or inspected state.
- Spacing: Keep source, proof, CTA, and state notes visually attached to Constellation Atlas, with tight native grouping, thumb-zone reach for actions, and enough separation to prevent equal-weight card stacking.
...
- Purpose: Show how Review/recovery/archive state changes the visible hierarchy, source/proof meaning, and available action for Goal Thread Detail.
- Contains: Constellation Atlas; visible ingredients include goal-thread title, life-area relationship, next-step or proof gap, progress/proof marker, source basis, receipt path, recovery cue, and detail disclosure. Supporting objects: Receipt System, Source Freshness Badge, Why This Sheet, Closure System. Region context: orientation, primary object, source/proof line.
- Primitives: GraphiteRecess ground, LuminousTrace attachment, CelestialField only for semantic orientation, SF Symbols, chevrons, label system.
- Typography: Use native iPhone semantic text hierarchy: compact region label, Constellation Atlas title or state label, source/proof caption, and readable action text. Emphasis stays on the active Constellation Atlas decision or inspected state.
- Spacing: Keep source, proof, CTA, and state notes visually attached to Constellation Atlas, with tight native grouping, thumb-zone reach for actions, and enough separation to prevent equal-weight card stacking.
...
This recipe is final-state design canon for Goal Thread Detail in Goals. It does not prove implementation or release readiness.

### Good / Bad Example

- Good: Goal Thread Detail stays attached to Constellation Atlas, with trust, proof, receipt, and recovery visible.
- Bad: Goal Thread Detail turns into a generic productivity pattern or hides its trust seam.
...
- recipe path: frontend/visual-encyclopedia/recipes/goals/goal_thread_detail.md
...
Goal Thread Detail keeps its trust seam attached to Constellation Atlas and Goals.
...
Goal Thread Detail exposes local runtime behavior as inspectable state, not hidden automation.
...
Goal Thread Detail previews local reset or forget consequences before the user commits to them.
...
## Purpose
Goal Thread Detail exists to make Goals understandable through the Constellation Atlas, not through a generic productivity screen.
Minimum specificity: the recipe must name goal_thread_detail, the user job, and the reason this surface owns that job.

## Surface Hierarchy
Goal Thread Detail sits inside Goals and is subordinate to the Constellation Atlas; supporting surfaces must not outrank it.
...
Minimum specificity: if Goal Thread Detail cannot show what Constellation Atlas owns, the surface is not final-proof eligible.

## Label-Off Signature
With labels blurred, Goal Thread Detail must still read as Constellation Atlas through life-area map, goal thread object, proof trail seam, source/proof edge, and recovery return path.
...
## Visible Regions
Visible regions for Goal Thread Detail: life-area map, goal thread object, proof trail seam, source/proof edge, and recovery return path.
...
Disclosure opens deeper source, proof, consequence, or correction detail for Goal Thread Detail; it is never decorative navigation noise.
Minimum specificity: every chevron must have a visible label or accessibility meaning tied to Constellation Atlas.

## Source / Trust Behavior
Source Link Status: intended_only. Goal Thread Detail must show whether source is linked, intended-only, stale, unavailable, or local-only.
...
Minimum specificity: proof cannot be a detached feed, generic toast, or release claim for Goal Thread Detail.

## Transaction Behavior
Transactions on Goal Thread Detail require visible intent, preview when consequence matters, explicit commit, receipt, and undo or recovery.
...
## VoiceOver Order
VoiceOver Order: Goals, Goal Thread Detail, Constellation Atlas, state, source status, proof or receipt status, primary action, correction path.
...
## Reduce Motion Behavior
Reduce Motion replaces movement in Goal Thread Detail with static before/after labels, source summaries, receipt state, and recovery labels.
...
## Native iPhone Believability Requirements
Goal Thread Detail must feel like a restrained native iPhone surface with thumb-reachable actions, semantic rows, sheets, and stable safe areas.
...
Source-family influence for Goal Thread Detail: visual canon, accessibility, source/proof receipts, local trust, transaction, and Goals object depth.
Minimum specificity: train references must explain behavior, not imply shipped SwiftUI implementation.

## Source Linkage
Source Link Status: intended_only. Implementation Proof Boundary: Not In Scope. Required gates: object_depth, proof_source_receipt, accessibility.
...
## Implementation Proof Boundary
Implementation Proof Boundary: this recipe is final-state design canon for Goal Thread Detail; it does not prove SwiftUI, device, release, or accessibility implementation.
Minimum specificity: proof requires source inspection, local validation, and release evidence outside this recipe.

## Unresolved Direction
Unresolved direction for Goal Thread Detail: any missing implementation source remains intended_only debt and must stay visible until a scoped implementation batch lands.
...
Validator hint: fail Goal Thread Detail if Constellation Atlas can be replaced by a generic list, calendar, chat, settings, score, or dashboard surface.
...
Minimum specificity: Goal Thread Detail may define design canon only and must not claim production readiness.

## Acceptance Checklist
Good example: one relationship object where goals, life areas, proof, and recovery stay connected.
Acceptance requires source linkage, proof boundary, VoiceOver/Dynamic Type/Reduce Motion coverage, primary action, correction path, and anti-generic checks for Goal Thread Detail.

## P0 Proof Appendix
- Source Link Status: intended_only; Goal Thread Detail must keep source status inspectable, including local-only and stale source states, and must not hide intended-only debt.
...
The proof appendix exists to prevent a false Green for Goal Thread Detail. It records what can be claimed as design canon and what remains outside implementation proof.
...
Evidence note: Goal Thread Detail must keep generated reports, source-link status, and Not In Scope implementation proof aligned before any Green claim.
If any report upgrades Goal Thread Detail without this boundary, the validator must fail rather than convert intended canon into shipped behavior.

## P0 Canon Appendix
VoiceOver must announce Goals, Goal Thread Detail, Constellation Atlas, state, Source, Proof, Receipt, Primary Action, and recovery in that order.
...
This appendix is canon authority for Goal Thread Detail, not implementation proof, release proof, or an approval to alter top-level IA.
