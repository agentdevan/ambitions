...
- Fused `RealityMeridianCurrentTimeCursor()` onto the `DayTimelineRail` composition through a top-leading rail overlay in `TodayScreen`.

## Proof Collected

- Source files are installed in the repo.
- Decision file and design-system matrix now mark the primitives as existing.
- Gap report and proof contract were updated to reflect the fused rail composition source installation.
