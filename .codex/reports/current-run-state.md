...
- current task: F18 Feature-Flagged Meridian Shell Implementation
- task size: XL release / architecture / product quality train
- active mode: F18 Green pending commit; F18.5 not triggered
...
- active surface: feature-flagged Meridian shell presentation and native fallback
...
- files touched: `Native/Ambitions/App/AppShellPresentationMode.swift`, `Native/Ambitions/App/AppMeridianShell.swift`, `Native/Ambitions/App/AmbitionsRootView.swift`, `Native/AmbitionsTests/App/AppShellNavigationTests.swift`, F18 report and tracking files
- decisions made: F18 is Green; Meridian is local-flagged and default-off; native fallback remains default; F18.5 is not triggered because no route ownership moved and no new extraction-required shell file appeared
- tests run: pre-edit dev tools PASS; batch preflight PASS; batch gate check PASS; F17 repair build PASS; focused AppShellNavigationTests PASS (`20` tests); focused ExternalRoutingTests + AppIntentRoutingTests PASS (`31` tests); F18 build PASS on iPhone 17; architecture scan advisory/PARTIAL with pre-existing warnings; diff check PASS
...
- next phase: commit and push F18; if pushed, start F19 route parity / fallback safety
...
- last checkpoint: F18 implementation and focused validation completed Green pending commit
