...
Next eligible batch: PK38 Move Domain To Package
Scope: PK37 Derived Read-Model Cache is complete / Green with local in-memory Today read-model caching and focused proof recorded in `docs/audits/pk37-batch-closeout-report.md`; PK38 Move Domain To Package is next after prior active dependencies. This run-state mirror does not claim release readiness, device validation, accessibility conformance, performance validation, sync/cloud behavior, hosted AI, TestFlight/App Store readiness, automatic multi-device merge execution, deletion/export execution, or global train completion.
