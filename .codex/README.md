...
- `state/` — compact mirrors for current facts, active batch, recent validation, and ledgers; owner docs and raw logs win.
- `manifests/` — machine-readable Codex OS maps for efficiency, ACX profiles, gates, skills, ownership, source truth, and no-double-work.
...
3. Choose one context pack.
4. Choose one primary skill.
5. Choose one operation protocol.
6. Choose one validation pack.
7. Use `scripts/ai/acx.py` for non-executing extraction and `scripts/ai/acx_local.py` only for allowlisted local profiles.
8. Close out with raw-log evidence, exit codes, Green/Yellow/Red, and next prompt.
