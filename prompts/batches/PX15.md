...
```bash
make batch BATCH=PX15 PROMPT=prompts/batches/PX15.md
```
...
Do not execute implementation from this historical_complete_do_not_run record; preserve PX15 as canonical coverage and use it only as historical, overlay, or conditional metadata until active truth reauthorizes it.
...
Preserve canonical ID `PX15` and canonical queue order. Do not reactivate completed batches, renumber IDs, collapse PK17-PK41, pull RHC broad cleanup early, turn AIR into a standalone train, turn EFC into a broad implementation stream, or make DPTG terminal proof executable before all pre-device gates close.
...
- Do not run PX15 as an implementation batch from this prompt.
- Use this file to preserve queue coverage for Cross Surface Continuity System, the canonical status `historical_complete_do_not_run`, and the next handoff `PX16`.
...
Create or update `docs/audits/px15-batch-closeout-report.md` with status, source truth inspected, files changed, validation commands and exit codes, EFC applicability, accepted-yellow rationale if any, claims not made, rollback notes, and next handoff.
