...
```bash
make batch BATCH=RHC05 PROMPT=prompts/batches/RHC05.md
```
...
Implement Validation Script Noise And Allowlist Hardening as owner-mapped repo hygiene only; classify and repair the named seam without broad cleanup or production behavior drift.
...
Preserve canonical ID `RHC05` and canonical queue order. Do not reactivate completed batches, renumber IDs, collapse PK17-PK41, pull RHC broad cleanup early, turn AIR into a standalone train, turn EFC into a broad implementation stream, or make DPTG terminal proof executable before all pre-device gates close.
...
- Inspect current git status, cleanup index, RHC train manifest, and prompt/scan outputs before touching Validation Script Noise And Allowlist Hardening.
...
Create or update `docs/audits/rhc05-batch-closeout-report.md` with status, source truth inspected, files changed, validation commands and exit codes, EFC applicability, accepted-yellow rationale if any, claims not made, rollback notes, and next handoff.
