...
```bash
make batch BATCH=FCP28 PROMPT=prompts/batches/FCP28.md
```
...
Implement Final Visual Proof Packet as a flagship closeout batch with source-backed UI/accessibility/visual proof and conservative release claim boundaries.
...
Preserve canonical ID `FCP28` and canonical queue order. Do not reactivate completed batches, renumber IDs, collapse PK17-PK41, pull RHC broad cleanup early, turn AIR into a standalone train, turn EFC into a broad implementation stream, or make DPTG terminal proof executable before all pre-device gates close.
...
- Inspect the current proof packets, release truth, and owner files before implementing Final Visual Proof Packet.
...
Create or update `docs/audits/fcp28-batch-closeout-report.md` with status, source truth inspected, files changed, validation commands and exit codes, EFC applicability, accepted-yellow rationale if any, claims not made, rollback notes, and next handoff.
