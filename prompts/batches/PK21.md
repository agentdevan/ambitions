...
```bash
make batch BATCH=PK21 PROMPT=prompts/batches/PK21.md
```
...
- Dependency gate: Complete prior PK batch PK20 and any data-safety proof named by the PK train.
- Next handoff: PK22

# Objective

Extract the Time service behind the user-facing Time surface while preserving Plan names only as internal compatibility seams.
...
Preserve canonical ID `PK21` and canonical queue order. Do not reactivate completed batches, renumber IDs, collapse PK17-PK41, pull RHC broad cleanup early, turn AIR into a standalone train, turn EFC into a broad implementation stream, or make DPTG terminal proof executable before all pre-device gates close.
...
- Inspect PlanScreen, PlanViewModel, PlanFeatureService, PlanFeatureModels, PlanLifeShapeTimeCapacityMap, PlanLifeSuiteState, and planning domain models.
- Move Time/LifeShape read and command boundaries into the smallest Plan-owned compatibility seam while keeping user-facing Time direction intact.
- Do not restore Plan as a top-level destination or calendar-grid primary object.
...
- `scripts/ambitions-xcode-validate.sh --batch PK21 --lane focused-test --test <TouchedOwnerTests>`
...
Create or update `docs/audits/pk21-batch-closeout-report.md` with status, source truth inspected, files changed, validation commands and exit codes, EFC applicability, accepted-yellow rationale if any, claims not made, rollback notes, and next handoff.
