...
```bash
make batch BATCH=LDI15 PROMPT=prompts/batches/LDI15.md
```
...
Implement Living Plan Recompiler only after dependency gates prove the required PK transaction/storage and Source Atlas boundaries.
...
Preserve canonical ID `LDI15` and canonical queue order. Do not reactivate completed batches, renumber IDs, collapse PK17-PK41, pull RHC broad cleanup early, turn AIR into a standalone train, turn EFC into a broad implementation stream, or make DPTG terminal proof executable before all pre-device gates close.
...
- Inspect LDI train docs, PlanFeatureService, planning domain models, DeterministicGoalPlanner, AmbitionsCommandExecutor, and receipt models.
- Create a deterministic recompile path that previews plan changes, records mutation permission, and produces rollback/receipt evidence.
- Do not silently mutate user plans, introduce cloud intelligence, or bypass Source Atlas freshness when source-backed claims are used.
...
Create or update `docs/audits/ldi15-batch-closeout-report.md` with status, source truth inspected, files changed, validation commands and exit codes, EFC applicability, accepted-yellow rationale if any, claims not made, rollback notes, and next handoff.
