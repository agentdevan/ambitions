...
```bash
make batch BATCH=SA10 PROMPT=prompts/batches/SA10.md
```
...
Implement Source Atlas freshness and risk modeling with conservative currentness, staleness, and source-risk boundaries.
...
Preserve canonical ID `SA10` and canonical queue order. Do not reactivate completed batches, renumber IDs, collapse PK17-PK41, pull RHC broad cleanup early, turn AIR into a standalone train, turn EFC into a broad implementation stream, or make DPTG terminal proof executable before all pre-device gates close.
...
- Inspect SourceAtlasPackModels, AmbitionsOSSourceTruthModels, Source Atlas train docs, and any existing freshness broker contracts.
- Represent freshness dates, stale/unknown state, adapter/source risk, and revocation without implying live verification unless proof exists.
- Add tests for stale source, unknown source, conflicting source, and no-release-claim wording.
...
Create or update `docs/audits/sa10-batch-closeout-report.md` with status, source truth inspected, files changed, validation commands and exit codes, EFC applicability, accepted-yellow rationale if any, claims not made, rollback notes, and next handoff.
