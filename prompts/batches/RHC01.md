...
```bash
make batch BATCH=RHC01 PROMPT=prompts/batches/RHC01.md
```
...
Perform repo hygiene triage and owner mapping only; create a scoped cleanup map without deleting or broad-refactoring production source.
...
Preserve canonical ID `RHC01` and canonical queue order. Do not reactivate completed batches, renumber IDs, collapse PK17-PK41, pull RHC broad cleanup early, turn AIR into a standalone train, turn EFC into a broad implementation stream, or make DPTG terminal proof executable before all pre-device gates close.
...
- Inspect repo status, docs/status/repo-cleanup-index.md, RHC train manifest, prompt audit output, generated artifact inventory, and large/stale-file evidence.
- Classify cleanup candidates by owner, risk, proof needed, and safe batch; do not execute broad cleanup in RHC01.
- Produce the RHC owner map and next safe cleanup prompts with no release or readiness claims.
...
Create or update `docs/audits/rhc01-batch-closeout-report.md` with status, source truth inspected, files changed, validation commands and exit codes, EFC applicability, accepted-yellow rationale if any, claims not made, rollback notes, and next handoff.
