...
```bash
make batch BATCH=PFC36 PROMPT=prompts/batches/PFC36.md
```
...
- Dependency gate: Relevant PK/FCP/FVQ/EFC proof and human/legal/device gates where named.
- Next handoff: PFC37

# Objective

Implement Performance And Observability Reconciliation as platform-framework compliance work with local validation evidence, proof packet updates, and no release/readiness overclaim.
...
Preserve canonical ID `PFC36` and canonical queue order. Do not reactivate completed batches, renumber IDs, collapse PK17-PK41, pull RHC broad cleanup early, turn AIR into a standalone train, turn EFC into a broad implementation stream, or make DPTG terminal proof executable before all pre-device gates close.
...
- Inspect the current proof packets, release truth, and owner files before implementing Performance And Observability Reconciliation.
- Update only the named evidence/source seam; separate verified proof, needs review proof, skipped proof, and human follow-up.
- Do not claim release, device, accessibility, performance, privacy/legal, TestFlight, or App Store readiness without current proof artifacts.
...
Create or update `docs/audits/pfc36-batch-closeout-report.md` with status, source truth inspected, files changed, validation commands and exit codes, EFC applicability, accepted-yellow rationale if any, claims not made, rollback notes, and next handoff.
