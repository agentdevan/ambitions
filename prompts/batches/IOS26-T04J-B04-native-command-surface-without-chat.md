...
- `capture_routing` is an accepted Yellow touch for `IOS26-T04J-B04` because native command routing must inspect Capture-owned object/action context without creating a new router.
- Affected canonical owner: `capture_root`.
- Consult `private_life_runtime`, `proof_receipt_replay`, and `you_root` only where command action mapping touches SourceRecord, Receipt, ReplayTrace, or You inspection paths.
- Safety reason: this batch extends local command affordances over existing Capture/command/search seams and does not introduce a parallel command intelligence owner, chat transcript, top-level Assistant UI, hosted model, or hosted personal-data backend.
...
- Follow-up gate: `capture_routing` remains locked until Champion Merge or owner-accepted follow-up proves route/search/command ownership Green with focused source and validation evidence.
