...
```bash
make batch BATCH=PX01 PROMPT=prompts/batches/PX01.md
```
...
Keep PXOS Parent Canon as historical/do-not-run coverage; do not execute implementation from this prompt unless a future approved PX train reactivation changes active truth.
...
Preserve canonical ID `PX01` and canonical queue order. Do not reactivate completed batches, renumber IDs, collapse PK17-PK41, pull RHC broad cleanup early, turn AIR into a standalone train, turn EFC into a broad implementation stream, or make DPTG terminal proof executable before all pre-device gates close.
...
- Use docs/truth/PRODUCT_DESIGN_TRUTH.md and PXOS docs only as compatible supporting material.
- Do not modify app source from this do-not-run prompt.
- If future owner reactivates PX work, create a new runnable prompt with exact source files and proof gates.
...
Create or update `docs/audits/px01-batch-closeout-report.md` with status, source truth inspected, files changed, validation commands and exit codes, EFC applicability, accepted-yellow rationale if any, claims not made, rollback notes, and next handoff.
