...
# IOS26-T04K-B02 - Sealed IOS26 Work Order
...
## Objective
Get multiple real paths to accomplish goals.
...
Green: Multi-path tests pass.
Yellow: bounded gap with owner, reason, no-claim boundary, validation posture, and post-batch gate.
Red: Alternatives are copy-only or not grounded in local objects.

## Rollback behavior
Revert only files touched by `IOS26-T04K-B02`. Preserve unrelated dirty work and generated artifacts outside this batch.
...
# IOS26-T04K-B02 - Multi-path execution compiler over real life objects
...
Multi-path goal execution job.

## Replacement P0 contract
Runtime path contract: fastest reliable, deadline-preserving, lighter recovery, protected-time-safe, proof-first, dependency-unblocking, schedule-first, knowledge-gathering paths over real local objects.
