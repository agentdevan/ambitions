...
```bash
make batch BATCH=PK24 PROMPT=prompts/batches/PK24.md
```
...
- Dependency gate: Complete prior PK batch PK23 and any data-safety proof named by the PK train.
- Next handoff: PK25

# Objective

Implement EventKit Through SideEffectLedger in the Platform Kernel owner seam, preserving PK serial order, local-first data boundaries, receipts, and compatibility with existing native SwiftUI surfaces.
...
Preserve canonical ID `PK24` and canonical queue order. Do not reactivate completed batches, renumber IDs, collapse PK17-PK41, pull RHC broad cleanup early, turn AIR into a standalone train, turn EFC into a broad implementation stream, or make DPTG terminal proof executable before all pre-device gates close.
...
- Inspect the existing native owner files for EventKit Through SideEffectLedger before patching; do not invent a parallel architecture.
- Land the smallest source change that makes EventKit Through SideEffectLedger real behind the existing app surfaces and domain/service contracts.
...
- `scripts/ambitions-xcode-validate.sh --batch PK24 --lane focused-test --test <TouchedOwnerTests>`
...
Create or update `docs/audits/pk24-batch-closeout-report.md` with status, source truth inspected, files changed, validation commands and exit codes, EFC applicability, accepted-yellow rationale if any, claims not made, rollback notes, and next handoff.
