...
```bash
make batch BATCH=CS02C PROMPT=prompts/batches/CS02C.md
```
...
Do not execute implementation from this retired_metadata_only record; preserve CS02C as canonical queue coverage and use it only as historical/overlay metadata. Future work requires a newly scoped prompt.
...
Preserve canonical ID `CS02C` and canonical queue order. Do not reactivate completed batches, renumber IDs, collapse PK17-PK41, pull RHC broad cleanup early, turn AIR into a standalone train, turn EFC into a broad implementation stream, or make DPTG terminal proof executable before all pre-device gates close.
...
Primary implementation action: Do not execute implementation from this retired_metadata_only record; preserve CS02C as canonical coverage and use it only as historical/overlay metadata.

Concrete instructions:
- Do not run CS02C as an implementation batch from this prompt.
- Use this file to preserve queue coverage for CSCS02C and the canonical status `retired_metadata_only`.
...
Create or update `docs/audits/cs02c-batch-closeout-report.md` with status, source truth inspected, files changed, validation commands and exit codes, EFC applicability, accepted-yellow rationale if any, claims not made, rollback notes, and next handoff.
