...
```bash
make batch BATCH=FCP29 PROMPT=prompts/batches/FCP29.md
```
...
Implement Accessibility And Dynamic Type Closeout as a flagship closeout batch with source-backed UI/accessibility/visual proof and conservative release claim boundaries.
...
Preserve canonical ID `FCP29` and canonical queue order. Do not reactivate completed batches, renumber IDs, collapse PK17-PK41, pull RHC broad cleanup early, turn AIR into a standalone train, turn EFC into a broad implementation stream, or make DPTG terminal proof executable before all pre-device gates close.
...
- Inspect the current proof packets, release truth, and owner files before implementing Accessibility And Dynamic Type Closeout.
...
Create or update `docs/audits/fcp29-batch-closeout-report.md` with status, source truth inspected, files changed, validation commands and exit codes, EFC applicability, accepted-yellow rationale if any, claims not made, rollback notes, and next handoff.
