...
- Dependency gate: Complete prior PK batch PK16 and any data-safety proof named by the PK train.
- Next handoff: PK18

# Objective

Extract a Today read-model boundary from the current Today feature so TodayScreen and Today panels consume a stable projected state instead of owning projection logic inline.
...
Preserve canonical ID `PK17` and canonical queue order. Do not reactivate completed batches, renumber IDs, collapse PK17-PK41, pull RHC broad cleanup early, turn AIR into a standalone train, turn EFC into a broad implementation stream, or make DPTG terminal proof executable before all pre-device gates close.
...
- Inspect TodayScreen, TodayViewModel, TodayExecutionProjector, TodayExecutionViewState, TodayFeatureService, StubTodayService, and Today feature tests before patching.
- Move read-only Today projection into the smallest existing or new Today-owned read-model/projector seam; keep command handling, persistence mutation, and visual redesign out of this batch.
- Preserve Start Here / Reality Meridian semantics, receipt display state, recovery language, and the active Today / Goals / Capture / Time / You IA.
...
Create or update `docs/audits/pk17-batch-closeout-report.md` with status, source truth inspected, files changed, validation commands and exit codes, EFC applicability, accepted-yellow rationale if any, claims not made, rollback notes, and next handoff.
