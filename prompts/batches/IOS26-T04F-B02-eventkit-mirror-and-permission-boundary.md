...
# IOS26-T04F-B02 - Sealed IOS26 Work Order
...
## Objective
Use Apple Calendar context safely.
...
- Accepted Yellow boundary carried forward: `proof_receipt_replay`. Owner: `proof_receipt_replay`. Reason: adjacent Smart Attachment drift remains unproven after Champion Merge. No-claim boundary: this batch may connect EventKit mirror/permission behavior to canonical `SourceRecord`, local `Receipt`, and `ReplayTrace` expectations only for calendar context permission safety and cannot claim broad proof/receipt/replay completion or Smart Attachment drift resolution. Follow-up gate: `proof_receipt_replay` remains Yellow until the adjacent drift gate is proven Green or owner-accepted. Affected canonical owner/concept: `proof_receipt_replay` / Proof / Receipt / ReplayTrace.
...
- Keep proof artifacts under `build/reports/time-operations/`.
...
Use Apple Calendar context safely. Implement only the scoped local-first behavior after inspecting current source, then prove it with focused tests and proof artifacts. EventKit mirror and permission behavior must preserve canonical `SourceRecord`, local `Receipt`, `ReplayTrace`, and You / `What Ambitions knows` inspection boundaries where calendar source knowledge, sensitive learned behavior, or local intelligence is touched; no broad proof/receipt/replay claim is allowed until the accepted Yellow follow-up gate clears.
...
Green: EventKit mirror permission boundaries are proven with mocks.
Yellow: bounded gap with owner, reason, no-claim boundary, validation posture, and post-batch gate.
Red: External calendar write can occur silently.

## Rollback behavior
Revert only files touched by `IOS26-T04F-B02`. Preserve unrelated dirty work and generated artifacts outside this batch.
...
# IOS26-T04F-B02 - EventKit mirror and permission boundary
...
Calendar context without surrendering local-first control.

## Replacement P0 contract
Calendar P0 EventKit contract: read/mirror behind permission, denied/stale/unavailable state, no external write without explicit approval, source record.
...
Native/Ambitions/Domain; Native/Ambitions/Services/EventKitCalendarService.swift; Time feature; Today feature; Persistence; Native/AmbitionsTests; build/reports/time-operations/.
...
Use Apple Calendar context safely. Implement only the scoped local-first behavior after inspecting current source, then prove it with focused tests and proof artifacts.
