...
```bash
make batch BATCH=SA07 PROMPT=prompts/batches/SA07.md
```
...
Implement the Source Atlas claim state machine with explicit state transitions, provenance requirements, and conservative non-claim defaults.
...
Preserve canonical ID `SA07` and canonical queue order. Do not reactivate completed batches, renumber IDs, collapse PK17-PK41, pull RHC broad cleanup early, turn AIR into a standalone train, turn EFC into a broad implementation stream, or make DPTG terminal proof executable before all pre-device gates close.
...
- Inspect SourceAtlasPackModels, AmbitionsOSSourceTruthModels, AmbitionsOSLivingDreamSourceClaimGraphModels, and the SA train manifest.
- Model claim states such as source-needed, sourced, stale, contradicted, revoked, and verified-by-local-proof only when evidence exists.
- Add tests proving invalid transitions fail and stale/unknown source states do not become confidence claims.
...
Create or update `docs/audits/sa07-batch-closeout-report.md` with status, source truth inspected, files changed, validation commands and exit codes, EFC applicability, accepted-yellow rationale if any, claims not made, rollback notes, and next handoff.
