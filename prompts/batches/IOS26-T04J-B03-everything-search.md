...
# IOS26-T04J-B03 - Sealed IOS26 Work Order
...
Find anything local. Implement only the scoped local-first behavior after inspecting current source, then prove it with focused tests and proof artifacts.
...
Green: Everything search tests pass.
Yellow: bounded gap with owner, reason, no-claim boundary, validation posture, and post-batch gate.
Red: Search requires an external service or omits object/source context.

## Rollback behavior
Revert only files touched by `IOS26-T04J-B03`. Preserve unrelated dirty work and generated artifacts outside this batch.
...
# IOS26-T04J-B03 - Everything search
...
Local find-anything job.

## Replacement P0 contract
Search contract: search all local life objects, object type, source/freshness, primary actions, filters, performance budget, no external service.
