...
- Dependency gate: Complete prior PK batch PK17 and any data-safety proof named by the PK train.
- Next handoff: PK19

# Objective

Extract Today command handling behind the existing Today action flow while preserving receipt and side-effect boundaries.
...
Preserve canonical ID `PK18` and canonical queue order. Do not reactivate completed batches, renumber IDs, collapse PK17-PK41, pull RHC broad cleanup early, turn AIR into a standalone train, turn EFC into a broad implementation stream, or make DPTG terminal proof executable before all pre-device gates close.
...
- Inspect TodayViewModel, TodayFeatureService, AmbitionsCommandExecutor, AmbitionsCommandModels, ActionClosureReceiptModels, and EventLedgerModels.
- Separate command dispatch from read-model projection; commands must produce explicit receipts or ledger evidence where current source supports it.
- Keep UI copy and shell IA unchanged except for any compile-required call-site adjustment.
...
- `scripts/ambitions-xcode-validate.sh --batch PK18 --lane focused-test --test <TouchedOwnerTests>`
...
Create or update `docs/audits/pk18-batch-closeout-report.md` with status, source truth inspected, files changed, validation commands and exit codes, EFC applicability, accepted-yellow rationale if any, claims not made, rollback notes, and next handoff.
