...
```bash
make batch BATCH=PK05 PROMPT=prompts/batches/PK05.md
```
...
- Dependency gate: Not runnable from this prompt; preserve as historical, overlay, or conditional metadata unless active truth reauthorizes execution.
- Next handoff: PK06

# Objective

Do not execute implementation from this historical_complete_do_not_run record; preserve PK05 as canonical coverage and use it only as historical, overlay, or conditional metadata until active truth reauthorizes it.
...
Preserve canonical ID `PK05` and canonical queue order. Do not reactivate completed batches, renumber IDs, collapse PK17-PK41, pull RHC broad cleanup early, turn AIR into a standalone train, turn EFC into a broad implementation stream, or make DPTG terminal proof executable before all pre-device gates close.
...
- Do not run PK05 as an implementation batch from this prompt.
- Use this file to preserve queue coverage for Atomic Clarification / Materialization, the canonical status `historical_complete_do_not_run`, and the next handoff `PK06`.
...
Create or update `docs/audits/pk05-batch-closeout-report.md` with status, source truth inspected, files changed, validation commands and exit codes, EFC applicability, accepted-yellow rationale if any, claims not made, rollback notes, and next handoff.
