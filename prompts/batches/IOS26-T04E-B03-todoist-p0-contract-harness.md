...
# IOS26-T04E-B03 - Sealed IOS26 Work Order
...
## Objective
Install Todoist replacement contract tests/fixtures/proof requirements.
...
Install Todoist replacement contract tests/fixtures/proof requirements. Install explicit fixtures/proof expectations and make later broad claims fail unless current evidence exists.
The contract harness must require `SourceRecord`, local `Receipt`, and `ReplayTrace` evidence before any broad Todoist replacement claim can pass.
...
## Validation commands
```bash
python3 scripts/ios26-flagship-preflight.py --batch IOS26-T04E-B03
python3 scripts/ios26-core-replacement-proof-shape-check.py --batch IOS26-T04E-B03
```
...
Green: Todoist P0 harness exists and blocks unsupported claims.
Yellow: bounded gap with owner, reason, no-claim boundary, validation posture, and post-batch gate.
Red: No explicit Todoist P0 contract harness exists.

## Rollback behavior
Revert only files touched by `IOS26-T04E-B03`. Preserve unrelated dirty work and generated artifacts outside this batch.
...
# IOS26-T04E-B03 - Todoist replacement contract harness
...
Todoist P0 projects, tasks, due/deadline, dependencies, labels, filters, saved views, recurrence, deterministic sort, proof/replay.
