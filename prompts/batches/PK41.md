...
```bash
make batch BATCH=PK41 PROMPT=prompts/batches/PK41.md
```
...
- Dependency gate: Complete prior PK batch PK40 and any data-safety proof named by the PK train.
- Next handoff: SA07

# Objective

Implement Move Feature Engines To Package in the Platform Kernel owner seam, preserving PK serial order, local-first data boundaries, receipts, and compatibility with existing native SwiftUI surfaces.
...
Preserve canonical ID `PK41` and canonical queue order. Do not reactivate completed batches, renumber IDs, collapse PK17-PK41, pull RHC broad cleanup early, turn AIR into a standalone train, turn EFC into a broad implementation stream, or make DPTG terminal proof executable before all pre-device gates close.
...
- Inspect the existing native owner files for Move Feature Engines To Package before patching; do not invent a parallel architecture.
- Land the smallest source change that makes Move Feature Engines To Package real behind the existing app surfaces and domain/service contracts.
...
- `scripts/ambitions-xcode-validate.sh --batch PK41 --lane focused-test --test <TouchedOwnerTests>`
...
Create or update `docs/audits/pk41-batch-closeout-report.md` with status, source truth inspected, files changed, validation commands and exit codes, EFC applicability, accepted-yellow rationale if any, claims not made, rollback notes, and next handoff.
