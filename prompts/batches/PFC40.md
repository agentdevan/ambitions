...
```bash
make batch BATCH=PFC40 PROMPT=prompts/batches/PFC40.md
```
...
Implement Platform Framework Compliance Closeout as platform-framework compliance work with local validation evidence, proof packet updates, and no release/readiness overclaim.
...
Preserve canonical ID `PFC40` and canonical queue order. Do not reactivate completed batches, renumber IDs, collapse PK17-PK41, pull RHC broad cleanup early, turn AIR into a standalone train, turn EFC into a broad implementation stream, or make DPTG terminal proof executable before all pre-device gates close.
...
- Inspect the current proof packets, release truth, and owner files before implementing Platform Framework Compliance Closeout.
...
Create or update `docs/audits/pfc40-batch-closeout-report.md` with status, source truth inspected, files changed, validation commands and exit codes, EFC applicability, accepted-yellow rationale if any, claims not made, rollback notes, and next handoff.
