...
```bash
make batch BATCH=PK38 PROMPT=prompts/batches/PK38.md
```
...
- Dependency gate: Complete prior PK batch PK37 and any data-safety proof named by the PK train.
- Next handoff: PK39

# Objective

Implement Move Domain To Package in the Platform Kernel owner seam, preserving PK serial order, local-first data boundaries, receipts, and compatibility with existing native SwiftUI surfaces.
...
Preserve canonical ID `PK38` and canonical queue order. Do not reactivate completed batches, renumber IDs, collapse PK17-PK41, pull RHC broad cleanup early, turn AIR into a standalone train, turn EFC into a broad implementation stream, or make DPTG terminal proof executable before all pre-device gates close.
...
- Inspect the existing native owner files for Move Domain To Package before patching; do not invent a parallel architecture.
- Land the smallest source change that makes Move Domain To Package real behind the existing app surfaces and domain/service contracts.
...
- `scripts/ambitions-xcode-validate.sh --batch PK38 --lane focused-test --test <TouchedOwnerTests>`
...
Create or update `docs/audits/pk38-batch-closeout-report.md` with status, source truth inspected, files changed, validation commands and exit codes, EFC applicability, accepted-yellow rationale if any, claims not made, rollback notes, and next handoff.
