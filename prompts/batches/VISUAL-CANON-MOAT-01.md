...
```text
Today becomes task list, calendar agenda, motivational wallpaper, dashboard, or detached card stack.
```
...
```text
Goals becomes dashboard, score screen, habit tracker, astrology, decorative constellation wallpaper, or generic goal list.
```
...
```text
Time becomes line graph, terrain, blobs, weather map, analytics dashboard, generic calendar clone, unsafe auto-reflow, or task planner.
```
...
```text
You becomes social profile, generic account page, SaaS/admin dashboard, chatbot settings, AI-wrapper wall, or family hub.
```
