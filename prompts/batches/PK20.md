...
```bash
make batch BATCH=PK20 PROMPT=prompts/batches/PK20.md
```
...
- Dependency gate: Complete prior PK batch PK19 and any data-safety proof named by the PK train.
- Next handoff: PK21

# Objective

Extract Capture service boundaries while keeping the Capture composer as the primary intake surface.
...
Preserve canonical ID `PK20` and canonical queue order. Do not reactivate completed batches, renumber IDs, collapse PK17-PK41, pull RHC broad cleanup early, turn AIR into a standalone train, turn EFC into a broad implementation stream, or make DPTG terminal proof executable before all pre-device gates close.
...
- Inspect CapturesScreen, CapturesViewModel, CaptureAtmosphereComposer, CapturePlacementReviewState, CaptureService, CaptureModels, and SmartAttachmentCaptureAdapter.
- Move routing/classification/service logic behind a Capture-owned service seam; keep notes-feed, inbox, and chatbot framing out.
- Preserve local-first receipt and placement-review behavior.
...
- `scripts/ambitions-xcode-validate.sh --batch PK20 --lane focused-test --test <TouchedOwnerTests>`
...
Create or update `docs/audits/pk20-batch-closeout-report.md` with status, source truth inspected, files changed, validation commands and exit codes, EFC applicability, accepted-yellow rationale if any, claims not made, rollback notes, and next handoff.
