...
def load_resolved_remaining_canon_collapse_ids() -> set[str]:
    if not REMAINING_CANON_COLLAPSE_RESOLUTION_JSON.exists():
        return set()
    try:
        payload = json.loads(REMAINING_CANON_COLLAPSE_RESOLUTION_JSON.read_text(encoding="utf-8"))
    except Exception:
        return set()
    if payload.get("status") != "GREEN":
        return set()
