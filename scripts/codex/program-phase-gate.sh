...
case "$program" in
 ...
 amb-master) rg -n "^## ${phase}([[:space:]-]|$)" artifacts/ambitions-master-build/AMB_MASTER_PHASE_GATES.md >/dev/null || status=1; python3 scripts/codex/amb-master-readiness-validate.py --phase "$phase" || status=1;;
