#!/usr/bin/env python3
"""One-shot repair for PR #23 Wave 0 constitutional review findings."""
...
def bind_annex() -> None:
    ...
    heading = "## AUTH-005A — Normative engineering annex binding"
    if heading not in text:
        marker = "\n## AUTH-006 — Claim discipline\n"
        if marker not in text:
            raise RuntimeError("AUTH-006 insertion marker missing")
        binding = """
...
def normalize_opportunities() -> None:
    p0_source = C / "opportunities" / "P0.json"
    if p0_source.exists():
        p0_items = json.loads(p0_source.read_text())["opportunities"]
    else:
        p0_items: list[dict] = []
        for name in ("P0-001-009.json", "P0-010-018.json"):
            p0_items.extend(json.loads((C / "opportunities" / name).read_text())["opportunities"])
    if len(p0_items) != 18:
        raise RuntimeError(f"expected 18 P0 items, found {len(p0_items)}")

    p0_items = [normalize_opportunity(item) for item in p0_items]
    write_json(C / "opportunities" / "P0-001-009.json", {"schema_version": 1, "opportunities": p0_items[:9]})
    write_json(C / "opportunities" / "P0-010-018.json", {"schema_version": 1, "opportunities": p0_items[9:]})
    if p0_source.exists():
        p0_source.unlink()

    for path in sorted((C / "opportunities").glob("P1-*.json")):
        data = json.loads(path.read_text())
        data["opportunities"] = [normalize_opportunity(item) for item in data["opportunities"]]
        write_json(path, data)

    register_path = C / "opportunity-register.json"
    ...
    graph_path = C / "dependency-graph.json"
    graph = json.loads(graph_path.read_text())
    graph["edge_source_parts"] = register["parts"]
    write_json(graph_path, graph)


def normalize_laws() -> None:
    index = json.loads((C / "laws.json").read_text())
    for part in index["parts"]:
        path = C / part
        data = json.loads(path.read_text())
        for law in data["laws"]:
            law["status"] = "normative-on-merge"
        write_json(path, data)


def expand_scenarios() -> None:
    path = C / "scenarios.json"
    data = json.loads(path.read_text())
    ...
    additions = [
        {
            ...
            "expected": [
                "canonical object remains",
                "provenance becomes unavailable",
                "no silent delete",
                "receipt-backed review only when consequence exists",
            ],
        },
        {
            ...
            "expected": [
                "local mutation and receipt immediately",
                "replay-safe relaunch",
                "no duplicate completion",
                "deterministic later continuity",
            ],
        },
        {
            ...
            "seed": "flagship root surfaces with dense and degraded states",
            "actions": [
                "render device matrix",
                "apply accessibility settings",
                "apply pseudolanguage and RTL",
                "measure interaction",
            ],
            "expected": [
                "semantic-token compliance",
                "stable hierarchy",
                "no clipped or hidden critical action",
                "measured budget compliance",
                "semantic parity",
            ],
        },
        {
            ...
            "expected": [
                "owned SBOM trace",
                "private egress remains blocked",
                "release gate stops unsafe candidate",
                "documented rollback or removal",
            ],
        },
        {
            ...
            "expected": [
                ...
                "user-reviewed export",
                "receipt-backed repair",
                "postmortem and regression follow-up",
            ],
        },
        {
            ...
            "actions": [
                "change locale",
                "change first weekday and 12/24-hour preference",
                "cross DST",
                "render and execute",
            ],
            "expected": [
                "intent preserved",
                "no string-concatenation failure",
                "correct recurrence and date semantics",
                "usable mirrored UI",
    ...
    for item in additions:
        by_id[item["id"]] = item
    data["scenarios"] = [by_id[key] for key in sorted(by_id)]
    write_json(path, data)


def update_docs() -> None:
    annex_path = C / "ENGINEERING_CONSTITUTION.md"
    annex = annex_path.read_text().replace(
    ...
    (C / "PR_23_RUTHLESS_REVIEW.md").write_text(
...
"""
    ...
    readback_path = C / "WAVE_0_READBACK.md"
    ...
    if "## Ruthless review repair" not in readback:
        readback += """
...
def replace_audit() -> None:
    ...
    (ROOT / "scripts" / "ambitions-constitution-audit.py").write_text(audit)
...
def main() -> None:
    ...
    update_docs()
    replace_audit()
    subprocess.run(["python3", "-m", "py_compile", "scripts/ambitions-constitution-audit.py"], cwd=ROOT, check=True)
    subprocess.run(["python3", "scripts/ambitions-constitution-audit.py"], cwd=ROOT, check=True)
