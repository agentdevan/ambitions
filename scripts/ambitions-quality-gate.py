...
REQUIRED_ARCHITECTURE_PATHS = [
    ...
    "Native/Ambitions/Quality/LifeShapeDerivationAudit.swift",
    ...
    "Native/Ambitions/Core/Domain/LifeShapeCorrection.swift",
    "Native/Ambitions/Core/Domain/FixedPoint.swift",
    "Native/Ambitions/Core/Domain/ProtectedBoundary.swift",
    ...
    "Native/Ambitions/Core/Domain/LifeShapeInputRef.swift",
    ...
    "Native/Ambitions/Scenarios/StressScenarios/LifeShapeStressScenarios.swift",
...
def check_release_non_claim_contract() -> list[Finding]:
    script = ROOT / "scripts" / "ambitions-release-non-claim-gate.py"
    if not script.exists():
        return [
            Finding(
                "release-non-claim",
                rel(script),
                "release non-claim gate is missing",
            )
        ]
    ...
    return [
        Finding(
            "release-non-claim",
            rel(script),
            output[:500] if output else "Release non-claim gate failed",
        )
    ]
