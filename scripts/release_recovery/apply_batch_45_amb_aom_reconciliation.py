...
ORDER = [
    "AMB-AOM-04 formal repair report/state reconciliation",
    "AMB-AOM-06 schema decision quality review or replay",
    "AMB-AOM-03 source-behavior audit or replay",
    "AMB-AOM-07 shell/visual foundation scope audit or replay",
    "AMB-AOM-08 Today blocker validation against App Testing Report",
    "AMB-AOM-00/01/02/05 proof-quality closeout",
]
...
def markdown() -> str:
    ...
    for batch, verdict, action, reason in ROWS:
        lines.append(f"| {batch} | {verdict} | {action} | {reason} |")
...
def main() -> int:
    ...
    payload = {"rows": ROWS, "required_order": ORDER}
    (OUT / "AMB-AOM-00-08-reconciliation.json").write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    (OUT / "AMB-AOM-00-08-reconciliation.md").write_text(markdown(), encoding="utf-8")
    print("AMB-AOM 00-08 reconciliation boundary written.")
    return 0
