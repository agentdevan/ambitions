...
function inferGoalMode(goal: LegacyGoal): GoalMode {
...
function inferGoalTempo(goal: LegacyGoal, mode: GoalMode): GoalTempo {
  ...
  if (explicit && Object.values(GoalTempo).includes(explicit as GoalTempo)) {
    return explicit as GoalTempo;
  }

  if (goal.targetDate) {
    return GoalTempo.DeadlineBased;
  }

  if ([GoalMode.Habit, GoalMode.Maintenance, GoalMode.Recovery, GoalMode.DelegatedSupport].includes(mode)) {
    return GoalTempo.Ongoing;
  }

  return GoalTempo.Untimed;
}
...
function buildActor(goal: LegacyGoal): GoalActor {
  ...
  return createDefaultGoalActor(ownership, displayName);
}

function inferRelationship(goal: LegacyGoal, actor: GoalActor, mode: GoalMode): GoalRelationshipKind {
  const explicit =
...
function milestoneStep(goal: LegacyGoal, milestone: GoalMilestone, sectionId: string, mode: GoalMode): Step {
...
function taskToStep(goal: LegacyGoal, task: Task, sectionId: string, mode: GoalMode, baseTiming: GoalTiming): Step {
  return {
    ...
    owner: buildActor(goal),
    timing: taskTiming(task, baseTiming),
...
export function migrateLegacyPlan(params: {
  ...
  const timing = buildGoalTiming(params.goal, mode);
  const now = params.now ?? params.goal.updatedAt;
  const sections: PlanSection[] = [];
  const milestoneSteps = (params.milestones ?? []).map((milestone) =>
    milestoneStep(params.goal, milestone, `section-overview-${params.goal.id}`, mode),
  );
  const activeTasks = (params.tasks ?? [])
    ...
    .map((task) => taskToStep(params.goal, task, `section-active-${params.goal.id}`, mode, timing));
  const upcomingTasks = (params.tasks ?? [])
    ...
    .map((task) => taskToStep(params.goal, task, `section-upcoming-${params.goal.id}`, mode, timing));
  const completedTasks = (params.tasks ?? [])
    .filter((task) => task.status === TaskStatus.Completed)
    .map((task) => taskToStep(params.goal, task, `section-completed-${params.goal.id}`, mode, timing));
...
export function migrateLegacyGoalDraft(params: { goal: LegacyGoal }): GoalDraft {
  const mode = inferGoalMode(params.goal);
  const actor = buildActor(params.goal);
  const relationshipKind = inferRelationship(params.goal, actor, mode);
  const timing = buildGoalTiming(params.goal, mode);
  ...
  return {
    ...
    timing,
    planningStrategy: createDefaultPlanningStrategy(mode),
    progressStrategy: createDefaultProgressStrategy(mode, timing.tempo),
  };
}
