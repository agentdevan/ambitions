...
Current M04 status: In Progress in Linear. `AMB-668` / `PLOS-040` through `AMB-671` / `PLOS-043` are Done. Current child is `AMB-672` / `PLOS-044` for release ring and rollback manifest behavior. Live Linear verification found `AMB-971` Canceled/non-authoritative under `AMB-612`; do not execute it. Later duplicate-looking Backlog children `AMB-730` through `AMB-737` are not marked Duplicate/Canceled by Linear as of this run and remain active parent-closeout blockers unless resolved in Linear, executed, or explicitly accepted non-blocking/Yellow with no-claim boundaries.
