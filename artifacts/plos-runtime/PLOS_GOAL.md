...
For AMB-660 execution, correct closeout says: reports/validation/control-plane artifacts only; no app source changes; no runtime features; compaction/annual snapshot policy is documentation/planning only; no compaction engine, annual snapshot model, SwiftData migration/index, export/reset/archive UX, privacy manifest change, CloudKit/R2 implementation, release, privacy/legal, performance, accessibility, device, or runtime behavior changed; AMB-610 / PLOS-M02 parent acceptance is the next eligible gate only after AMB-660 is committed, pushed to `main`, moved to Done in Linear, and all live-resolved M02 children are verified Done.
