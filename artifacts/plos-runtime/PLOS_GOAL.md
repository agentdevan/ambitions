...
For AMB-666 execution, correct closeout says: reports/validation/control-plane artifacts only; no app source changes; no runtime features; third-party SDK minimization policy is documentation/planning only; no SDK removal, dependency change, package manifest change, scanner installation, CI implementation, hosted service, telemetry/analytics/crash SDK, security SDK, external AI SDK, signing automation, credential provisioning, Cloudflare/R2 configuration, production pack publication, release, privacy/legal, security certification, performance, accessibility, device, or runtime behavior changed; AMB-667 / PLOS-036 is the next eligible child only after AMB-666 is committed, pushed to `main`, and updated in Linear.
