...
extension TimeLifeSuiteProjector {
    ...
    func semanticMarks(
        ...
        var marks = [
            ...
            makeMark(kind: .recoveryNeed, valueLabel: recoveryNeed > 0.55 ? "Needed" : "Reserve", detail: "Recovery need keeps room available without shame.", intensity: recoveryNeed, visualState: recoveryNeed > 0.55 ? .warning : .default, inputRefs: [weekInput]),
            makeMark(kind: .freeTimeQuality, valueLabel: openDays == 0 ? "Thin" : "\(openDays) open", detail: "Free time shows where action may fit.", intensity: freeTimeQuality, visualState: openDays == 0 ? .warning : .selected, inputRefs: [weekInput]),
