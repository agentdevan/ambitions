...
struct SwiftDataExecutionLedgerReplayInspectionRepository: ExecutionLedgerReplayInspectionRepository {
    ...
    private struct ReplayCandidate: Sendable {
    ...
    private static func receiptID(from metadata: [String: String]) -> String? {
    ...
    private static func commandSort(_ lhs: AmbitionsCommandExecutionRecord, _ rhs: AmbitionsCommandExecutionRecord) -> Bool {
        let lhsDate = PersistedTemporalValue.date(from: lhs.recordedAt)
        let rhsDate = PersistedTemporalValue.date(from: rhs.recordedAt)
        if lhsDate == rhsDate {
            return lhs.id > rhs.id
