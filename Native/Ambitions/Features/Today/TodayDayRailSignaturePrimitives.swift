...
struct DayRailRhythmStrip: View {
    ...
    @ViewBuilder
    private var rhythmItems: some View {
    ...
    private func value(for slot: DayRailRowSlot) -> String {
        switch slot {
        case .now:
    ...
    private func stateForSlot(_ slot: DayRailRowSlot) -> AmbitionSemanticState {
...
private struct DayRailRhythmItem: View {
    @Environment(\.ambitionTheme) private var theme
