    ...
    func postureSummary(
        ...
        if activeGoalCount == 0 {
            return InsightsPostureSummary(
                title: "Insights are waiting for live evidence",
                detail: "This screen becomes useful once active goals, progress signals, and feedback start accumulating.",
                label: "Quiet",
                visualState: .default
            )
        }
        if blockedCount > 0 {
            return InsightsPostureSummary(
                title: "Some goals need clarification before the signal gets cleaner",
                detail: "Blocked or clarification-first goals are the strongest source of uncertainty in the current read.",
                label: "Needs care",
                visualState: .warning
            )
        }
        ...
        if completionCount + minimumCount == 0 {
            return InsightsPostureSummary(
                ...
                detail: "Insights can only stay honest here: active goals exist, but this week still needs more visible evidence before a stronger claim would be trustworthy.",
