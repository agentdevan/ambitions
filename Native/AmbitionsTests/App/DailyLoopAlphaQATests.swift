...
final class DailyLoopAlphaQATests: XCTestCase {
    ...
    func testDailyLoopEmptyAndReturnPathsRouteToOneRealAction() {
        ...
        XCTAssertEqual(capture.title, "Capture Anything")
        XCTAssertEqual(capture.primaryAction.routingHint, .today)
        XCTAssertEqual(capture.secondaryAction?.routingHint, .createGoal)
        XCTAssertTrue(capture.explanation.contains("Start here"))
        XCTAssertTrue(capture.explanation.contains("Create goal"))
        XCTAssertTrue(capture.explanation.contains("Shape time"))
