...
final class SemanticDesignTokenCatalogTests: XCTestCase {
    func testSemanticTokenSnapshotIsStableAndCanonical() {
        XCTAssertEqual(
            ...
            goals.constellationAtlas | Goals | Life Area Atlas | Goal-thread linkage, path proof, and ambition direction | dark=#A9C0D6/#141A24/#BBD2E6 | increased=#A9C0D6/#0B0D10/#F1DEAA | reduceTransparency=Use solid celestial field instead of translucent depth. | symbol=sparkle.magnifyingglass
            ...
            time.lifeShapeField | Time | Life Calendar / Time Texture | Availability, capacity, protected time, and pressure | dark=#89A4C2/#101722/#A9C3DE | increased=#89A4C2/#0B0D10/#F1DEAA | reduceTransparency=Use opaque field bands with shape and label cues. | symbol=clock.badge.checkmark
            motion.motionCurrent | Motion | Stage Motion behavior | Inspectable movement, recovery, and return without pressure metrics | dark=#8BC6A8/#101915/#A9DDBF | increased=#8BC6A8/#0B0D10/#F1DEAA | reduceTransparency=Use opaque history rows with visible labels. | symbol=waveform.path.ecg
    ...
    func testSemanticTokensAvoidForbiddenTopLevelAndAccessibilityBreakingTheming() {
        ...
        XCTAssertFalse(searchable.localizedCaseInsensitiveContains(["Plan", "tab"].joined(separator: " ")))
    ...
    func testAMB1191SemanticFoundationTokenAuditCoversThemeMotionHapticsAndGlyphs() {
        XCTAssertTrue(
            AmbitionSemanticFoundationTokenAudit.validationFailures().isEmpty,
            AmbitionSemanticFoundationTokenAudit.validationFailures().joined(separator: "\n")
        )
        XCTAssertEqual(
            Set(AmbitionFoundationTokenFamily.allCases),
            [.color, .material, .typography, .spacing, .motion, .haptics, .semanticGlyph]
        )
        XCTAssertTrue(AmbitionFoundationMotionRole.allCases.allSatisfy { $0.themeBridge.hasPrefix("AmbitionTheme.") && !$0.rule.isEmpty })
        XCTAssertTrue(AmbitionFoundationHapticRole.allCases.allSatisfy { $0.themeBridge.hasPrefix("AmbitionTheme.") && !$0.rule.isEmpty })
        XCTAssertEqual(AmbitionSemanticGlyphRole.globalCapture.symbolName, "square.and.pencil")
        XCTAssertEqual(AmbitionSemanticGlyphRole.search.symbolName, "magnifyingglass")
        XCTAssertEqual(AmbitionSemanticGlyphRole.protectedTime.accessibilityLabel, "Protected time")
        XCTAssertFalse(AmbitionSemanticFoundationTokenAudit.snapshot.localizedCaseInsensitiveContains("beta"))
    }
