...
final class TodayViewModelTests: XCTestCase {
    ...
    func testAFRI022ActionClosureConfirmationPersistsReceiptAndFeedsLocalReplayInspection() async throws {
        ...
        XCTAssertEqual(mutation.stageMutation.proofArtifact.state, .available)
        XCTAssertEqual(mutation.stageMutation.proofArtifact.beforeSnapshot?.summary, "Today before closure \(mutation.record.id)")
        XCTAssertEqual(mutation.stageMutation.proofArtifact.action?.commandID, "closure.\(mutation.record.id)")
        XCTAssertEqual(mutation.stageMutation.proofArtifact.afterSnapshot?.summary, "Today after closure still_counts")
        XCTAssertEqual(mutation.stageMutation.receipt.proofArtifactID, mutation.stageMutation.proofArtifact.artifactID)
        XCTAssertEqual(mutation.stageMutation.undoAvailability.isAvailable, true)
        XCTAssertEqual(mutation.stageMutation.undoAvailability.restoresSnapshot?.summary, "Today before closure \(mutation.record.id)")
